...
let currentRestaurantCategory = null; // catégorie restaurant active (pour le bouton retour)
let cart              = {};
let locMode           = 'gps';
let gpsCoords         = null;
...
// ════════════════════════════════════════
const VIEWS = ['list','restaurants','rest-category','kits','kit-detail','immo-options','immo-form','catalogue','form','delivery','payment','success'];
function showView(v) {
  VIEWS.forEach(x => {
...
// ── Catégories restaurants (style GoChap) ──
const RESTAURANT_CATEGORIES = [
  {
    id: 'plats_locaux',
    label: '🇹🇬 Plats Locaux Togolais',
    emoji: '🇹🇬',
    desc: 'Fufu, akumé, gboma dessi, aboboi et spécialités togolaises',
    bg: 'linear-gradient(135deg,#E8F5E9,#C8E6C9)',
    icoBg: '#C8E6C9',
    field: 'categorie'
  },
  {
    id: 'cuisine_africaine',
    label: '🌍 Cuisine Africaine',
    emoji: '🌍',
    desc: 'Thiéboudienne, attiéké, ndolé, kedjenou et saveurs du continent',
    bg: 'linear-gradient(135deg,#FFF8E1,#FFE082)',
    icoBg: '#FFE082',
    field: 'categorie'
  },
  {
    id: 'cuisine_europeenne',
    label: '🍝 Cuisine Européenne & Internationale',
    emoji: '🍝',
    desc: 'Pizza, pâtes, burgers gastronomiques, sushi et cuisine du monde',
  ...
  },
  {
    ...
    emoji: '🍟',
    desc: 'Sandwichs, tacos, brochettes, omelettes, snacks rapides',
  ...
  {
    id: 'repas_entreprise',
    label: '🍱 Repas d\'Entreprise',
    emoji: '🍱',
    desc: 'Plateaux repas, buffets, traiteur pour réunions et événements',
    bg: 'linear-gradient(135deg,#EDE7F6,#CE93D8)',
    icoBg: '#CE93D8',
    field: 'categorie'
  },
  {
    id: 'boissons_desserts',
    label: '🥤 Boissons & Desserts',
    emoji: '🥤',
    desc: 'Jus frais, smoothies, gâteaux, glaces et douceurs',
  ...
  {
    id: 'cuisine_maison',
    label: '🧺 Cuisine Maison & Produits Frais',
    emoji: '🧺',
    desc: 'Plats faits maison, légumes, épices et produits du marché livrés',
...
// ── Afficher les catégories de restaurants ──
function loadRestaurantsList() {
  if (!currentUser) { openAuthModal('login'); showToast('⚠️ Connectez-vous pour passer une commande', '#F5820A'); return; }
  const container = document.getElementById('rst-categories-list');
  if (!container) return;

  container.innerHTML = RESTAURANT_CATEGORIES.map(cat => `
    <div class="rst-cat-card" onclick="openRestaurantCategory('${cat.id}','${cat.label.replace(/'/g,"\\'")}','${cat.emoji}')">
      <div class="rst-cat-ico-wrap" style="background:${cat.icoBg}">${cat.emoji}</div>
      <div class="rst-cat-body">
        <div class="rst-cat-name">${cat.label}</div>
        <div class="rst-cat-desc">${cat.desc}</div>
      </div>
      <div class="rst-cat-arrow">›</div>
    </div>
  `).join('');
}
window.loadRestaurantsList = loadRestaurantsList;

// ── Ouvrir une catégorie → afficher les restaurants de cette catégorie ──
async function openRestaurantCategory(catId, catLabel, catEmoji) {
  // Mémoriser la catégorie active (pour le bouton retour depuis le menu d'un restaurant)
  currentRestaurantCategory = catId;

  // Mettre à jour le header de la vue catégorie
  const icoEl = document.getElementById('rst-cat-ico');
  const titleEl = document.getElementById('rst-cat-title');
  const sectionEl = document.getElementById('rst-cat-section-title');
  if (icoEl) icoEl.textContent = catEmoji;
  if (titleEl) titleEl.textContent = catLabel;
  if (sectionEl) sectionEl.textContent = `🏪 Restaurants — ${catLabel}`;

  showView('rest-category');
  ...
  try {
    let snap;
    try {
      const q = query(collection(db,'restaurants'), where('categorie','==',catId), orderBy('ordre','asc'));
      snap = await getDocs(q);
    } catch(_) {
      try {
        const q2 = query(collection(db,'restaurants'), where('categorie','==',catId));
        snap = await getDocs(q2);
      } catch(__) {
        const q3 = query(collection(db,'restaurants'), orderBy('ordre','asc'));
  ...
  // Fusionner DB + standards non encore présents en DB (filtrés par catégorie si possible)
  const dbIds = new Set(dbRestaurants.map(r => r.id));
  const stdRests = DEFAULT_RESTAURANTS
    .filter(r => !dbIds.has(r.id) && (!r.categorie || r.categorie === catId))
  ...
  if (!allRests.length) {
    container.innerHTML = `<div style="text-align:center;padding:40px 20px;color:#9999BB">
      <div style="font-size:40px;margin-bottom:12px">${catEmoji}</div>
      <div style="font-size:14px;font-weight:600;margin-bottom:6px">Aucun restaurant disponible</div>
      <div style="font-size:12px">Aucun partenaire dans cette catégorie pour le moment.<br/>Revenez bientôt !</div>
    </div>`;
    return;
  }
...
window.openRestaurantCategory = openRestaurantCategory;
...
async function openRestaurant(restaurantId, restaurantNom, restaurantEmoji) {
  ...
  // Le bouton retour revient à la liste de la catégorie si on vient d'une catégorie, sinon aux restaurants
  const backBtn = document.getElementById('catalogue-back-btn');
  if (backBtn) backBtn.onclick = () => showView(currentRestaurantCategory ? 'rest-category' : 'restaurants');
