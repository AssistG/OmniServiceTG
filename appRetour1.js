...
// ════════════════════════════════════════
// Stratégie : on maintient une pile (_navStack) qui reflète l'état actuel
// de l'interface (onglet actif + vue interne). À chaque navigation, on
// pushState un état vide pour "occuper" un slot dans l'historique du
// navigateur. Quand l'utilisateur appuie sur le retour physique, le
// navigateur déclenche "popstate" → on lit la pile et on navigue en arrière
// dans l'app au lieu de quitter.

(function () {

  // ── Pile de navigation interne ──
  // Chaque entrée : { tab, view, extra }
  // tab  : id de l'onglet actif ('home', 'services', 'orders', 'profile', 'notifs')
  // view : vue interne services active (ou null si tab ≠ 'services')
  // extra: données contextuelles libres (non utilisé pour la navigation, réservé)
  const _navStack = [];

  // ── État courant (snapshot avant chaque navigation) ──
  function _currentState() {
    // Onglet actif
    const activeTab = (() => {
      const el = document.querySelector('.btab.on');
      if (!el) return 'home';
      return el.id.replace('t-', '');
    })();

    // Vue interne active (pour l'onglet services)
    const activeView = (() => {
  ...
  // ── Pousser un état dans la pile + dans l'historique du navigateur ──
  function _pushNav(state) {
    _navStack.push(state);
    // On push un état identifié dans l'historique du navigateur
    // (la valeur du state n'est pas utilisée pour la navigation : on lit _navStack)
    history.pushState({ omni: true, depth: _navStack.length }, '');
  }

  // ── Intercepter goTab pour alimenter la pile ──
  const _origGoTab = window.goTab;
  window.goTab = function (id) {
    const before = _currentState();
    _origGoTab(id);
    const after = _currentState();
    // Ne pas empiler si rien n'a changé
    if (before.tab !== after.tab || before.view !== after.view) {
      _pushNav(before);
    }
  };

  // ── Intercepter showView pour alimenter la pile ──
  const _origShowView = window.showView;
  window.showView = function (v) {
    const before = _currentState();
    _origShowView(v);
    // Pousser seulement si la vue a changé
    if (before.view !== v) {
      _pushNav(before);
    }
  };

  // ── Intercepter openService pour alimenter la pile ──
  ...
  // ── Naviguer vers un état sauvegardé ──
  function _restoreState(state) {
    if (!state) return;
    const { tab, view } = state;

    // Restaurer l'onglet (appel direct sans passer par le wrapper)
    _origGoTab(tab);

    // Restaurer la vue interne si on revient dans 'services'
  ...
  // ── Variable pour la confirmation de sortie ──
  let _exitPending = false;
  let _exitTimer   = null;

  // ── Toast de confirmation de sortie ──
  function _showExitToast() {
    // Réutiliser showToast si disponible, sinon créer un toast temporaire
    if (typeof showToast === 'function') {
      showToast('Appuyez encore une fois pour quitter OmniService TG', '#1E6FBE');
    } else {
      // Fallback : toast maison
      let toast = document.getElementById('_exit-toast');
      if (!toast) {
        toast = document.createElement('div');
        toast.id = '_exit-toast';
        toast.style.cssText = [
          'position:fixed', 'bottom:90px', 'left:50%', 'transform:translateX(-50%)',
          'background:#1E6FBE', 'color:#fff', 'padding:12px 22px',
      ...
      toast.textContent = 'Appuyez encore une fois pour quitter OmniService TG';
  ...
  // ── Gestionnaire popstate (bouton retour physique) ──
  window.addEventListener('popstate', function (e) {
    // Si la modale auth est ouverte → la fermer
    const authModal = document.getElementById('auth-modal');
    if (authModal && authModal.style.display === 'flex') {
      authModal.style.display = 'none';
      // Repousser un état pour rester dans l'app
      history.pushState({ omni: true, depth: _navStack.length }, '');
      return;
    }

    // S'il reste des états dans la pile → revenir en arrière dans l'app
    if (_navStack.length > 0) {
      const prevState = _navStack.pop();
      _restoreState(prevState);
      // Repousser un état pour que le prochain popstate soit intercepté
      history.pushState({ omni: true, depth: _navStack.length }, '');
      // Réinitialiser la confirmation de sortie
      _exitPending = false;
      if (_exitTimer) { clearTimeout(_exitTimer); _exitTimer = null; }
      return;
    }

    // Pile vide → on est sur l'écran d'accueil
    // Premier appui : avertir l'utilisateur
    if (!_exitPending) {
      _exitPending = true;
      _showExitToast();
      // Repousser un état pour intercepter le prochain popstate
      history.pushState({ omni: true, depth: 0 }, '');
      // Réinitialiser après 2,5s si l'utilisateur ne confirme pas
    ...
    // Deuxième appui consécutif → quitter réellement
    _exitPending = false;
    if (_exitTimer) { clearTimeout(_exitTimer); _exitTimer = null; }
    // Laisser l'événement popstate passer naturellement → le navigateur quitte
    // Mais comme on a pushState, on doit manuellement reculer
    history.go(-1);
  });

  // ── Initialisation : pousser un état de base pour capturer le premier retour ──
  // On attend que le DOM soit prêt
  function _initHistory() {
    // Remplacer l'état courant (ne pas ajouter d'entrée)
    history.replaceState({ omni: true, depth: 0 }, '');
    // Pousser un premier état sentinel pour que le premier popstate soit intercepté
    history.pushState({ omni: true, depth: 0 }, '');
  }
