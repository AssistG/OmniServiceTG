...
function openService(id) {
  ...
  if (CATALOGUE_SERVICES.includes(id)) {
    ...
    // Réinitialiser le bouton retour vers la liste des services (évite qu'il reste câblé sur 'restaurants' après une visite restaurant)
...
function renderRestCarouselCard(r, fallbackBg) {
  ...
  const img   = r.imageUrl
    ...
    : '';
  return `
  <div class="rst-carousel-card" onclick="openRestaurant('${r.id}','${nom}','${emoji}')">
...
// ════════════════════════════════════════
// LIVRAISON SHEET — NAVIGATION PAR ÉTAPES
// ════════════════════════════════════════

let _livCurrentStep = 1;

function _livSetStep(step) {
  _livCurrentStep = step;
  [1,2,3].forEach(s => {
    const el = document.getElementById('liv-step-' + s);
    if (el) el.style.display = s === step ? 'block' : 'none';
  });
  // Barre de progression
  [1,2,3].forEach(s => {
    const st = document.getElementById('omni-st-' + s);
    if (!st) return;
    st.className = 'omni-step' + (s < step ? ' done' : s === step ? ' active' : '');
    if (s < step) st.querySelector('.omni-step-num').textContent = '✓';
    else st.querySelector('.omni-step-num').textContent = s;
  });
  [1,2].forEach(s => {
    const ln = document.getElementById('omni-stl-' + s);
    if (ln) ln.className = 'omni-step-line' + (s < step ? ' done' : '');
  });
  // En-tête
  const titles = {1:'Point de départ', 2:'Point d\'arrivée', 3:'Paiement'};
  const subs   = {1:'Étape 1 sur 3', 2:'Étape 2 sur 3', 3:'Étape 3 sur 3'};
  const titleEl = document.getElementById('liv-hdr-title');
  const subEl   = document.getElementById('liv-hdr-sub');
  if (titleEl) titleEl.textContent = titles[step];
  if (subEl)   subEl.textContent   = subs[step];
  // Bouton retour
  const backBtn = document.getElementById('liv-back-btn');
  if (backBtn) backBtn.style.display = step > 1 ? 'block' : 'none';
  // Scroll haut de la feuille
  const sheet = document.querySelector('#sheet-livraison .omni-sheet');
  if (sheet) sheet.scrollTop = 0;
}

function livGoBack() {
  if (_livCurrentStep > 1) _livSetStep(_livCurrentStep - 1);
}
window.livGoBack = livGoBack;

function livNextStep(from) {
  if (from === 1) {
    // Valider Point A
    if (_livState.A.lat === null) {
      showToast('⚠️ Définissez le point de départ (GPS ou zone)', '#F5820A');
      return;
    }
    // Mettre à jour résumé A
    const recapA = document.getElementById('liv-recap-A');
    if (recapA) recapA.innerHTML = `🟢 <strong>Départ :</strong> ${_livState.A.label}${document.getElementById('omni-desc-A')?.value.trim() ? ' — ' + document.getElementById('omni-desc-A').value.trim() : ''}`;
    _livSetStep(2);
  } else if (from === 2) {
    // Valider Point B
    if (_livState.B.lat === null) {
      showToast('⚠️ Définissez le point d\'arrivée (GPS ou zone)', '#F5820A');
      return;
    }
    // Vérification anti-doublon : même zone choisie
    if (_livState.A.label && _livState.B.label && _livState.A.label === _livState.B.label) {
      showToast('⚠️ Le point de départ et d\'arrivée sont identiques !', '#F5820A');
      return;
    }
    // Vérification anti-doublon par proximité GPS (< 200 m)
    if (_livState.A.lat !== null && _livState.B.lat !== null) {
      const dist = _haversine(_livState.A.lat, _livState.A.lng, _livState.B.lat, _livState.B.lng);
      if (dist < 0.2) {
        showToast('⚠️ Les deux points sont trop proches (< 200 m)', '#F5820A');
        return;
      }
    }
    // Calculer tarif
    const km = _haversine(_livState.A.lat, _livState.A.lng, _livState.B.lat, _livState.B.lng);
    const {fare, detail} = _fmtFare(km);
    _livState.fare = fare;
    const fmtN = n => n.toLocaleString('fr-FR') + ' FCFA';
    // Mettre à jour UI étape 3
    document.getElementById('omni-fare-liv-km').textContent    = km.toFixed(1) + ' km';
    document.getElementById('omni-fare-liv-price').textContent = fmtN(fare);
    document.getElementById('omni-fare-liv-detail').textContent = detail;
    document.getElementById('omni-liv-fare-line').textContent  = fmtN(fare);
    document.getElementById('omni-liv-total').textContent      = fmtN(fare);
    // Résumé trajet
    const recap = document.getElementById('liv-recap-trajet');
    if (recap) {
      const descA = document.getElementById('omni-desc-A')?.value.trim() || '';
      const descB = document.getElementById('omni-desc-B')?.value.trim() || '';
      recap.innerHTML =
        `🟢 <strong>Départ :</strong> ${_livState.A.label}${descA ? ' — ' + descA : ''}<br/>` +
        `🔴 <strong>Arrivée :</strong> ${_livState.B.label}${descB ? ' — ' + descB : ''}`;
    }
    _livSetStep(3);
  }
}
window.livNextStep = livNextStep;

// Patch openDeliverySheet pour initialiser l'étape 1
const _origOpenDeliverySheet = window.openDeliverySheet;
window.openDeliverySheet = function() {
  _origOpenDeliverySheet();
  _livSetStep(1);
};


// ════════════════════════════════════════
// LOC-SIMPLE : Livraison pour services catalogue (food, marketplace, etc.)
// ════════════════════════════════════════

const _locSimpleState = { lat: null, lng: null, label: '', selectedPay: 'livraison' };

// Interception : pour les services catalogue, on redirige le cart bar
// vers loc-simple au lieu de l'ancienne vue delivery
const _origOpenCartCheckout = window.openCartCheckout;
window.openCartCheckout = function() {
  if (currentService === 'restaurant' && currentRestaurant) {
    _openRstLocFromCurrentRestaurant();
    return;
  }
  if (CATALOGUE_SERVICES.includes(currentService)) {
    _openLocSimple();
    return;
  }
  // kits et autres → ancienne vue delivery
  showView('delivery');
};

function _openLocSimple() {
  if (!currentUser) { openAuthModal('login'); return; }
  // Reset state
  _locSimpleState.lat = null;
  _locSimpleState.lng = null;
  _locSimpleState.label = '';
  _locSimpleState.selectedPay = 'livraison';
  // Reset UI
  const titleEl  = document.getElementById('loc-simple-gps-title');
  const coordsEl = document.getElementById('loc-simple-gps-coords');
  const statusEl = document.getElementById('loc-simple-gps-status');
  const card     = document.getElementById('loc-simple-gps-card');
  const btn      = document.getElementById('loc-simple-gps-btn');
  const zoneEl   = document.getElementById('loc-simple-zone');
  const descEl   = document.getElementById('loc-simple-desc');
  if (titleEl)  titleEl.textContent  = 'Localisation GPS';
  if (coordsEl) coordsEl.textContent = 'Non détectée';
  if (statusEl) statusEl.textContent = 'Appuyez pour détecter';
  if (card)     card.classList.remove('active');
  if (btn)      { btn.disabled = false; btn.textContent = '📍 Détecter'; }
  if (zoneEl)   zoneEl.value = '';
  if (descEl)   descEl.value = '';
  // Back button target
  const backBtn = document.getElementById('loc-simple-back-btn');
  if (backBtn) backBtn.onclick = () => showView('catalogue');
  // Add to VIEWS if not present
  if (!VIEWS.includes('loc-simple'))  VIEWS.push('loc-simple');
  if (!VIEWS.includes('pay-simple'))  VIEWS.push('pay-simple');
  showView('loc-simple');
}

function locSimpleDetectGPS() {
  const btn  = document.getElementById('loc-simple-gps-btn');
  const card = document.getElementById('loc-simple-gps-card');
  if (btn) { btn.disabled = true; btn.innerHTML = '<span class="omni-spin"></span>'; }

  const onSuccess = pos => {
    const {latitude: lat, longitude: lng} = pos.coords;
    _locSimpleState.lat = lat;
    _locSimpleState.lng = lng;
    _locSimpleState.label = 'Position GPS';
    // Reset zone select
    const zoneEl = document.getElementById('loc-simple-zone');
    if (zoneEl) zoneEl.value = '';
    document.getElementById('loc-simple-gps-title').textContent  = 'Position détectée ✅';
    document.getElementById('loc-simple-gps-coords').textContent = `${lat.toFixed(5)}° N, ${lng.toFixed(5)}° E`;
    document.getElementById('loc-simple-gps-status').textContent = 'Position actuelle';
    if (card) card.classList.add('active');
    if (btn)  { btn.disabled = false; btn.textContent = '🔄 Actualiser'; }
  };
  const onError = () => {
    // Démo fallback
    const lat = 6.1375 + ((Math.random() - .5) * .04);
    const lng = 1.2123 + ((Math.random() - .5) * .04);
...
window.locSimpleDetectGPS = locSimpleDetectGPS;

function locSimpleSelectZone() {
  const sel = document.getElementById('loc-simple-zone');
  const val = sel.value;
  if (!val) { _locSimpleState.lat = null; _locSimpleState.lng = null; _locSimpleState.label = ''; return; }
  const zone = OMNI_ZONES[val];
  if (!zone) return;
  _locSimpleState.lat   = zone.lat;
  _locSimpleState.lng   = zone.lng;
  _locSimpleState.label = val;
  // Effacer GPS card
  const card = document.getElementById('loc-simple-gps-card');
  if (card) card.classList.remove('active');
}
window.locSimpleSelectZone = locSimpleSelectZone;

function locSimpleGoToPayment() {
  if (_locSimpleState.lat === null) {
    showToast('⚠️ Définissez votre lieu de livraison (GPS ou zone)', '#F5820A');
    return;
  }
  const desc  = document.getElementById('loc-simple-desc')?.value.trim() || '';
  const label = _locSimpleState.label;

  // Remplir récap paiement
  const locDetail = document.getElementById('pay-simple-loc-detail');
  if (locDetail) {
    locDetail.innerHTML = `📍 <strong>${label}</strong>${desc ? '<br/><span style="font-size:11px;color:#9999BB">' + desc + '</span>' : ''}`;
  }
  const items = Object.values(cart);
  let recapHtml = items.map(a => `
    <div class="recap-item">
      <div><span class="recap-name">${a.name}</span><span class="recap-qty">x${a.qty}</span></div>
      <div class="recap-price">${(a.price * a.qty).toLocaleString('fr-FR')} FCFA</div>
    </div>`).join('');
  const recapEl = document.getElementById('pay-simple-recap-items');
  const totalEl = document.getElementById('pay-simple-total-val');
  if (recapEl) recapEl.innerHTML = recapHtml;
  if (totalEl) totalEl.textContent = cartTotal().toLocaleString('fr-FR') + ' FCFA';
  // Reset paiement
  selectSimplePay('livraison');
  if (!VIEWS.includes('pay-simple')) VIEWS.push('pay-simple');
  showView('pay-simple');
}
window.locSimpleGoToPayment = locSimpleGoToPayment;

function selectSimplePay(mode) {
  _locSimpleState.selectedPay = mode;
  ['tmoney', 'flooz', 'livraison'].forEach(m => {
    const el = document.getElementById('pay-simple-' + m);
    if (el) el.classList.toggle('selected', m === mode);
  });
}
window.selectSimplePay = selectSimplePay;

async function confirmSimpleOrder() {
  if (!currentUser) { openAuthModal('login'); return; }
  const btn = document.getElementById('pay-simple-confirm-btn');
  if (btn) { btn.innerHTML = '<span class="spinner"></span> Enregistrement...'; btn.disabled = true; }
  ...
  try {
    const docRef = await addDoc(collection(db, 'commandes'), {
      service:        currentService,
      serviceName:    svc?.name || currentService,
      statut:         'En attente',
      uid:            currentUser.uid,
      clientNom:      currentUser.nom    || '',
      clientPrenom:   currentUser.prenom || '',
      ...
      phone:          currentUser.phone  || '',
      clientVille:    currentUser.ville  || '',
      livraisonZone:  _locSimpleState.label,
      livraisonLat:   _locSimpleState.lat,
      livraisonLng:   _locSimpleState.lng,
      livraisonDesc:  desc,
      articles:       items,
      total,
      modePaiement:   _locSimpleState.selectedPay,
      paygateStatus:  'non_configure',
      createdAt:      serverTimestamp()
    });
    const fmtN = n => n.toLocaleString('fr-FR') + ' FCFA';
    document.getElementById('succ-msg').innerHTML =
      `Commande <strong style="color:var(--blue)">${svc?.name || currentService}</strong> confirmée !<br/>
       Référence : <strong>#${docRef.id.slice(0, 8).toUpperCase()}</strong><br/>
       Livraison : <strong>${_locSimpleState.label}</strong>${desc ? ' — ' + desc : ''}<br/>
       Total : <strong>${fmtN(total)}</strong><br/>
       <small style="color:var(--light)">Contact : ${currentUser.phone || ''}</small>`;
    cart = {};
    updateCartBar();
    showSuccessView();
...
window.confirmSimpleOrder = confirmSimpleOrder;

// ════════════════════════════════════════
// OMNI LOCALISATION — MODULE PARTAGÉ
// ════════════════════════════════════════

// ── Zones de Lomé ──
const OMNI_ZONES = {
  'Tokoin':        {lat:6.1520, lng:1.2220},
  'Adidogomé':     {lat:6.1050, lng:1.1780},
  'Bè':            {lat:6.1370, lng:1.2310},
  'Agbalépédogan': {lat:6.1200, lng:1.2150},
  'Hédzranawoé':   {lat:6.1450, lng:1.2050},
  'Kégué':         {lat:6.1280, lng:1.2400},
  'Agoè':          {lat:6.1800, lng:1.2100},
  'Cacavéli':      {lat:6.1150, lng:1.1900},
  'Kodjoviakopé':  {lat:6.1310, lng:1.2290},
  'Nyékonakpoè':   {lat:6.1390, lng:1.2260},
  'Baguida':       {lat:6.1000, lng:1.3100},
  'Aflao':         {lat:6.1000, lng:1.1900},
  'Lomé Centre':   {lat:6.1375, lng:1.2123},
  'Djidjolé':      {lat:6.1600, lng:1.2000},
  'Wété':          {lat:6.1420, lng:1.2180},
};
...
function _calcFare(km){
  if(km<=0) return 0;
  if(km<2)  return 500;
  return 500+Math.ceil(km-1)*100;
}
function _fmtFare(km){
  const fare=_calcFare(km);
  const extra=km>=2?` (500 + ${Math.ceil(km-1)}×100 FCFA)`:'(forfait)';
  return {fare, detail:`${km.toFixed(1)} km · ${extra}`};
}
function _fmtFCFA(n){ return n.toLocaleString('fr-FR')+' FCFA'; }

// ── GPS générique ──
function _detectGPS(onSuccess, onError){
  if(!navigator.geolocation){ onError(); return; }
  navigator.geolocation.getCurrentPosition(
...
function _simulatePos(){ // fallback démo Lomé
  return {lat:6.1375+((Math.random()-.5)*.04), lng:1.2123+((Math.random()-.5)*.04)};
}

// ── Mise à jour UI GPS card ──
function _setGpsCard(prefix, title, coords, status, active){
  const t=document.getElementById(prefix+'-title');
  const c=document.getElementById(prefix+'-coords');
  const s=document.getElementById(prefix+'-status');
  const card=document.getElementById(prefix+'-card');
  const btn=document.getElementById(prefix+'-btn');
  if(t) t.textContent=title;
  if(c) c.textContent=coords;
  if(s) s.textContent=status;
  if(card) card.classList.toggle('active',active);
  if(btn){ btn.disabled=false; btn.textContent=active?'🔄 Actualiser':'📍 Détecter'; }
}


// ════════════════════════════════════════
// LIVRAISON & COURSE — 3 ÉTAPES
// ════════════════════════════════════════

let _livStep = 1;
const _livState = {
  A:      {lat:null, lng:null, label:'', mode:null},
  B:      {lat:null, lng:null, label:'', mode:null},
  payMode:'livraison',
  fare:   0,
};
...
function openDeliverySheet(){
  ...
  // Reset
  _livStep=1;
  _livState.A={lat:null,lng:null,label:'',mode:null};
  _livState.B={lat:null,lng:null,label:'',mode:null};
  _livState.payMode='livraison';
  _livState.fare=0;

  // Reset UI points
  ['A','B'].forEach(p=>{
    _setGpsCard('omni-gps-'+p,'Localisation GPS','Non détectée','Appuyez pour détecter',false);
    const sel=document.getElementById('omni-zone-'+p);
    const desc=document.getElementById('omni-desc-'+p);
    if(sel) sel.value='';
    if(desc) desc.value='';
  });

  // Paiement par défaut
  omniSelectPay('livraison');

  // Afficher étape 1 uniquement
  _livShowStep(1);
  document.getElementById('sheet-livraison').classList.add('open');
  document.getElementById('sheet-livraison').scrollTop=0;
}
window.openDeliverySheet=openDeliverySheet;

function closeLivraisonSheet(){
  document.getElementById('sheet-livraison').classList.remove('open');
}
window.closeLivraisonSheet=closeLivraisonSheet;

function _livShowStep(n){
  _livStep=n;
  [1,2,3].forEach(i=>{
    const el=document.getElementById('liv-step-'+i);
    if(el) el.style.display=i===n?'block':'none';
  });
  // Step bar
  [1,2,3].forEach(i=>{
    const st=document.getElementById('omni-st-'+i);
    if(!st) return;
    st.className='omni-step'+(i<n?' done':i===n?' active':'');
    if(i<n){ const num=st.querySelector('.omni-step-num'); if(num) num.textContent='✓'; }
    else   { const num=st.querySelector('.omni-step-num'); if(num) num.textContent=i; }
  });
  [1,2].forEach(i=>{
    const ln=document.getElementById('omni-stl-'+i);
    if(ln) ln.className='omni-step-line'+(i<n?' done':'');
  });
  // Header
  const titles=['','Point de départ','Point d\'arrivée','Mode de paiement'];
  const subs  =['','Étape 1 sur 3','Étape 2 sur 3','Étape 3 sur 3'];
  const titleEl=document.getElementById('liv-hdr-title');
  const subEl=document.getElementById('liv-hdr-sub');
  ...
  // Scroll top of sheet
  const sheet=document.getElementById('sheet-livraison');
  if(sheet) sheet.scrollTop=0;
}

function livGoBack(){
  if(_livStep>1) _livShowStep(_livStep-1);
}
window.livGoBack=livGoBack;

function livNextStep(fromStep){
  if(fromStep===1){
    // Valider point A
    if(_livState.A.lat===null){
      showToast('⚠️ Définissez le point de départ (GPS ou quartier)','#F5820A'); return;
    }
    // Afficher récap A sur étape 2
    const recapA=document.getElementById('liv-recap-A');
    if(recapA) recapA.innerHTML=`📍 Départ : <strong>${_livState.A.label}</strong>${document.getElementById('omni-desc-A')?.value.trim()?` — <em>${document.getElementById('omni-desc-A').value.trim()}</em>`:''}`;
    _livShowStep(2);
  } else if(fromStep===2){
    // Valider point B
    if(_livState.B.lat===null){
      showToast('⚠️ Définissez le point d\'arrivée (GPS ou quartier)','#F5820A'); return;
    }
    // Vérifier que A ≠ B (distance min 200m)
    const distAB=_haversine(_livState.A.lat,_livState.A.lng,_livState.B.lat,_livState.B.lng);
    if(distAB<0.2){
      showToast('⚠️ Le départ et l\'arrivée semblent être au même endroit','#F5820A'); return;
    }
    // Calculer tarif
    const {fare,detail}=_fmtFare(distAB);
    _livState.fare=fare;
    // Remplir étape 3
    const recapTrajet=document.getElementById('liv-recap-trajet');
    if(recapTrajet) recapTrajet.innerHTML=
      `<div style="font-weight:700;color:#1A1A2E;margin-bottom:6px">📋 Récap du trajet</div>`+
      `<div>🟢 <strong>Départ :</strong> ${_livState.A.label}</div>`+
      (document.getElementById('omni-desc-A')?.value.trim()?`<div style="font-size:11px;color:#9999BB;margin-left:16px">${document.getElementById('omni-desc-A').value.trim()}</div>`:'')+
      `<div style="margin-top:4px">🔴 <strong>Arrivée :</strong> ${_livState.B.label}</div>`+
      (document.getElementById('omni-desc-B')?.value.trim()?`<div style="font-size:11px;color:#9999BB;margin-left:16px">${document.getElementById('omni-desc-B').value.trim()}</div>`:'');
    const kmEl=document.getElementById('omni-fare-liv-km');
    const priceEl=document.getElementById('omni-fare-liv-price');
    const detailEl=document.getElementById('omni-fare-liv-detail');
    const fareLineEl=document.getElementById('omni-liv-fare-line');
    const totalEl=document.getElementById('omni-liv-total');
    if(kmEl) kmEl.textContent=distAB.toFixed(1)+' km';
    if(priceEl) priceEl.textContent=_fmtFCFA(fare);
    if(detailEl) detailEl.textContent=detail;
    if(fareLineEl) fareLineEl.textContent=_fmtFCFA(fare);
    if(totalEl) totalEl.textContent=_fmtFCFA(fare);
    _livShowStep(3);
  }
}
window.livNextStep=livNextStep;

// GPS détection pour A ou B
function omniDetectGPS(target){
  const isRst=target==='rst';
  const isCat=target==='cat'; // livraison catalogue
  const prefix = isRst?'omni-gps-rst':isCat?'omni-gps-cat':'omni-gps-'+target;
  const btn=document.getElementById(prefix+'-btn');
  if(btn){btn.disabled=true;btn.innerHTML='<span class="omni-spin"></span>';}

  const onSuccess=(lat,lng)=>{
    if(isRst){
      _rstClientCoords={lat,lng};
      _calcRstFareFromCoords(lat,lng);
      _setGpsCard('omni-gps-rst','Position détectée ✅',`${lat.toFixed(5)}° N, ${lng.toFixed(5)}° E`,'Votre position de livraison',true);
    } else if(isCat){
      _catLocState.lat=lat; _catLocState.lng=lng; _catLocState.label='Votre position GPS'; _catLocState.mode='gps';
      document.getElementById('omni-zone-cat').value='';
      _setGpsCard('omni-gps-cat','Position détectée ✅',`${lat.toFixed(5)}° N, ${lng.toFixed(5)}° E`,'Position actuelle',true);
    } else {
      _livState[target].lat=lat; _livState[target].lng=lng;
      _livState[target].label='Votre position GPS'; _livState[target].mode='gps';
      // Reset le select correspondant
      const sel=document.getElementById('omni-zone-'+target);
      if(sel) sel.value='';
      _setGpsCard('omni-gps-'+target,'Position détectée ✅',`${lat.toFixed(5)}° N, ${lng.toFixed(5)}° E`,'Position actuelle',true);
    }
  };

  _detectGPS(
    (lat,lng)=>onSuccess(lat,lng),
    ()=>{ const p=_simulatePos(); onSuccess(p.lat,p.lng); }
  );
}
window.omniDetectGPS=omniDetectGPS;

// Sélection zone via select
function omniSelectZone(point){
  const isCat=point==='cat';
  const selId=isCat?'omni-zone-cat':'omni-zone-'+point;
  const sel=document.getElementById(selId);
  if(!sel) return;
  const val=sel.value;
  const state=isCat?_catLocState:_livState[point];
  if(!val){
    state.lat=null; state.lng=null; state.label=''; state.mode=null;
    return;
  }
  const zone=OMNI_ZONES[val];
  if(!zone) return;
  state.lat=zone.lat; state.lng=zone.lng; state.label=val; state.mode='zone';
  // Effacer visuellement le GPS card
  if(!isCat){
    const card=document.getElementById('omni-gps-'+point+'-card');
    if(card) card.classList.remove('active');
  } else {
    const card=document.getElementById('omni-gps-cat-card');
    if(card) card.classList.remove('active');
  }
}
window.omniSelectZone=omniSelectZone;

// Mode de paiement (livraison & course)
function omniSelectPay(mode){
  _livState.payMode=mode;
  ['tmoney','flooz','livraison'].forEach(m=>{
    const el=document.getElementById('omni-pay-'+m);
    if(el) el.classList.toggle('selected',m===mode);
  });
}
window.omniSelectPay=omniSelectPay;

// Soumettre commande livraison
async function submitDeliveryOrder(){
  if(!currentUser){openAuthModal('login');return;}
  const btn=document.getElementById('omni-liv-submit-btn');
  if(btn){btn.innerHTML='<span class="spinner"></span> Enregistrement...';btn.disabled=true;}
  const descA=document.getElementById('omni-desc-A')?.value.trim()||'';
  const descB=document.getElementById('omni-desc-B')?.value.trim()||'';
  const km=_haversine(_livState.A.lat,_livState.A.lng,_livState.B.lat,_livState.B.lng);
  ...
  try{
    const docRef=await addDoc(collection(db,'commandes'),{
      ...
      uid:currentUser.uid, clientNom:currentUser.nom||'', clientPrenom:currentUser.prenom||'',
      clientGenre:currentUser.genre||'', phone:currentUser.phone||'', clientVille:currentUser.ville||'',
      pointA:_livState.A.label, pointALat:_livState.A.lat, pointALng:_livState.A.lng,
      pointAMode:_livState.A.mode, pointADesc:descA,
      pointB:_livState.B.label, pointBLat:_livState.B.lat, pointBLng:_livState.B.lng,
      pointBMode:_livState.B.mode, pointBDesc:descB,
      distanceKm:parseFloat(km.toFixed(2)), fraisLivraison:fare, total:fare,
      modePaiement:_livState.payMode, paygateStatus:'non_configure',
      createdAt:serverTimestamp()
    });
    closeLivraisonSheet();
    const succMsg=document.getElementById('succ-msg');
    if(succMsg) succMsg.innerHTML=
      `Livraison <strong style="color:var(--blue)">${_livState.A.label} → ${_livState.B.label}</strong> confirmée !<br/>
       Référence : <strong>#${docRef.id.slice(0,8).toUpperCase()}</strong><br/>
       Distance : ${km.toFixed(1)} km — Tarif : ${_fmtFCFA(fare)}<br/>
       <small style="color:var(--light)">Contact : ${currentUser.phone||''}</small>`;
    if(window.showSuccessView) window.showSuccessView();
  }catch(err){
    console.error(err);
    showToast('❌ Erreur : '+err.message,'#C62828');
    if(btn){btn.innerHTML='🛵 Confirmer la livraison';btn.disabled=false;}
  }
}
window.submitDeliveryOrder=submitDeliveryOrder;


// ════════════════════════════════════════
// LIVRAISON CATALOGUE (Food / Marketplace / Kits…)
// Étapes : 1-Localisation → 2-Paiement
// ════════════════════════════════════════

let _catLocStep=1;
const _catLocState={lat:null,lng:null,label:'',mode:null};

function openCatLocSheet(serviceId){
  if(!currentUser){openAuthModal('login');showToast('⚠️ Connectez-vous pour passer une commande','#F5820A');return;}
  _catLocStep=1;
  _catLocState.lat=null; _catLocState.lng=null; _catLocState.label=''; _catLocState.mode=null;
  window._catLocServiceId=serviceId||currentService;

  // Reset UI
  _setGpsCard('omni-gps-cat','Localisation GPS','Non détectée','Appuyez pour détecter',false);
  const sel=document.getElementById('omni-zone-cat');
  const desc=document.getElementById('omni-cat-desc');
  if(sel) sel.value='';
  if(desc) desc.value='';

  _catLocSelectPay('livraison');
  _catShowStep(1);
  const sheet=document.getElementById('sheet-cat-loc');
  if(sheet){ sheet.classList.add('open'); sheet.scrollTop=0; }
}
window.openCatLocSheet=openCatLocSheet;

function closeCatLocSheet(){
  const sheet=document.getElementById('sheet-cat-loc');
  if(sheet) sheet.classList.remove('open');
}
window.closeCatLocSheet=closeCatLocSheet;

function _catShowStep(n){
  _catLocStep=n;
  [1,2].forEach(i=>{
    const el=document.getElementById('cat-loc-step-'+i);
    if(el) el.style.display=i===n?'block':'none';
  });
  // Step bar
  [1,2].forEach(i=>{
    const st=document.getElementById('cat-st-'+i);
    if(!st) return;
    st.className='omni-step'+(i<n?' done':i===n?' active':'');
    const num=st.querySelector('.omni-step-num');
    if(num) num.textContent=i<n?'✓':i;
  });
  const ln=document.getElementById('cat-stl-1');
  if(ln) ln.className='omni-step-line'+(n>1?' done':'');
  // Back btn
  const backBtn=document.getElementById('cat-back-btn');
  if(backBtn) backBtn.style.display=n>1?'inline-flex':'none';
  // Header
  const titles=['','Votre position de livraison','Mode de paiement'];
  const subs  =['','Étape 1 sur 2','Étape 2 sur 2'];
  const t=document.getElementById('cat-hdr-title');
  const s=document.getElementById('cat-hdr-sub');
  if(t) t.textContent=titles[n];
  if(s) s.textContent=subs[n];
  const sheet=document.getElementById('sheet-cat-loc');
  if(sheet) sheet.scrollTop=0;
}

function catLocGoBack(){
  if(_catLocStep>1) _catShowStep(_catLocStep-1);
}
window.catLocGoBack=catLocGoBack;

function catLocNext(){
  if(_catLocState.lat===null){
    showToast('⚠️ Indiquez votre position (GPS ou quartier)','#F5820A'); return;
  }
  // Remplir le récap paiement
  const items=Object.values(cart);
  const sub=cartTotal();
  let recapHtml=items.map(a=>`
    <div class="omni-total-row"><span class="omni-total-label">${a.name} ×${a.qty}</span><span class="omni-total-val">${_fmtFCFA(a.price*a.qty)}</span></div>`).join('');
  const recapEl=document.getElementById('cat-pay-recap');
  if(recapEl) recapEl.innerHTML=recapHtml;
  const subEl=document.getElementById('cat-pay-subtotal');
  const locEl=document.getElementById('cat-pay-location');
  const totalEl=document.getElementById('cat-pay-total');
  if(subEl) subEl.textContent=_fmtFCFA(sub);
  if(locEl) locEl.textContent=`📍 ${_catLocState.label}`+(document.getElementById('omni-cat-desc')?.value.trim()?` — ${document.getElementById('omni-cat-desc').value.trim()}`:'');
  if(totalEl) totalEl.textContent=_fmtFCFA(sub);
  _catShowStep(2);
}
window.catLocNext=catLocNext;

function _catLocSelectPay(mode){
  window._catPayMode=mode||'livraison';
  ['tmoney','flooz','livraison'].forEach(m=>{
    const el=document.getElementById('cat-pay-'+m);
    if(el) el.classList.toggle('selected',m===mode);
  });
}
window._catLocSelectPay=_catLocSelectPay;

async function submitCatOrder(){
  if(!currentUser){openAuthModal('login');return;}
  const btn=document.getElementById('cat-submit-btn');
  if(btn){btn.innerHTML='<span class="spinner"></span> Enregistrement...';btn.disabled=true;}
  const desc=document.getElementById('omni-cat-desc')?.value.trim()||'';
  const svc=SVCS[currentService];
  const items=Object.values(cart).map(a=>({id:a.id,name:a.name,price:a.price,qty:a.qty}));
  const total=cartTotal();
  const kitInfo=(currentService==='kits'&&currentKit)?{kitId:currentKit.id,kitNom:currentKit.nom}:{};
  try{
    const docRef=await addDoc(collection(db,'commandes'),{
      service:currentService, serviceName:svc?svc.name:'Commande', statut:'En attente',
      uid:currentUser.uid, clientNom:currentUser.nom||'', clientPrenom:currentUser.prenom||'',
      clientGenre:currentUser.genre||'', phone:currentUser.phone||'', clientVille:currentUser.ville||'',
      positionType:_catLocState.mode==='gps'?'GPS':'zone',
      lat:_catLocState.lat, lng:_catLocState.lng,
      adresse:_catLocState.label, descriptionLivraison:desc,
      articles:items, total,
      modePaiement:window._catPayMode||'livraison',
      paygateStatus:'non_configure',
      paygateTransactionId:'', paygateReference:'',
      ...kitInfo,
      createdAt:serverTimestamp()
    });
    closeCatLocSheet();
    ...
    const succMsg=document.getElementById('succ-msg');
    if(succMsg) succMsg.innerHTML=
      `Commande <strong style="color:var(--blue)">${svc?svc.name:'commande'}</strong> confirmée !<br/>
       Référence : <strong>#${docRef.id.slice(0,8).toUpperCase()}</strong><br/>
       Livraison à : ${_catLocState.label}<br/>
       <small style="color:var(--light)">Contact : ${currentUser.phone||''}</small>`;
    if(window.showSuccessView) window.showSuccessView();
  }catch(err){
    console.error(err);
...
window.submitCatOrder=submitCatOrder;


// ════════════════════════════════════════
// RESTAURANT — LOCALISATION AUTOMATIQUE
// ════════════════════════════════════════

let _rstClientCoords=null;
let _rstDeliveryFare=0;

function openRstLocSheet(restaurantId,restaurantNom,restaurantEmoji,rstLat,rstLng,rstLocalite){
  _rstClientCoords=null; _rstDeliveryFare=0;
  window._currentRstPos={lat:rstLat,lng:rstLng,nom:restaurantNom,localite:rstLocalite||'Lomé'};
  // Remplir UI resto
  const els={
    'rst-loc-nom':restaurantNom,
    'rst-loc-info-nom':restaurantNom,
    'rst-loc-info-loc':'📍 '+(rstLocalite||'Lomé, Togo'),
    'rst-loc-info-coords':`${rstLat.toFixed(4)}° N, ${rstLng.toFixed(4)}° E — position fixe`,
  };
  Object.entries(els).forEach(([id,txt])=>{ const el=document.getElementById(id); if(el) el.textContent=txt; });
  const ico=document.getElementById('rst-loc-ico'); if(ico) ico.textContent=restaurantEmoji;
  // Reset GPS
  _setGpsCard('omni-gps-rst','Localisation GPS','Détection en cours...','Calcul automatique des frais',false);
  const descEl=document.getElementById('omni-rst-desc'); if(descEl) descEl.value='';
  ['omni-fare-rst','omni-total-rst','omni-rst-submit-btn'].forEach(id=>{
    const el=document.getElementById(id); if(el) el.classList.add('omni-fare-hidden');
  });
  document.getElementById('sheet-rst-localisation').classList.add('open');
  // Lancer GPS auto
  omniDetectGPS('rst');
}
window.openRstLocSheet=openRstLocSheet;

function closeRstLocSheet(){
  document.getElementById('sheet-rst-localisation').classList.remove('open');
}
window.closeRstLocSheet=closeRstLocSheet;

function _calcRstFareFromCoords(clientLat,clientLng){
  const rst=window._currentRstPos; if(!rst) return;
  const km=_haversine(clientLat,clientLng,rst.lat,rst.lng);
  const {fare,detail}=_fmtFare(km);
  _rstDeliveryFare=fare;
  const sub=cartTotal();
  const setTxt=(id,v)=>{ const el=document.getElementById(id); if(el) el.textContent=v; };
  setTxt('omni-fare-rst-km',km.toFixed(1)+' km');
  setTxt('omni-fare-rst-price',_fmtFCFA(fare));
  setTxt('omni-fare-rst-detail',detail);
  setTxt('omni-rst-subtotal',_fmtFCFA(sub));
  setTxt('omni-rst-delivery-fee',_fmtFCFA(fare));
  setTxt('omni-rst-total',_fmtFCFA(sub+fare));
  ['omni-fare-rst','omni-total-rst','omni-rst-submit-btn'].forEach(id=>{
    const el=document.getElementById(id); if(el) el.classList.remove('omni-fare-hidden');
  });
}

function confirmRstWithFare(){
  if(!_rstClientCoords){ showToast('⚠️ Activez le GPS pour calculer les frais de livraison','#F5820A'); return; }
  gpsCoords={lat:_rstClientCoords.lat,lng:_rstClientCoords.lng};
  locMode='gps';
  window._rstFareToApply=_rstDeliveryFare;
  window._rstDescToApply=document.getElementById('omni-rst-desc')?.value.trim()||'';
  closeRstLocSheet();
  // Aller directement au paiement (recap déjà calculé)
  const items=Object.values(cart);
  const sub=cartTotal();
  let recapHtml=items.map(a=>`
    <div class="recap-item">
      <div><span class="recap-name">${a.name}</span><span class="recap-qty">x${a.qty}</span></div>
      <div class="recap-price">${_fmtFCFA(a.price*a.qty)}</div>
    </div>`).join('');
  // Ajouter ligne frais livraison
  recapHtml+=`<div class="recap-item"><div><span class="recap-name">🛵 Frais de livraison</span></div><div class="recap-price">${_fmtFCFA(_rstDeliveryFare)}</div></div>`;
  const recapEl=document.getElementById('recap-items');
  const totalEl=document.getElementById('recap-total-val');
  if(recapEl) recapEl.innerHTML=recapHtml;
  if(totalEl) totalEl.textContent=_fmtFCFA(sub+_rstDeliveryFare);
  const backBtn=document.getElementById('delivery-back-btn');
  if(backBtn) backBtn.onclick=()=>showView('catalogue');
  // Masquer blocs GPS dans view-delivery
  const rstBlock=document.getElementById('del-rst-gps-block');
  const stdBlock=document.getElementById('del-std-gps-block');
  if(rstBlock) rstBlock.style.display='none';
  if(stdBlock) stdBlock.style.display='none';
  showView('payment');
}
window.confirmRstWithFare=confirmRstWithFare;


// ════════════════════════════════════════
// CART BAR & CHECKOUT ROUTER
// ════════════════════════════════════════

function openCartCheckout(){
  if(currentService==='restaurant'&&currentRestaurant){
    _openRstLocFromCurrentRestaurant();
  } else if(CATALOGUE_SERVICES.includes(currentService)||currentService==='kits'||currentService==='food'||currentService==='marketplace'||currentService==='omni_drink'||currentService==='clothes'){
    openCatLocSheet(currentService);
  } else {
    showView('delivery');
  }
}
...
async function _openRstLocFromCurrentRestaurant(){
  if(!currentRestaurant){ showView('delivery'); return; }
  let rstLat=6.1375,rstLng=1.2123,rstLocalite='Lomé';
  try{
    const snap=await getDoc(doc(db,'restaurants',currentRestaurant.id));
    if(snap.exists()){ const d=snap.data(); if(d.lat) rstLat=parseFloat(d.lat); if(d.lng) rstLng=parseFloat(d.lng); if(d.localite) rstLocalite=d.localite; }
  }catch(e){}
  openRstLocSheet(currentRestaurant.id,currentRestaurant.nom,currentRestaurant.emoji||'🍽️',rstLat,rstLng,rstLocalite);
}


// ════════════════════════════════════════
// HOOK openService('delivery')
// ════════════════════════════════════════
const _origOpenService=window.openService;
window.openService=function(id){
  if(id==='delivery'){
    if(!currentUser){openAuthModal('login');showToast('⚠️ Connectez-vous pour passer une commande','#F5820A');return;}
    currentService='delivery'; cart={};
    openDeliverySheet(); return;
  }
  _origOpenService(id);
};
...
// ════════════════════════════════════════
// PATCH commanderKit → utilise CatLocSheet
// ════════════════════════════════════════
const _origCommanderKit=window.commanderKit;
window.commanderKit=async function(){
  if(!currentKit) return;
  if(!currentUser){ openAuthModal('login'); return; }
  cart={};
  const articles=currentKit.articles||[];
  if(currentKit.prix_total){
    cart[`kit_${currentKit.id}`]={id:`kit_${currentKit.id}`,name:currentKit.nom,price:currentKit.prix_total,qty:1,emoji:currentKit.emoji||'🎁'};
  } else {
    articles.forEach((a,i)=>{
      const id=`kit_${currentKit.id}_${i}`;
      cart[id]={id,name:a.name,price:a.prix||0,qty:a.qty||1,emoji:a.emoji||'📦'};
  ...
  currentService='kits';
  updateCartBar();
  openCatLocSheet('kits');
};


// ════════════════════════════════════════
// PATCH finalizeOrder — frais resto
// ════════════════════════════════════════
const _origFinalizeOrder=window.finalizeOrder;
window.finalizeOrder=async function(pgData={}){
  if(currentService==='restaurant'&&window._rstFareToApply){
    const svc=SVCS[currentService];
    const addr=document.getElementById('del-address')?.value.trim()||'';
    const notes=document.getElementById('del-notes')?.value.trim()||'';
    const extraDesc=window._rstDescToApply||'';
    const btn=document.getElementById('confirm-btn');
    if(btn){btn.innerHTML='<span class="spinner"></span> Enregistrement...';btn.disabled=true;}
    try{
      const items=Object.values(cart).map(a=>({id:a.id,name:a.name,price:a.price,qty:a.qty}));
      const subtotal=cartTotal(); const total=subtotal+window._rstFareToApply;
      const docRef=await addDoc(collection(db,'commandes'),{
        service:'restaurant', serviceName:`Restaurants — ${currentRestaurant?.nom||''}`,
        statut:'En attente', uid:currentUser.uid,
        clientNom:currentUser.nom||'', clientPrenom:currentUser.prenom||'',
        clientGenre:currentUser.genre||'', phone:currentUser.phone||'', clientVille:currentUser.ville||'',
        restaurantId:currentRestaurant?.id||'', restaurantNom:currentRestaurant?.nom||'',
        adresse:addr, notes, descriptionLivraison:extraDesc,
        positionType:'GPS', lat:gpsCoords?.lat||null, lng:gpsCoords?.lng||null,
        fraisLivraison:window._rstFareToApply, sousTotal:subtotal, total,
        modePaiement:selectedPayment,
        paygateTransactionId:pgData.paygateTransactionId||'',
        paygateReference:pgData.paygateReference||'',
        paygateStatus:pgData.paygateStatus||'non_configure',
        phonePayment:pgData.phoneUsed||'',
        articles:items, createdAt:serverTimestamp()
      });
      document.getElementById('succ-msg').innerHTML=
        `Commande <strong style="color:var(--blue)">${currentRestaurant?.nom||'Restaurant'}</strong> confirmée !<br/>
         Référence : <strong>#${docRef.id.slice(0,8).toUpperCase()}</strong><br/>
         Sous-total : ${_fmtFCFA(subtotal)} + Livraison : ${_fmtFCFA(window._rstFareToApply)} = <strong>${_fmtFCFA(total)}</strong><br/>
         <small style="color:var(--light)">Contact : ${currentUser.phone||''}</small>`;
      window._rstFareToApply=0; window._rstDescToApply='';
      cart={}; updateCartBar(); showSuccessView();
    }catch(err){
      console.error(err);
      showToast('❌ Erreur lors de l\'enregistrement.','#C62828');
      if(btn){btn.innerHTML='✅ Confirmer la commande';btn.disabled=false;}
    }
    return;
  }
  return _origFinalizeOrder(pgData);
};
