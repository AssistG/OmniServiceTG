...
function goToPayment() {
  ...
  // Si c'est un restaurant, gérer différemment (GPS obligatoire)
  if (currentService === 'restaurant' || currentRestaurant) {
    // Vérifier si la section restaurant GPS est visible
    const rstSection = document.getElementById('restaurant-location-section');
    const normalSection = document.getElementById('loc-gps-panel');
    
    if (rstSection && rstSection.style.display === 'none') {
      // Première fois : afficher la section restaurant et masquer la section normale
      rstSection.style.display = 'block';
      if (normalSection) normalSection.closest('.order-card').style.display = 'none';
      showToast('📍 Veuillez détecter votre position GPS pour calculer les frais de livraison');
      return;
    }
    
    // Vérifier que le GPS a été détecté
    if (!rstGPSCoords) {
      showToast('⚠️ Veuillez détecter votre position GPS', '#F5820A');
      return;
    }
    // Pour restaurant, on passe directement au paiement (pas besoin de recap intermédiaire)
  } else {
    // Flux normal pour les autres services
  ...
  // Pour restaurant, ajouter les frais de livraison au total
  let total = cartTotal();
  if (currentService === 'restaurant' || currentRestaurant) {
    const deliveryFee = parseInt(document.getElementById('rst-delivery-fee')?.textContent.replace(/[^\d]/g, '') || 0);
    total += deliveryFee;
  }
  
  document.getElementById('recap-total-val').textContent = fmt(total);
  showView('payment');
}
...
// ════════════════════════════════════════════════════════════════════════════
// SYSTÈME DE LOCALISATION ET CALCULS DE TARIFS
// ════════════════════════════════════════════════════════════════════════════

// ── État pour le système de localisation ──
let rstGPSCoords = null;  // GPS du client pour restaurant
let deliveryState = {
  A: { lat: null, lng: null, label: '', desc: '' },  // Point départ
  B: { lat: null, lng: null, label: '', desc: '' },  // Point arrivée
  payMode: null,
  deliveryFee: 0
};
let courseState = {
  A: { lat: null, lng: null, label: '', desc: '' },
  B: { lat: null, lng: null, label: '', desc: '' },
  payMode: null,
  courseFee: 0
};

// ── Zones prédéfinies Lomé ──
const ZONES_LOME = [
  { name: 'Adidogomé', lat: 6.1622, lng: 1.2127 },
  { name: 'Aflao Gakli', lat: 6.1055, lng: 1.2055 },
  { name: 'Agoè', lat: 6.1888, lng: 1.1977 },
  { name: 'Amoutivé', lat: 6.1344, lng: 1.2344 },
  { name: 'Anfamé', lat: 6.1255, lng: 1.2455 },
  { name: 'Baguida', lat: 6.1166, lng: 1.2766 },
  { name: 'Bè', lat: 6.1388, lng: 1.2188 },
  { name: 'Cacavéli', lat: 6.1544, lng: 1.2244 },
  { name: 'Centre-ville', lat: 6.1375, lng: 1.2123 },
  { name: 'Gbényédzi', lat: 6.1477, lng: 1.2377 },
  { name: 'Kégué', lat: 6.1611, lng: 1.2511 },
  { name: 'Kodjoviakopé', lat: 6.1722, lng: 1.1922 },
  { name: 'Légbassito', lat: 6.1777, lng: 1.2077 },
  { name: 'Novissi', lat: 6.1455, lng: 1.1955 },
  { name: 'Nyékonakpoè', lat: 6.1666, lng: 1.2166 },
  { name: 'Port', lat: 6.1211, lng: 1.2411 },
  { name: 'Tokoin', lat: 6.1555, lng: 1.2155 }
];

// Position du restaurant (exemple - à remplacer par la vraie position de chaque restaurant)
const RESTAURANT_POS = { lat: 6.1375, lng: 1.2123 }; // Centre-ville par défaut

// ════════════════════════════════════════
// UTILITAIRES DE CALCUL DE DISTANCE ET TARIF
// ════════════════════════════════════════
// ── Haversine : calcul distance GPS (km) ──
...
function calcFare(km) {
  if (km <= 0) return 0;
  if (km < 2) return 500;  // Forfait pour moins de 2km
  return 500 + Math.ceil(km - 1) * 100;  // 500 base + 100 par km supplémentaire
}

// ── Format du tarif avec détails ──
function fmtFare(km) {
...
// ════════════════════════════════════════
// RESTAURANT — LOCALISATION GPS CLIENT AUTOMATIQUE
// ════════════════════════════════════════
// Fonction pour détecter la position GPS du client (restaurant)
function detectRestaurantGPS() {
  const btn = document.getElementById('rst-gps-btn');
  if (!btn) return;
  const card = document.getElementById('rst-gps-card');
  const title = document.getElementById('rst-gps-title');
  const coords = document.getElementById('rst-gps-coords');
  const status = document.getElementById('rst-gps-status');
  ...
  navigator.geolocation.getCurrentPosition(
    pos => {
      ...
      rstGPSCoords = { lat, lng };
      
      if (title) title.textContent = 'Position détectée ✅';
      if (coords) coords.textContent = `${lat.toFixed(5)}° N, ${lng.toFixed(5)}° E`;
      if (status) status.textContent = 'Votre position de livraison';
      if (card) card.classList.add('active');
      
      calcRestaurantFare(lat, lng);
      btn.disabled = false;
      btn.innerHTML = '🔄 Actualiser';
    },
    err => {
      // Fallback position simulée Lomé en cas d'erreur
      const lat = 6.1375 + ((Math.random() - 0.5) * 0.04);
      const lng = 1.2123 + ((Math.random() - 0.5) * 0.04);
      rstGPSCoords = { lat, lng };
      
      if (title) title.textContent = 'Position simulée (démo)';
      if (coords) coords.textContent = `${lat.toFixed(5)}° N, ${lng.toFixed(5)}° E`;
      if (status) status.textContent = '⚠️ GPS refusé — position aléatoire Lomé';
      if (card) card.classList.add('active');
...
window.detectRestaurantGPS = detectRestaurantGPS;

// Calculer le tarif de livraison restaurant
function calcRestaurantFare(clientLat, clientLng) {
  const km = haversine(clientLat, clientLng, RESTAURANT_POS.lat, RESTAURANT_POS.lng);
  const { fare, detail } = fmtFare(km);
  
  // Afficher les résultats dans l'UI
  const fareKm = document.getElementById('rst-fare-km');
  const farePrice = document.getElementById('rst-fare-price');
  const fareDetail = document.getElementById('rst-fare-detail');
  const fareBox = document.getElementById('rst-fare-box');
  const totalBox = document.getElementById('rst-total-box');
  const submitBtn = document.getElementById('rst-submit-btn');
  
  if (fareKm) fareKm.textContent = km.toFixed(1) + ' km';
  if (farePrice) farePrice.textContent = fmt(fare);
  if (fareDetail) fareDetail.textContent = detail;
  
  // Afficher la section tarif et total
  if (fareBox) fareBox.classList.remove('fare-hidden');
  if (totalBox) totalBox.classList.remove('fare-hidden');
  if (submitBtn) submitBtn.classList.remove('fare-hidden');
  
  // Calculer le total (subtotal + frais de livraison)
  const subtotal = cartTotal();
  const rstSubtotal = document.getElementById('rst-subtotal');
  const rstDeliveryFee = document.getElementById('rst-delivery-fee');
  const rstTotal = document.getElementById('rst-total');
  
  if (rstSubtotal) rstSubtotal.textContent = fmt(subtotal);
  if (rstDeliveryFee) rstDeliveryFee.textContent = fmt(fare);
  if (rstTotal) rstTotal.textContent = fmt(subtotal + fare);
}
window.calcRestaurantFare = calcRestaurantFare;

// Soumission commande restaurant
function submitRestaurantOrder() {
  if (!rstGPSCoords) {
    showToast('⚠️ Veuillez détecter votre position GPS', '#F5820A');
    return;
  }
  
  // Récupérer la description supplémentaire optionnelle
  const descInput = document.getElementById('rst-desc-supp');
  const descSupp = descInput ? descInput.value.trim() : '';
  
  // Stocker les infos de localisation
  window.rstLocationData = {
    gpsCoords: rstGPSCoords,
    descSupp: descSupp
  };
...
window.submitRestaurantOrder = submitRestaurantOrder;

// ════════════════════════════════════════
// LIVRAISON — 2 POINTS (GPS + ZONES + DESCRIPTION)
// ════════════════════════════════════════
// Détection GPS pour un point (A ou B)
function detectDeliveryGPS(point) {
  const btn = document.getElementById(`del-gps-${point}-btn`);
  if (!btn) return;
  const card = document.getElementById(`del-gps-${point}-card`);
  const title = document.getElementById(`del-gps-${point}-title`);
  const coords = document.getElementById(`del-gps-${point}-coords`);
  const status = document.getElementById(`del-gps-${point}-status`);
  ...
  navigator.geolocation.getCurrentPosition(
    pos => {
      const { latitude: lat, longitude: lng } = pos.coords;
      deliveryState[point].lat = lat;
      deliveryState[point].lng = lng;
      deliveryState[point].label = 'Votre position GPS';
      
      if (title) title.textContent = 'Position détectée ✅';
      if (coords) coords.textContent = `${lat.toFixed(5)}° N, ${lng.toFixed(5)}° E`;
      if (status) status.textContent = 'Position actuelle';
      if (card) card.classList.add('active');
    ...
    err => {
      // Fallback position simulée
      const lat = 6.1375 + ((Math.random() - 0.5) * 0.04);
      const lng = 1.2123 + ((Math.random() - 0.5) * 0.04);
      deliveryState[point].lat = lat;
      deliveryState[point].lng = lng;
      deliveryState[point].label = 'Position simulée';
...
window.detectDeliveryGPS = detectDeliveryGPS;

// Sélection d'une zone via select pour livraison
function selectDeliveryZone(point) {
  const select = document.getElementById(`del-zone-${point}-select`);
  if (!select) return;
  
  const selectedZone = ZONES_LOME.find(z => z.name === select.value);
  
  if (selectedZone) {
    deliveryState[point].lat = selectedZone.lat;
    deliveryState[point].lng = selectedZone.lng;
    deliveryState[point].label = selectedZone.name;
    tryCalcDeliveryFare();
  }
}
window.selectDeliveryZone = selectDeliveryZone;

// Mise à jour de la description supplémentaire
function updateDeliveryDesc(point) {
  const descInput = document.getElementById(`del-desc-${point}`);
  const desc = descInput ? descInput.value.trim() : '';
  deliveryState[point].desc = desc;
}
window.updateDeliveryDesc = updateDeliveryDesc;

// Tentative de calcul du tarif de livraison
function tryCalcDeliveryFare() {
  if (deliveryState.A.lat === null || deliveryState.B.lat === null) return;
  
  const km = haversine(deliveryState.A.lat, deliveryState.A.lng, deliveryState.B.lat, deliveryState.B.lng);
  const { fare, detail } = fmtFare(km);
  
  // Stocker le tarif calculé
  deliveryState.deliveryFee = fare;
  
  const fareKm = document.getElementById('del-fare-km');
  const farePrice = document.getElementById('del-fare-price');
  const fareDetail = document.getElementById('del-fare-detail');
  const fareBox = document.getElementById('del-fare-box');
  const paySection = document.getElementById('del-pay-section');
  const totalBox = document.getElementById('del-total-box');
  const fareLine = document.getElementById('del-fare-line');
  const total = document.getElementById('del-total');
  
  if (fareKm) fareKm.textContent = km.toFixed(1) + ' km';
  if (farePrice) farePrice.textContent = fmt(fare);
  if (fareDetail) fareDetail.textContent = `De ${deliveryState.A.label || 'Point A'} → ${deliveryState.B.label || 'Point B'} · ${detail}`;
  
  // Afficher le tarif et le paiement
  if (fareBox) fareBox.classList.remove('fare-hidden');
  if (paySection) paySection.classList.remove('fare-hidden');
  if (totalBox) totalBox.classList.remove('fare-hidden');
  
  if (fareLine) fareLine.textContent = fmt(fare);
  if (total) total.textContent = fmt(fare);
}
window.tryCalcDeliveryFare = tryCalcDeliveryFare;

// Sélection du mode de paiement (livraison)
function selectDeliveryPay(mode) {
  deliveryState.payMode = mode;
  ['tmoney', 'flooz', 'livraison'].forEach(m => {
    const el = document.getElementById(`del-pay-${m}`);
    if (el) el.classList.toggle('selected', m === mode);
  });
...
window.selectDeliveryPay = selectDeliveryPay;

// Soumission commande livraison
function submitDeliveryOrder() {
  if (!deliveryState.A.lat || !deliveryState.B.lat) {
    showToast('⚠️ Veuillez définir les deux positions (départ et arrivée)', '#F5820A');
    return;
  }
  
  if (!deliveryState.payMode) {
    showToast('⚠️ Choisissez un mode de paiement', '#F5820A');
    return;
  }
  ...
  showToast('✅ Livraison confirmée — ' + labels[deliveryState.payMode]);
...
// ════════════════════════════════════════
// COURSE — 2 POINTS (GPS + ZONES + DESCRIPTION)
// ════════════════════════════════════════
// Détection GPS pour un point (A ou B) - Course
function detectCourseGPS(point) {
  const btn = document.getElementById(`course-gps-${point}-btn`);
  if (!btn) return;
  const card = document.getElementById(`course-gps-${point}-card`);
  const title = document.getElementById(`course-gps-${point}-title`);
  const coords = document.getElementById(`course-gps-${point}-coords`);
  const status = document.getElementById(`course-gps-${point}-status`);
  ...
  navigator.geolocation.getCurrentPosition(
    pos => {
      const { latitude: lat, longitude: lng } = pos.coords;
      courseState[point].lat = lat;
      courseState[point].lng = lng;
      courseState[point].label = 'Votre position GPS';
    ...
    err => {
      ...
      courseState[point].lat = lat;
      courseState[point].lng = lng;
      courseState[point].label = 'Position simulée';
...
window.detectCourseGPS = detectCourseGPS;

// Sélection d'une zone via select pour course
function selectCourseZone(point) {
  const select = document.getElementById(`course-zone-${point}-select`);
  if (!select) return;
  
  const selectedZone = ZONES_LOME.find(z => z.name === select.value);
  
  if (selectedZone) {
    courseState[point].lat = selectedZone.lat;
    courseState[point].lng = selectedZone.lng;
    courseState[point].label = selectedZone.name;
    tryCalcCourseFare();
  }
}
window.selectCourseZone = selectCourseZone;

// Mise à jour de la description supplémentaire - Course
function updateCourseDesc(point) {
  const descInput = document.getElementById(`course-desc-${point}`);
  const desc = descInput ? descInput.value.trim() : '';
  courseState[point].desc = desc;
}
window.updateCourseDesc = updateCourseDesc;

// Tentative de calcul du tarif de course
function tryCalcCourseFare() {
  if (courseState.A.lat === null || courseState.B.lat === null) return;
  
  const km = haversine(courseState.A.lat, courseState.A.lng, courseState.B.lat, courseState.B.lng);
  const { fare, detail } = fmtFare(km);
  
  // Stocker le tarif calculé
  courseState.courseFee = fare;
  
  const fareKm = document.getElementById('course-fare-km');
  const farePrice = document.getElementById('course-fare-price');
  const fareDetail = document.getElementById('course-fare-detail');
  const fareBox = document.getElementById('course-fare-box');
  const paySection = document.getElementById('course-pay-section');
  const totalBox = document.getElementById('course-total-box');
  const fareLine = document.getElementById('course-fare-line');
  const total = document.getElementById('course-total');
  
  if (fareKm) fareKm.textContent = km.toFixed(1) + ' km';
  if (farePrice) farePrice.textContent = fmt(fare);
  if (fareDetail) fareDetail.textContent = `De ${courseState.A.label || 'Point A'} → ${courseState.B.label || 'Point B'} · ${detail}`;
...
window.tryCalcCourseFare = tryCalcCourseFare;

// Sélection du mode de paiement (course)
function selectCoursePay(mode) {
  courseState.payMode = mode;
  ['tmoney', 'flooz', 'livraison'].forEach(m => {
    const el = document.getElementById(`course-pay-${m}`);
    if (el) el.classList.toggle('selected', m === mode);
  });
...
window.selectCoursePay = selectCoursePay;

// Soumission commande course
function submitCourseOrder() {
  if (!courseState.A.lat || !courseState.B.lat) {
    showToast('⚠️ Veuillez définir les deux positions (départ et arrivée)', '#F5820A');
    return;
  }
  
  if (!courseState.payMode) {
    showToast('⚠️ Choisissez un mode de paiement', '#F5820A');
    return;
  }
  ...
  showToast('✅ Course confirmée — ' + labels[courseState.payMode]);
...
window.submitCourseOrder = submitCourseOrder;
