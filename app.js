...
// ════════════════════════════════════════
function goTab(id) {
  // Annuler la redirection automatique post-commande si l'utilisateur navigue manuellement
  if (id !== 'orders' && window._succTimer) {
    clearInterval(window._succTimer);
    window._succTimer = null;
  }
...
function showSuccessView() {
  ...
  if (window._succTimer) clearInterval(window._succTimer);
  window._succTimer = setInterval(() => {
    count--;
    if (cdEl) cdEl.textContent = count;
    if (count <= 0) {
      clearInterval(window._succTimer);
