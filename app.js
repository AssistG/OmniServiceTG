...
function openService(id) {
  ...
  // ── Cas spécial : service Livraison → vue description colis ──
  if (id === 'delivery') {
    showView('delivery-description');
    return;
  }
  
  // ── Cas spécial : service Dépannage → vue localisation dépannage ──
  if (id === 'maintenance') {
    showView('depannage-location');
    return;
  }
...
const DEFAULT_KITS = [
  // Kits Immobilier
  {
    ...
    nom: 'À vendre',
    emoji: '🏘️',
    description: 'Trouvez le bien idéal à acheter',
    categorie: 'Service Immobilier',
    articles: [],
    prix_total: 0,
    actif: true,
    ordre: 1
  },
  {
    ...
    nom: 'À louer',
    emoji: '🏠',
    description: 'Découvrez nos locations disponibles',
    categorie: 'Service Immobilier',
    articles: [],
    prix_total: 0,
    actif: true,
  ...
  },
  {
    ...
    emoji: '📋',
    description: 'Confiez-nous la gestion de votre bien',
    categorie: 'Service Immobilier',
    articles: [],
    prix_total: 0,
    actif: true,
  ...
  },
  {
    id: 'immo-diaspora',
    service: 'immobilier',
    nom: 'Assistance Diaspora',
    emoji: '✈️',
    description: 'Investissez depuis l\'étranger en toute sérénité',
    categorie: 'Service Immobilier',
    articles: [],
    prix_total: 0,
    actif: true,
...
// ════════════════════════════════════════
// LIVRAISON & COURSE — BOTTOM SHEET
// ════════════════════════════════════════

// ── Coordonnées des zones de Lomé ──
const OMNI_ZONES = {
...
function _fmtFare(km){
  ...
  const extra=km>=2?` (500 + ${Math.ceil(km-1)}×100)`:'(forfait)';
  return {fare, detail:`${km.toFixed(1)} km · calcul ${extra}`};
}

// ── Ouvrir / fermer le bottom sheet livraison ──
function openDeliverySheet(){
  ...
  // Reset UI
  ['A','B'].forEach(p=>{
    const titleEl=document.getElementById('omni-gps-'+p+'-title');
    const coordsEl=document.getElementById('omni-gps-'+p+'-coords');
    const statusEl=document.getElementById('omni-gps-'+p+'-status');
    const card=document.getElementById('omni-gps-'+p+'-card');
    const btn=document.getElementById('omni-gps-'+p+'-btn');
  ...
  ['omni-fare-liv','omni-pay-liv','omni-total-liv','omni-liv-submit-btn'].forEach(id=>{
    const el=document.getElementById(id);
    if(el) el.classList.add('omni-fare-hidden');
  });
  // Reset paiement
...
// ── Détecter GPS pour un point (A, B ou rst) ──
function omniDetectGPS(target){
  const isRst = target==='rst';
  const btnId = isRst?'omni-gps-rst-btn':'omni-gps-'+target+'-btn';
  ...
  const onSuccess=pos=>{
    ...
    if(isRst){
      ...
      const titleEl=document.getElementById('omni-gps-rst-title');
      const coordsEl=document.getElementById('omni-gps-rst-coords');
      const statusEl=document.getElementById('omni-gps-rst-status');
      const card=document.getElementById('omni-gps-rst-card');
      if(titleEl) titleEl.textContent='Position détectée ✅';
      if(coordsEl) coordsEl.textContent=`${lat.toFixed(5)}° N, ${lng.toFixed(5)}° E`;
      if(statusEl) statusEl.textContent='Votre position de livraison';
      ...
      document.getElementById('omni-zone-'+target).value='';
      const titleEl=document.getElementById('omni-gps-'+target+'-title');
      const coordsEl=document.getElementById('omni-gps-'+target+'-coords');
      const statusEl=document.getElementById('omni-gps-'+target+'-status');
      const card=document.getElementById('omni-gps-'+target+'-card');
      if(titleEl) titleEl.textContent='Position détectée ✅';
      if(coordsEl) coordsEl.textContent=`${lat.toFixed(5)}° N, ${lng.toFixed(5)}° E`;
      if(statusEl) statusEl.textContent='Position actuelle';
      if(card) card.classList.add('active');
      _tryCalcLivFare();
    }
  ...
  const onError=()=>{
    // Fallback démo — position simulée Lomé
...
// ── Sélectionner une zone via le select ──
function omniSelectZone(point){
  const sel=document.getElementById('omni-zone-'+point);
  const val=sel.value;
  if(!val){
    _livState[point].lat=null;_livState[point].lng=null;_livState[point].label='';_livState[point].mode=null;
    // Masquer fare si pas les 2 points
    ['omni-fare-liv','omni-pay-liv','omni-total-liv','omni-liv-submit-btn'].forEach(id=>{
  ...
  if(!zone) return;
  _livState[point].lat=zone.lat;_livState[point].lng=zone.lng;
  _livState[point].label=val;_livState[point].mode='zone';
  // Effacer GPS visuel pour ce point
...
// ── Calculer le tarif livraison ──
function _tryCalcLivFare(){
  if(_livState.A.lat===null||_livState.B.lat===null) return;
  const km=_haversine(_livState.A.lat,_livState.A.lng,_livState.B.lat,_livState.B.lng);
  const {fare,detail}=_fmtFare(km);
  _livState.fare=fare;
  const fmtNum=n=>n.toLocaleString('fr-FR')+' FCFA';
  document.getElementById('omni-fare-liv-km').textContent=km.toFixed(1)+' km';
  document.getElementById('omni-fare-liv-price').textContent=fmtNum(fare);
  document.getElementById('omni-fare-liv-detail').textContent=
    `De ${_livState.A.label||'Point A'} → ${_livState.B.label||'Point B'} · ${detail}`;
  document.getElementById('omni-liv-fare-line').textContent=fmtNum(fare);
  document.getElementById('omni-liv-total').textContent=fmtNum(fare);
  ['omni-fare-liv','omni-pay-liv','omni-total-liv'].forEach(id=>{
    const el=document.getElementById(id);if(el) el.classList.remove('omni-fare-hidden');
  });
  // Afficher submit seulement si mode paiement choisi
  const submitBtn=document.getElementById('omni-liv-submit-btn');
  if(submitBtn) submitBtn.classList.remove('omni-fare-hidden');
}
...
// ── Soumettre la commande livraison ──
async function submitDeliveryOrder(){
  if(!currentUser){openAuthModal('login');return;}
  if(_livState.A.lat===null||_livState.B.lat===null){
    showToast('⚠️ Définissez les deux points de livraison','#F5820A');return;
  }
  ...
  try {
    const docRef=await addDoc(collection(db,'commandes'),{
      service:        'delivery',
      serviceName:    'Livraison et courses',
      statut:         'En attente',
      uid:            currentUser.uid,
      clientNom:      currentUser.nom    ||'',
      clientPrenom:   currentUser.prenom ||'',
      ...
      phone:          currentUser.phone  ||'',
      clientVille:    currentUser.ville  ||'',
      pointA:         _livState.A.label,
      pointALat:      _livState.A.lat,
      pointALng:      _livState.A.lng,
      pointAMode:     _livState.A.mode,
      pointADesc:     descA,
      pointB:         _livState.B.label,
      pointBLat:      _livState.B.lat,
      pointBLng:      _livState.B.lng,
      pointBMode:     _livState.B.mode,
      pointBDesc:     descB,
      distanceKm:     parseFloat(km.toFixed(2)),
      fraisLivraison: fare,
      total:          fare,
      modePaiement:   _livState.payMode,
      paygateStatus:  'non_configure',
      createdAt:      serverTimestamp()
    });
    ...
    if(succMsg) succMsg.innerHTML=
      `Livraison <strong style="color:var(--blue)">${_livState.A.label||'Point A'} → ${_livState.B.label||'Point B'}</strong> confirmée !<br/>
       Référence : <strong>#${docRef.id.slice(0,8).toUpperCase()}</strong><br/>
       Distance : ${km.toFixed(1)} km — Tarif : ${fmtNum(fare)}<br/>
       <small style="color:var(--light)">Contact : ${currentUser.phone||''}</small>`;
...
// Ouvre le sheet restaurant localisation avant le checkout
function openRstLocSheet(restaurantId, restaurantNom, restaurantEmoji, rstLat, rstLng, rstLocalite){
  _rstClientCoords=null;
  _rstDeliveryFare=0;
  // Stocker coords restaurant dans état global
  window._currentRstPos={lat:rstLat, lng:rstLng, nom:restaurantNom, localite:rstLocalite||'Lomé'};

  // Remplir UI
  const nomEl=document.getElementById('rst-loc-nom');
  const icoEl=document.getElementById('rst-loc-ico');
  const infoNomEl=document.getElementById('rst-loc-info-nom');
  const infoLocEl=document.getElementById('rst-loc-info-loc');
  const infoCoordsEl=document.getElementById('rst-loc-info-coords');
  if(nomEl) nomEl.textContent=restaurantNom;
  if(icoEl) icoEl.textContent=restaurantEmoji;
  if(infoNomEl) infoNomEl.textContent=restaurantNom;
  if(infoLocEl) infoLocEl.textContent='📍 '+(rstLocalite||'Lomé, Togo');
  if(infoCoordsEl) infoCoordsEl.textContent=`${rstLat.toFixed(4)}° N, ${rstLng.toFixed(4)}° E — position fixe`;

  // Reset GPS UI
  const title=document.getElementById('omni-gps-rst-title');
  const coords=document.getElementById('omni-gps-rst-coords');
  const status=document.getElementById('omni-gps-rst-status');
  const card=document.getElementById('omni-gps-rst-card');
  const btn=document.getElementById('omni-gps-rst-btn');
  const desc=document.getElementById('omni-rst-desc');
  if(title) title.textContent='Localisation GPS';
  if(coords) coords.textContent='Détection en cours...';
  if(status) status.textContent='Calcul automatique des frais';
  if(card) card.classList.remove('active');
  if(btn){btn.disabled=false;btn.textContent='🔄 Actualiser';}
  if(desc) desc.value='';
...
// Calcul fare restaurant
function _calcRstFareFromCoords(clientLat, clientLng){
  const rst=window._currentRstPos;
  if(!rst) return;
  ...
  document.getElementById('omni-fare-rst-km').textContent=km.toFixed(1)+' km';
  document.getElementById('omni-fare-rst-price').textContent=fmtNum(fare);
  document.getElementById('omni-fare-rst-detail').textContent=detail;
  document.getElementById('omni-rst-subtotal').textContent=fmtNum(sub);
  document.getElementById('omni-rst-delivery-fee').textContent=fmtNum(fare);
  document.getElementById('omni-rst-total').textContent=fmtNum(sub+fare);
  ['omni-fare-rst','omni-total-rst','omni-rst-submit-btn'].forEach(id=>{
    const el=document.getElementById(id);if(el) el.classList.remove('omni-fare-hidden');
  });
}

// Confirmer restaurant avec fare → passer en checkout standard
function confirmRstWithFare(){
  if(!_rstClientCoords){
    showToast('⚠️ Activez le GPS pour calculer les frais de livraison','#F5820A');return;
  }
  // Pré-remplir gpsCoords pour que finalizeOrder() l'utilise
  gpsCoords={lat:_rstClientCoords.lat, lng:_rstClientCoords.lng};
  locMode='gps';
  // Stocker frais livraison restaurant pour l'inclure dans la commande
  window._rstFareToApply=_rstDeliveryFare;
  window._rstDescToApply=document.getElementById('omni-rst-desc')?.value.trim()||'';
  closeRstLocSheet();
  // Aller au checkout livraison (paiement)
  const backBtn=document.getElementById('delivery-back-btn');
  if(backBtn) backBtn.onclick=()=>{
    if(window._currentRstPos) showView('catalogue');
    else showView('list');
  };
  // Passer en mode restaurant (masquer bloc GPS standard)
  const rstBlock=document.getElementById('del-rst-gps-block');
  const stdBlock=document.getElementById('del-std-gps-block');
  if(rstBlock) rstBlock.style.display='none'; // déjà passé via sheet
  if(stdBlock) stdBlock.style.display='none';
  showView('delivery');
  // Simuler goToPayment directement puisque GPS déjà obtenu
  const items=Object.values(cart);
  let recapHtml=items.map(a=>`
    <div class="recap-item">
      <div><span class="recap-name">${a.name}</span><span class="recap-qty">x${a.qty}</span></div>
      <div class="recap-price">${a.price*a.qty>0?(a.price*a.qty).toLocaleString('fr-FR')+' FCFA':''}</div>
    </div>`).join('');
  ...
  const grandTotal=cartTotal()+_rstDeliveryFare;
  if(totalEl) totalEl.textContent=grandTotal.toLocaleString('fr-FR')+' FCFA';
  showView('payment');
}
window.confirmRstWithFare=confirmRstWithFare;

// Hook dans openService pour 'delivery'
...
// Appelé quand l'utilisateur clique sur la barre panier
function openCartCheckout(){
  if(currentService==='restaurant' && currentRestaurant){
    // Récupérer les coords du restaurant depuis la DB ou utiliser des coords par défaut
    // On va chercher les coordonnées du restaurant actuel
...
async function _openRstLocFromCurrentRestaurant(){
  ...
  // Chercher le restaurant dans Firestore pour avoir ses coords
  let rstLat=6.1375, rstLng=1.2123, rstLocalite='Lomé';
  try {
    const snap=await getDoc(doc(db,'restaurants',currentRestaurant.id));
    if(snap.exists()){
      const d=snap.data();
      if(d.lat) rstLat=parseFloat(d.lat);
      if(d.lng) rstLng=parseFloat(d.lng);
      if(d.localite) rstLocalite=d.localite;
    }
  } catch(e){}
  openRstLocSheet(
    currentRestaurant.id,
    currentRestaurant.nom,
    currentRestaurant.emoji||'🍽️',
    rstLat, rstLng, rstLocalite
  );
}

// Patch finalizeOrder pour inclure les frais restaurant si applicable
...
window.finalizeOrder=async function(pgData={}){
  if(currentService==='restaurant' && window._rstFareToApply){
    // Injecter les frais de livraison restaurant dans la commande
    const svc=SVCS[currentService];
    const addr=document.getElementById('del-address')?.value.trim()||'';
    const notes=document.getElementById('del-notes')?.value.trim()||'';
    ...
    try{
      ...
      const total=subtotal+window._rstFareToApply;
      const docRef=await addDoc(collection(db,'commandes'),{
        service:      'restaurant',
        serviceName:  `Restaurants — ${currentRestaurant?.nom||''}`,
        statut:       'En attente',
        uid:          currentUser.uid,
        clientNom:    currentUser.nom    ||'',
        clientPrenom: currentUser.prenom ||'',
        ...
        restaurantNom:currentRestaurant?.nom||'',
        adresse:      addr,
        notes,
        descriptionLivraison: extraDesc,
        positionType: 'GPS',
        lat:          gpsCoords?.lat||null,
        lng:          gpsCoords?.lng||null,
        fraisLivraison: window._rstFareToApply,
        sousTotal:    subtotal,
        total,
        modePaiement: selectedPayment,
        paygateTransactionId: pgData.paygateTransactionId||'',
      ...
      document.getElementById('succ-msg').innerHTML=
        `Commande <strong style="color:var(--blue)">${currentRestaurant?.nom||'Restaurant'}</strong> confirmée !<br/>
         Référence : <strong>#${docRef.id.slice(0,8).toUpperCase()}</strong><br/>
         Sous-total : ${fmtN(subtotal)} + Livraison : ${fmtN(window._rstFareToApply)} = <strong>${fmtN(total)}</strong><br/>
         ${selectedPayment==='livraison'?'💵 Paiement à la livraison':'📱 Paiement mobile confirmé ✅'}<br/>
         <small style="color:var(--light)">Contact : ${currentUser.phone||''}</small>`;
      window._rstFareToApply=0;
      window._rstDescToApply='';
      cart={};
      updateCartBar();
      showSuccessView();
    } catch(err){
