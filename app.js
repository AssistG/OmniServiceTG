...
// Promesse résolue dès que Firebase Auth est prêt (au plus 8s)
const _authReady = new Promise((resolve) => {
  // Timeout de sécurité : si Firebase Auth ne répond pas, résoudre null
  const timeout = setTimeout(() => {
    console.warn('[FirebaseAuth] Timeout — domaine peut-être non autorisé. Ajoutez-le dans Firebase Console → Authentication → Authorized domains :', window.location.hostname);
    resolve(null);
  }, 8000);
  ...
  const unsub = onAuthStateChanged(auth, async (user) => {
    if (user) {
      ...
      try {
        ...
        clearTimeout(timeout);
        unsub();
        if (e.code === 'auth/unauthorized-domain' || e.code === 'auth/configuration-not-found') {
          console.error('[FirebaseAuth] DOMAINE NON AUTORISÉ. Ajoutez', window.location.hostname, 'dans Firebase Console → Authentication → Authorized domains');
        } else {
          console.error('[FirebaseAuth] signInAnonymously échoué:', e.code, e.message);
        }
        resolve(null);
      }
    }
...
async function doLogin() {
  ...
  try {
    ...
    if (!firebaseUser) {
      err.textContent = '❌ Connexion Firebase impossible. Si vous utilisez un nouveau navigateur, le domaine ' + window.location.hostname + ' doit être ajouté dans Firebase Console → Authentication → Authorized domains.';
      return;
    }
...
async function doSignup() {
  ...
  try {
    ...
    if (!firebaseUser) {
      err.textContent = '❌ Impossible de joindre Firebase. Vérifiez que le domaine ' + window.location.hostname + ' est bien ajouté dans Firebase Console → Authentication → Authorized domains.';
      return;
    }
...
async function paygateInitPayment({ channel, amount, phone, reference, description }) {
  // Appel via proxy Cloudflare Worker (évite CORS + masque le token)
  const channelCode = channel === 'tmoney' ? 1 : 2;

  const response = await fetch('/api/paygate', {
    method:  'POST',
...
async function paygateCheckStatus(transactionId) {
  // Vérification statut via proxy Cloudflare Worker
  const response = await fetch('/api/paygate', {
    method:  'POST',
    headers: { 'Content-Type': 'application/json' },
