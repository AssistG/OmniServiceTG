...
const _authReady = new Promise((resolve) => {
  const timeout = setTimeout(() => {
    console.warn('[FirebaseAuth] Timeout — ajoutez', window.location.hostname, 'dans Firebase Console → Authentication → Authorized domains');
    resolve(null);
  }, 8000);
    ...
    if (user) {
      ...
      try {
        ...
        if (e.code === 'auth/unauthorized-domain' || e.code === 'auth/configuration-not-found') {
          console.error('[FirebaseAuth] DOMAINE NON AUTORISÉ :', window.location.hostname, '→ Firebase Console → Authentication → Authorized domains');
        } else {
...
async function doLogin() {
  ...
  try {
    ...
    // lastLogin en arrière-plan — silencieux si bloqué (autre appareil)
