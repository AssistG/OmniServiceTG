...
async function doLogin() {
  ...
  try {
    ...
    // Mettre à jour currentUser immédiatement (lecture seule — pas d'écriture Firestore)
    // Le setDoc lastLogin est ignoré car sur un nouvel appareil l'UID anonyme est différent
    // → écriture silencieuse en arrière-plan, sans bloquer la connexion
    setDoc(doc(db, 'users', existingUid), { lastLogin: serverTimestamp() }, { merge: true })
      .catch(() => {}); // Silencieux si les rules bloquent (autre appareil)
