...
const CATALOGUE_SERVICES = [
  ...
  // Nouveaux services immobilier (catalogue d'articles)
...
function openService(id) {
  ...
  if (CATALOGUE_SERVICES.includes(id)) {
    ...
    if (backBtn) {
      const isImmoSub = id.startsWith('immo_') || id === 'location_assistance';
...
function renderRestCarouselCard(r, fallbackBg) {
  ...
  const fermeClick = (statut.ouvert === false)
    ...
    : `onclick="openRestaurant('${r.id}','${nom}','${emoji}')"`;
...
async function loadCatalogue(svcId) {
  ...
  if (bannerEl) {
    ...
    bannerEl.style.display = count > 0 ? 'flex' : 'none';
    bannerEl.textContent = `${count} article${count > 1 ? 's' : ''} disponible${count > 1 ? 's' : ''}`;
  }
...
function renderArticleCard(a) {
  ...
  const isComposable = (a.composable === true);
...
const IMMO_CATEGORIES = [
  ...
  { id:'location_assistance',     label:'Location & Assistance',                        emoji:'🤝', color:'#EDE7F6' },
];
...
function showImmoSubMenu() {
  ...
  container.innerHTML = IMMO_CATEGORIES.map(cat => `
    <div class="kit-card" onclick="openImmoService('${cat.id}')" style="cursor:pointer">
      <div class="kit-body">
        <div>
          <div class="kit-badge" style="color:#2E7D32;background:#E8F5E9">Immobilier</div>
          <div class="kit-name">${cat.label}</div>
        </div>
        <div class="kit-footer">
          <div style="font-size:11px;color:#1E6FBE;font-weight:600">Voir les annonces</div>
