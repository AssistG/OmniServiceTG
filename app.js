...
// ════════════════════════════════════════
// LOCALISATION — Quartiers de Lomé + Frais
// ════════════════════════════════════════
// Distance en km depuis le centre OmniService (Adidogomé)
const LOME_QUARTIERS = [
  { nom:'Adidogomé',        km:0  },
  { nom:'Agbalépédogan',    km:1  },
  { nom:'Hédzranawoé',      km:1  },
  { nom:'Gbossimé',         km:2  },
  { nom:'Agoè Logopé',      km:3  },
  { nom:'Agoè Nyivé',       km:3  },
  { nom:'Sagbado',          km:3  },
  { nom:'Baguida',          km:4  },
  { nom:'Bè',               km:4  },
  ...
  { nom:'Hanoukopé',        km:4  },
  { nom:'Kégué',            km:3  },
  { nom:'Kodjoviakopé',     km:5  },
  { nom:'Légba',            km:4  },
  { nom:'Nyékonakpoè',      km:4  },
  { nom:'Sanguéra',         km:6  },
  { nom:'Tokoin',           km:3  },
  { nom:'Vakpossito',       km:5  },
  { nom:'Wuiti',            km:4  },
  { nom:'Zanguéra',         km:5  },
  { nom:'Aflao (frontière)',km:7  },
  { nom:'Avénou',           km:6  },
  { nom:'Cacavéli',         km:5  },
  { nom:'Lomé Centre',      km:5  },
  { nom:'Lomé Port',        km:6  },
  { nom:'Adoboukomé',       km:4  },
  { nom:'Aguiarkomé',       km:5  },
  { nom:'Amoutivé',         km:5  },
...
function calcFraisLivraison(km) {
  if (km <= 0) return 0;
  if (km === 1) return 500;
  return 500 + (km - 1) * 100;
}

// Génère le HTML du composant localisation
function renderLocalisationField() {
  const opts = LOME_QUARTIERS.map(q => {
    const frais = calcFraisLivraison(q.km);
    const fraisStr = frais === 0 ? 'Gratuit' : `+${frais.toLocaleString()} FCFA`;
    return `<option value="${q.nom}" data-km="${q.km}" data-frais="${frais}">${q.nom} — ${fraisStr}</option>`;
  }).join('');

  return `
    <label class="f-label">📍 Quartier / Localisation *</label>
    <select class="f-select" id="ff-quartier" onchange="onQuartierChange(this)">
      <option value="">— Choisir votre quartier —</option>
      ${opts}
    </select>
    <div id="frais-livraison-info" style="display:none;background:#E8F5E9;border-radius:10px;padding:10px 13px;margin-top:-8px;margin-bottom:13px;font-size:12px;color:#2E7D32;font-weight:600;"></div>
    <label class="f-label" style="margin-top:4px">Précision <span style="font-weight:400;font-size:10px;color:#C5C5D8;text-transform:none">(optionnel — ex: près du carrefour, derrière l'église...)</span></label>
    <input type="text" class="f-input" id="ff-precision" placeholder="Précision sur votre position..." style="margin-bottom:13px"/>
  `;
}

// Mise à jour de l'affichage des frais quand quartier change
window.onQuartierChange = function(sel) {
  const opt = sel.options[sel.selectedIndex];
  const frais = parseInt(opt.dataset.frais || '0', 10);
  const km    = parseInt(opt.dataset.km || '0', 10);
  const info  = document.getElementById('frais-livraison-info');
  if (!info) return;
  if (!sel.value) { info.style.display='none'; return; }
  if (frais === 0) {
    info.innerHTML = '🚀 Livraison gratuite pour votre zone !';
  } else {
    info.innerHTML = `🛵 Frais de livraison : <strong>${frais.toLocaleString()} FCFA</strong> (${km} km)`;
  }
  info.style.display = 'block';
};

// Services qui n'ont PAS besoin de localisation (immobilier, consultances, traiteur événementiel)
const NO_LOCATION_SERVICES = ['immobilier','togo_expertise','mathivick','omega_conseil','restaurant','kits'];
...
function showView(v) {
  ...
  // Initialiser le select quartiers à la vue livraison
  if (v === 'delivery') initDelQuartierSelect();
  window.scrollTo({top:0,behavior:"smooth"});
}
window.showView = showView;

// ── Initialise le select de quartiers dans la vue livraison ──
function initDelQuartierSelect() {
  const sel = document.getElementById('del-quartier');
  if (!sel || sel.options.length > 1) return; // déjà initialisé
  LOME_QUARTIERS.forEach(q => {
    const frais   = calcFraisLivraison(q.km);
    const fraisStr = frais === 0 ? 'Gratuit' : `+${frais.toLocaleString()} FCFA`;
    const opt = document.createElement('option');
    opt.value        = q.nom;
    opt.textContent  = `${q.nom} — ${fraisStr}`;
    opt.dataset.km   = q.km;
    opt.dataset.frais = frais;
    sel.appendChild(opt);
  });
}

// ── Affichage frais livraison dans vue livraison ──
window.onDelQuartierChange = function(sel) {
  const opt   = sel.options[sel.selectedIndex];
  const frais = parseInt(opt.dataset.frais || '0', 10);
  const km    = parseInt(opt.dataset.km    || '0', 10);
  const info  = document.getElementById('del-frais-info');
  if (!info) return;
  if (!sel.value) { info.style.display='none'; return; }
  info.innerHTML = frais === 0
    ? '🚀 Livraison gratuite pour votre zone !'
    : `🛵 Frais de livraison : <strong>${frais.toLocaleString()} FCFA</strong> (${km} km)`;
  info.style.display = 'block';
...
function openService(id) {
  ...
  if (CATALOGUE_SERVICES.includes(id)) {
    ...
    let html = '';
    // Injecter localisation en premier pour les services qui en ont besoin
    if (!NO_LOCATION_SERVICES.includes(id)) {
      html += renderLocalisationField();
    }
    svc.fields.forEach(f => {
      // Sauter les anciens champs adresse (remplacés par localisation)
      if (['adresse','localisation'].includes(f.n)) return;
...
function renderRestaurantCard(r) {
  const imgHtml = r.imageUrl
    ? `<img src="${r.imageUrl}" alt="${r.nom}" style="width:100%;height:100%;object-fit:cover" onerror="this.outerHTML='<span style=font-size:52px;position:absolute;inset:0;display:flex;align-items:center;justify-content:center>${r.emoji||'🍽️'}</span>'">`
    : `<span style="font-size:52px;position:absolute;inset:0;display:flex;align-items:center;justify-content:center">${r.emoji||'🍽️'}</span>`;
...
async function loadCatalogue(svcId) {
  ...
  container.innerHTML = `<div style="padding:40px;text-align:center;color:var(--light)"><div class="spinner" style="border-color:rgba(30,111,190,.2);border-top-color:var(--blue)"></div><div style="margin-top:10px;font-size:12px">Chargement...</div></div>`;
  updateCartBar();
  ...
  let articles = [...dbArticles, ...stdArticles].filter(a => a.actif !== false);
  articles.sort((a,b) => (a.ordre ?? 99) - (b.ordre ?? 99) || (a.name||''). localeCompare(b.name||''));

  if (!articles.length) {
    container.innerHTML = `<div style="padding:40px;text-align:center;color:var(--light)">Aucun article disponible.</div>`;
    return;
  }
  ...
  const groups = {};
  articles.forEach(a => {
    const cat = a.categorie || a.category || 'Autres';
    if (!groups[cat]) groups[cat] = [];
    groups[cat].push(a);
  });

  container.innerHTML = Object.entries(groups).map(([cat, arts]) => `
    <div class="cat-section">
      <div class="cat-section-header">
        <div class="cat-section-title">${cat}</div>
        <div class="cat-section-count">${arts.length} article${arts.length>1?'s':''}</div>
      </div>
      <div class="cat-carousel">
        ${arts.map(a => renderArticleCard(a)).join('')}
      </div>
    </div>
  `).join('');
}
...
function renderArticleCard(a) {
  ...
  const imgHtml = a.imageUrl
    ? `<img src="${a.imageUrl}" alt="${a.name}" loading="lazy" style="width:100%;height:100%;object-fit:cover;border-radius:14px 14px 0 0"/>`
    : `<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:52px;background:linear-gradient(135deg,#f0f4ff,#e8f0fe);border-radius:14px 14px 0 0">${a.emoji||'📦'}</div>`;

  return `
  <div class="gochap-card${qty>0?' selected':''}" id="acard-${a.id}" style="${epuise?'opacity:.6;pointer-events:none':''}" >
    <div class="gochap-card-img">
      ${imgHtml}
      ${epuise ? `<div class="gochap-badge-epuise">ÉPUISÉ</div>` : ''}
      ${a.promo ? `<div class="gochap-badge-promo">🔥 PROMO</div>` : ''}
      ${qty > 0 ? `<div class="gochap-qty-bubble" id="bubble-${a.id}">${qty}</div>` : ''}
    </div>
    <div class="gochap-card-body">
      <div class="gochap-card-name">${a.name}</div>
      ${a.desc ? `<div class="gochap-card-desc">${a.desc}</div>` : ''}
      <div class="gochap-card-footer">
        <div class="gochap-card-price">
          ${fmt(a.price)}
          ${a.unit ? `<span class="gochap-card-unit">/ ${a.unit}</span>` : ''}
        </div>
        ${epuise
          ? `<span class="gochap-btn-epuise">Épuisé</span>`
          : `<div class="gochap-qty-ctrl" id="ctrl-${a.id}">
               <button class="gochap-qty-btn minus" onclick="changeQty('${a.id}',-1,event)" style="${qty===0?'display:none':''}" >−</button>
               <span class="gochap-qty-num" id="qty-${a.id}" style="${qty===0?'display:none':''}">${qty}</span>
               <button class="gochap-qty-btn plus" onclick="addToCart('${a.id}',event)">+</button>
             </div>`
        }
      </div>
    </div>
...
function refreshCard(id) {
  ...
  // Support nouveaux cards GoChap + anciens article-card
  if (card.classList.contains('gochap-card')) {
    ...
    if (qty > 0) {
      if (!bubble) {
        ...
        bubble.className = 'gochap-qty-bubble';
        bubble.id = `bubble-${id}`;
        card.querySelector('.gochap-card-img').appendChild(bubble);
      }
    ...
    const minusBtn = card.querySelector('.gochap-qty-btn.minus');
    if (qtyNum)  { qtyNum.textContent = qty; qtyNum.style.display = qty > 0 ? 'inline' : 'none'; }
...
async function finalizeOrder(pgData = {}) {
  ...
  const quartier = document.getElementById('del-quartier')?.value.trim() || '';
  const precision= document.getElementById('del-address')?.value.trim()  || '';
  const notes    = document.getElementById('del-notes')?.value.trim()    || '';

  // Validation quartier
  if (!quartier) {
    showToast('⚠️ Veuillez choisir votre quartier de livraison', '#F5820A');
    const sel = document.getElementById('del-quartier');
    if (sel) sel.style.borderColor = '#F5820A';
    return;
  }

  // Frais de livraison depuis le select
  const quartierSel = document.getElementById('del-quartier');
  const opt = quartierSel?.options[quartierSel.selectedIndex];
  const fraisLivraison = parseInt(opt?.dataset.frais || '0', 10);
  const kmDistance     = parseInt(opt?.dataset.km    || '0', 10);
  const adresse        = precision ? `${quartier} — ${precision}` : quartier;
  ...
  try {
    ...
    const fraisStr = fraisLivraison > 0
      ? `<br/><small style="color:var(--light)">🛵 Frais de livraison : ${fraisLivraison.toLocaleString()} FCFA</small>`
      : '<br/><small style="color:#2E7D32">🚀 Livraison gratuite !</small>';
    ...
    document.getElementById('succ-msg').innerHTML =
      `Commande <strong style="color:var(--blue)">${svc.name}</strong> confirmée !<br/>
       ...
       ${selectedPayment === 'livraison'
         ...
         : `📱 Paiement ${selectedPayment === 'tmoney' ? 'T-Money' : 'Flooz'} confirmé ✅`}${fraisStr}<br/>
       <small style="color:var(--light)">Contact : ${currentUser.phone || ''}</small>`;
    cart = {};
    updateCartBar();
    showSuccessView();
...
async function submitStandardForm() {
  ...
  // ── Récupérer localisation si applicable ──
  let quartier = '', precision = '', fraisLivraison = 0, kmDistance = 0;
  if (!NO_LOCATION_SERVICES.includes(currentService)) {
    const quartierEl = document.getElementById('ff-quartier');
    quartier = quartierEl?.value.trim() || '';
    if (!quartier) {
      if (quartierEl) quartierEl.style.borderColor = '#F5820A';
      showToast('⚠️ Veuillez choisir votre quartier', '#F5820A');
      return;
    }
    const opt = quartierEl.options[quartierEl.selectedIndex];
    fraisLivraison = parseInt(opt.dataset.frais || '0', 10);
    kmDistance     = parseInt(opt.dataset.km || '0', 10);
    precision      = document.getElementById('ff-precision')?.value.trim() || '';
  }
  ...
  if (quartier) {
    ...
    data.adresse        = precision ? `${quartier} — ${precision}` : quartier;
    data.fraisLivraison = fraisLivraison;
    data.kmDistance     = kmDistance;
  }

  let valid = true;
  svc.fields.forEach(f => {
    if (['adresse','localisation'].includes(f.n)) return; // déjà géré
  ...
  try {
    ...
    const fraisStr = fraisLivraison > 0 ? `<br/><small style="color:var(--light)">Frais de livraison : ${fraisLivraison.toLocaleString()} FCFA</small>` : '';
    document.getElementById('succ-msg').innerHTML =
      `Demande <strong style="color:var(--blue)">${svc.name}</strong> envoyée !<br/>
       Référence : <strong>#${docRef.id.slice(0,8).toUpperCase()}</strong><br/>
       Notre équipe vous contactera très bientôt.${fraisStr}`;
    showSuccessView();
  } catch(err) {
    showToast('❌ Erreur d\'envoi. Vérifiez votre connexion.','#C62828');
