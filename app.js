...
// Masquer le splash dès que app.js est chargé (type="module" = différé automatiquement)
...
const PAYGATE_CONFIG = {
  // TODO: Remplacer par vos vraies clés PayGate
  merchantId:  'TODO_VOTRE_MERCHANT_ID',
  apiKey:      'TODO_VOTRE_API_KEY',
  // URL de base de l'API PayGate Togo
  baseUrl:     'https://api.paygate.africa/v1',   // TODO: confirmer l'URL exacte avec PayGate
  // Canaux de paiement (codes PayGate)
  channels: {
    tmoney: 'TMONEY',    // TODO: confirmer le code canal T-Money
    flooz:  'FLOOZ',     // TODO: confirmer le code canal Flooz
  },
  // URL de callback après paiement (votre domaine)
  callbackUrl:  'TODO_https://votre-domaine.com/paygate-callback',
  returnUrl:    'TODO_https://votre-domaine.com',
};

/**
 * Initie un paiement mobile via PayGate (T-Money ou Flooz).
 * Retourne { success, transactionId, message } ou lève une erreur.
 *
 * @param {Object} params
 * @param {string} params.channel     - 'tmoney' | 'flooz'
 * @param {number} params.amount      - Montant en FCFA (entier)
 * @param {string} params.phone       - Numéro client (ex: "90123456")
 * @param {string} params.reference   - Référence commande interne
...
async function paygateInitPayment({ channel, amount, phone, reference, description }) {
  // TODO: Vérifier le format exact du payload avec la doc PayGate
  const payload = {
  ...
  const response = await fetch(`${PAYGATE_CONFIG.baseUrl}/payment/initiate`, {
    ...
    headers: {
      ...
      'Authorization': `Bearer ${PAYGATE_CONFIG.apiKey}`,  // TODO: confirmer le schéma d'auth
    },
  ...
  // TODO: Vérifier la structure exacte de la réponse PayGate
  // Exemple attendu: { success: true, transaction_id: "...", payment_url: "..." }
  return {
    success:       data.success,
    transactionId: data.transaction_id,
    paymentUrl:    data.payment_url,   // optionnel : si PayGate redirige vers une page de paiement
    message:       data.message || '',
  };
}

/**
 * Vérifie le statut d'un paiement PayGate.
 * @param {string} transactionId - ID retourné par paygateInitPayment
 */
async function paygateCheckStatus(transactionId) {
  // TODO: Confirmer le endpoint de vérification avec PayGate
  const response = await fetch(`${PAYGATE_CONFIG.baseUrl}/payment/status/${transactionId}`, {
    headers: { 'Authorization': `Bearer ${PAYGATE_CONFIG.apiKey}` },
  });
  const data = await response.json();
  // TODO: Vérifier la structure : { status: 'SUCCESS'|'PENDING'|'FAILED', ... }
  return data;
}
...
async function confirmOrder() {
  ...
  try {
    ...
    if (selectedPayment === 'tmoney' || selectedPayment === 'flooz') {
      // TODO: Activer quand les clés PayGate sont configurées dans PAYGATE_CONFIG
      const PAYGATE_ACTIVE = PAYGATE_CONFIG.merchantId !== 'TODO_VOTRE_MERCHANT_ID';
      if (PAYGATE_ACTIVE) {
        try {
          const pgRef = `OMN-${Date.now()}`;
          ...
          paygateStatus        = pgResult.success ? 'initiated' : 'failed';
          // Si PayGate retourne une URL de paiement, rediriger
          if (pgResult.paymentUrl) {
            window.location.href = pgResult.paymentUrl;
            return;
          }
        } catch(pgErr) {
          console.warn('[PayGate] Erreur paiement mobile :', pgErr.message);
        ...
        // TODO: Retirer ce bloc quand PayGate est configuré
        console.info('[PayGate] Clés non configurées — paiement enregistré sans traitement.');
      }
    }
