...
async function doLogin() {
  ...
  try {
    // Attendre Firebase Auth (nécessaire pour satisfaire les règles Firestore)
    await ensureAuth();
...
// ════════════════════════════════════════
// PAYGATE — Squelette d'intégration
// Les valeurs marquées TODO seront fournies par PayGate
// ════════════════════════════════════════
// ════════════════════════════════════════════════════════════
// CONFIGURATION PAYGATE
// ⚠️ Remplacer les valeurs ci-dessous avec les clés reçues de PayGate
...
const PAYGATE_CONFIG = {
  ...
  returnUrl:   'https://assistg.github.io/OmniServiceTG/',
};

// Vrai si les clés sont configurées (pas encore des valeurs TODO)
const PAYGATE_ACTIVE = PAYGATE_CONFIG.merchantId !== 'TODO_VOTRE_MERCHANT_ID'
                    && PAYGATE_CONFIG.apiKey      !== 'TODO_VOTRE_API_KEY';
...
async function paygateInitPayment({ channel, amount, phone, reference, description }) {
  const payload = {
    merchant_id:  PAYGATE_CONFIG.merchantId,
    channel:      PAYGATE_CONFIG.channels[channel],
    amount:       Math.round(amount),
    currency:     'XOF',
    phone:        phone.replace(/\D/g, ''),
    reference:    reference,
    description:  description,
    callback_url: PAYGATE_CONFIG.callbackUrl,
    return_url:   PAYGATE_CONFIG.returnUrl,
  };

  const response = await fetch(`${PAYGATE_CONFIG.baseUrl}/payment/initiate`, {
    method:  'POST',
    headers: {
      'Content-Type':  'application/json',
      'Authorization': `Bearer ${PAYGATE_CONFIG.apiKey}`,
      'X-Merchant-ID': PAYGATE_CONFIG.merchantId,
    },
...
async function paygateCheckStatus(transactionId) {
  const response = await fetch(`${PAYGATE_CONFIG.baseUrl}/payment/status/${transactionId}`, {
    headers: {
      'Authorization': `Bearer ${PAYGATE_CONFIG.apiKey}`,
      'X-Merchant-ID': PAYGATE_CONFIG.merchantId,
    },
