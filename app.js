...
const _authReady = new Promise((resolve) => {
  const timeout = setTimeout(() => {
    console.warn('[FirebaseAuth] Timeout — vérifiez Firebase → Authentication → Authorized domains :', window.location.hostname);
    resolve(null);
  }, 8000);
...
async function restoreSession() {
  ...
  try {
    ...
    if (snap && snap.exists()) {
      // sessionUid = UID Firebase Auth actuel (peut différer sur un autre appareil)
      const savedFuid  = localStorage.getItem('omni_fuid');
      const sessionUid = _firebaseUid || savedFuid || savedUid;
      localStorage.setItem('omni_fuid', sessionUid);
...
async function doLogin() {
  ...
  try {
    ...
    // Sur ce nouvel appareil, l'UID Firebase Auth est différent du profileUid
    // On utilise _firebaseUid comme uid de session → les nouvelles commandes
    // créées depuis cet appareil passeront les règles Firestore
    const sessionUid = _firebaseUid || profileUid;

    // lastLogin + firebaseUid en arrière-plan — silencieux si bloqué
    setDoc(doc(db, 'users', profileUid), {
      lastLogin:   serverTimestamp(),
      firebaseUid: sessionUid
    }, { merge: true }).catch(() => {});

    currentUser = { uid: sessionUid, profileUid, ...userData };
    localStorage.setItem('omni_uid',  profileUid);
    localStorage.setItem('omni_fuid', sessionUid);
