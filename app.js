...
// ════════════════════════════════════════
// WINDOW EXPOSURES — fonctions accessibles depuis scripts inline
// ════════════════════════════════════════
// app.js est type="module" : les vars/fonctions ne sont PAS globales automatiquement.
// On les expose ici, en fin de module, après que tout est défini.
window.currentKit        = null; // sera synchronisé via les fonctions ci-dessous
...
// Proxy bidirectionnel pour les vars d'état partagées
// Utilise des fonctions get/set explicites — PAS Object.defineProperties
// (qui peut conflictuyer avec des propriétés natives de window)
function _syncState() {
  // Synchronise les vars du module vers window à chaque appel
  window._moduleState = {
    get currentService()    { return currentService;    },
    set currentService(v)   { currentService    = v;    },
    get currentRestaurant() { return currentRestaurant; },
    set currentRestaurant(v){ currentRestaurant = v;    },
    get currentKit()        { return currentKit;        },
    set currentKit(v)       { currentKit        = v;    },
    get cart()              { return cart;              },
    set cart(v)             { cart              = v;    },
    get gpsCoords()         { return gpsCoords;         },
    set gpsCoords(v)        { gpsCoords         = v;    },
    get selectedPayment()   { return selectedPayment;   },
    set selectedPayment(v)  { selectedPayment   = v;    },
    get locMode()           { return locMode;           },
    set locMode(v)          { locMode           = v;    },
...
const OMNI_ZONES = {
  'Tokoin':{lat:6.1520,lng:1.2220},'Adidogomé':{lat:6.1050,lng:1.1780},
  'Bè':{lat:6.1370,lng:1.2310},'Agbalépédogan':{lat:6.1200,lng:1.2150},
  'Hédzranawoé':{lat:6.1450,lng:1.2050},'Kégué':{lat:6.1280,lng:1.2400},
  'Agoè':{lat:6.1800,lng:1.2100},'Cacavéli':{lat:6.1150,lng:1.1900},
  'Kodjoviakopé':{lat:6.1310,lng:1.2290},'Nyékonakpoè':{lat:6.1390,lng:1.2260},
  'Baguida':{lat:6.1000,lng:1.3100},'Aflao':{lat:6.1000,lng:1.1900},
  'Lomé Centre':{lat:6.1375,lng:1.2123},'Djidjolé':{lat:6.1600,lng:1.2000},
  'Wété':{lat:6.1420,lng:1.2180},
};
function _hav(la1,lo1,la2,lo2){const R=6371,dL=(la2-la1)*Math.PI/180,dO=(lo2-lo1)*Math.PI/180,a=Math.sin(dL/2)**2+Math.cos(la1*Math.PI/180)*Math.cos(la2*Math.PI/180)*Math.sin(dO/2)**2;return R*2*Math.atan2(Math.sqrt(a),Math.sqrt(1-a));}
function _fare(km){if(km<=0)return 0;if(km<2)return 500;return 500+Math.ceil(km-1)*100;}
function _fareFmt(km){const f=_fare(km),e=km>=2?` (500 + ${Math.ceil(km-1)}×100)`:' (forfait)';return{fare:f,detail:`${km.toFixed(1)} km ·${e}`};}
function _fcfa(n){return n.toLocaleString('fr-FR')+' FCFA';}
function _detectPos(ok,fail){if(!navigator.geolocation){fail();return;}navigator.geolocation.getCurrentPosition(p=>ok(p.coords.latitude,p.coords.longitude),fail,{timeout:8000,enableHighAccuracy:true});}
function _simPos(){return{lat:6.1375+((Math.random()-.5)*.04),lng:1.2123+((Math.random()-.5)*.04)};}
function _setGpsUi(pfx,title,coords,status,active){
  ['title','coords','status'].forEach((k,i)=>{const e=document.getElementById(pfx+'-'+k);if(e)e.textContent=[title,coords,status][i];});
  const card=document.getElementById(pfx+'-card');if(card)card.classList.toggle('active',active);
  const btn=document.getElementById(pfx+'-btn');if(btn){btn.disabled=false;btn.textContent=active?'🔄 Actualiser':'📍 Détecter';}
}

function omniDetectGPS(target){
  const isRst=target==='rst',isCat=target==='cat';
  const pfx=isRst?'omni-gps-rst':isCat?'omni-gps-cat':'omni-gps-'+target;
  const btn=document.getElementById(pfx+'-btn');
  if(btn){btn.disabled=true;btn.innerHTML='<span class="omni-spin"></span>';}
  const ok=(lat,lng)=>{
    if(isRst){_rstCoords={lat,lng};_calcRstFare(lat,lng);_setGpsUi('omni-gps-rst','Position détectée ✅',`${lat.toFixed(5)}° N, ${lng.toFixed(5)}° E`,'Votre position',true);}
    else if(isCat){_catState.lat=lat;_catState.lng=lng;_catState.label='Votre position GPS';_catState.mode='gps';const s=document.getElementById('omni-zone-cat');if(s)s.value='';_setGpsUi('omni-gps-cat','Position détectée ✅',`${lat.toFixed(5)}° N, ${lng.toFixed(5)}° E`,'Position actuelle',true);}
    else{_livState[target].lat=lat;_livState[target].lng=lng;_livState[target].label='Votre position GPS';_livState[target].mode='gps';const s=document.getElementById('omni-zone-'+target);if(s)s.value='';_setGpsUi('omni-gps-'+target,'Position détectée ✅',`${lat.toFixed(5)}° N, ${lng.toFixed(5)}° E`,'Position actuelle',true);}
  };
  _detectPos(ok,()=>{const p=_simPos();ok(p.lat,p.lng);});
}
window.omniDetectGPS=omniDetectGPS;

function omniSelectZone(point){
  const isCat=point==='cat';
  const sel=document.getElementById(isCat?'omni-zone-cat':'omni-zone-'+point);if(!sel)return;
  const val=sel.value;const state=isCat?_catState:_livState[point];
  if(!val){state.lat=null;state.lng=null;state.label='';state.mode=null;return;}
  const z=OMNI_ZONES[val];if(!z)return;
  state.lat=z.lat;state.lng=z.lng;state.label=val;state.mode='zone';
  const card=document.getElementById(isCat?'omni-gps-cat-card':'omni-gps-'+point+'-card');if(card)card.classList.remove('active');
}
window.omniSelectZone=omniSelectZone;

// ── LIVRAISON 3 étapes ──
let _livStep=1;
const _livState={A:{lat:null,lng:null,label:'',mode:null},B:{lat:null,lng:null,label:'',mode:null},payMode:'livraison',fare:0};

function openDeliverySheet(){
  if(!currentUser){openAuthModal('login');showToast('⚠️ Connectez-vous','#F5820A');return;}
  _livStep=1;_livState.A={lat:null,lng:null,label:'',mode:null};_livState.B={lat:null,lng:null,label:'',mode:null};_livState.payMode='livraison';
  ['A','B'].forEach(p=>{_setGpsUi('omni-gps-'+p,'Localisation GPS','Non détectée','Appuyez pour détecter',false);const s=document.getElementById('omni-zone-'+p);if(s)s.value='';const d=document.getElementById('omni-desc-'+p);if(d)d.value='';});
  omniSelectPay('livraison');_livShowStep(1);
  const sh=document.getElementById('sheet-livraison');if(sh){sh.classList.add('open');sh.scrollTop=0;}
}
window.openDeliverySheet=openDeliverySheet;
function closeLivraisonSheet(){const sh=document.getElementById('sheet-livraison');if(sh)sh.classList.remove('open');}
window.closeLivraisonSheet=closeLivraisonSheet;
function _livShowStep(n){
  _livStep=n;
  [1,2,3].forEach(i=>{const e=document.getElementById('liv-step-'+i);if(e)e.style.display=i===n?'block':'none';});
  [1,2,3].forEach(i=>{const st=document.getElementById('omni-st-'+i);if(!st)return;st.className='omni-step'+(i<n?' done':i===n?' active':'');const num=st.querySelector('.omni-step-num');if(num)num.textContent=i<n?'✓':String(i);});
  [1,2].forEach(i=>{const ln=document.getElementById('omni-stl-'+i);if(ln)ln.className='omni-step-line'+(i<n?' done':'');});
  const t=document.getElementById('liv-hdr-title');const s=document.getElementById('liv-hdr-sub');
  if(t)t.textContent=['','Point de départ','Point d\'arrivée','Paiement'][n];
  if(s)s.textContent=['','Étape 1 sur 3','Étape 2 sur 3','Étape 3 sur 3'][n];
  const bk=document.getElementById('liv-back-btn');if(bk)bk.style.display=n>1?'inline-flex':'none';
  const sh=document.getElementById('sheet-livraison');if(sh)sh.scrollTop=0;
}
function livGoBack(){if(_livStep>1)_livShowStep(_livStep-1);}
window.livGoBack=livGoBack;
function livNextStep(from){
  if(from===1){
    if(_livState.A.lat===null){showToast('⚠️ Indiquez le point de départ','#F5820A');return;}
    const rA=document.getElementById('liv-recap-A');const dA=document.getElementById('omni-desc-A')?.value.trim()||'';
    if(rA)rA.innerHTML=`📍 Départ : <strong>${_livState.A.label}</strong>${dA?` — <em>${dA}</em>`:''}`;
    _livShowStep(2);
  } else {
    if(_livState.B.lat===null){showToast('⚠️ Indiquez le point d\'arrivée','#F5820A');return;}
    const dist=_hav(_livState.A.lat,_livState.A.lng,_livState.B.lat,_livState.B.lng);
    if(dist<0.2){showToast('⚠️ Départ et arrivée identiques','#F5820A');return;}
    const{fare,detail}=_fareFmt(dist);_livState.fare=fare;
    const dA=document.getElementById('omni-desc-A')?.value.trim()||'';const dB=document.getElementById('omni-desc-B')?.value.trim()||'';
    const recap=document.getElementById('liv-recap-trajet');
    if(recap)recap.innerHTML=`<div style="font-weight:700;color:#1A1A2E;margin-bottom:6px">📋 Récap</div><div>🟢 <strong>Départ :</strong> ${_livState.A.label}${dA?`<br/><span style="font-size:11px;color:#9999BB;margin-left:12px">${dA}</span>`:''}</div><div style="margin-top:4px">🔴 <strong>Arrivée :</strong> ${_livState.B.label}${dB?`<br/><span style="font-size:11px;color:#9999BB;margin-left:12px">${dB}</span>`:''}</div>`;
    ['omni-fare-liv-km','omni-fare-liv-price','omni-fare-liv-detail','omni-liv-fare-line','omni-liv-total'].forEach((id,i)=>{const e=document.getElementById(id);if(e)e.textContent=[dist.toFixed(1)+' km',_fcfa(fare),detail,_fcfa(fare),_fcfa(fare)][i];});
    _livShowStep(3);
  }
}
...
function omniSelectPay(mode){
  ...
  ['tmoney','flooz','livraison'].forEach(m=>{const e=document.getElementById('omni-pay-'+m);if(e)e.classList.toggle('selected',m===mode);});
}
window.omniSelectPay=omniSelectPay;
async function submitDeliveryOrder(){
  if(!currentUser){openAuthModal('login');return;}
  const btn=document.getElementById('omni-liv-submit-btn');if(btn){btn.innerHTML='<span class="spinner"></span>';btn.disabled=true;}
  const dA=document.getElementById('omni-desc-A')?.value.trim()||'';const dB=document.getElementById('omni-desc-B')?.value.trim()||'';
  const km=_hav(_livState.A.lat,_livState.A.lng,_livState.B.lat,_livState.B.lng);const fare=_fare(km);
  try{
    const docRef=await addDoc(collection(db,'commandes'),{service:'delivery',serviceName:'Livraison et courses',statut:'En attente',uid:currentUser.uid,clientNom:currentUser.nom||'',clientPrenom:currentUser.prenom||'',clientGenre:currentUser.genre||'',phone:currentUser.phone||'',clientVille:currentUser.ville||'',pointA:_livState.A.label,pointALat:_livState.A.lat,pointALng:_livState.A.lng,pointAMode:_livState.A.mode,pointADesc:dA,pointB:_livState.B.label,pointBLat:_livState.B.lat,pointBLng:_livState.B.lng,pointBMode:_livState.B.mode,pointBDesc:dB,distanceKm:parseFloat(km.toFixed(2)),fraisLivraison:fare,total:fare,modePaiement:_livState.payMode,paygateStatus:'non_configure',createdAt:serverTimestamp()});
    closeLivraisonSheet();
    const sm=document.getElementById('succ-msg');if(sm)sm.innerHTML=`Livraison <strong style="color:var(--blue)">${_livState.A.label} → ${_livState.B.label}</strong> confirmée !<br/>Référence : <strong>#${docRef.id.slice(0,8).toUpperCase()}</strong><br/>${km.toFixed(1)} km — ${_fcfa(fare)}<br/><small style="color:var(--light)">Contact : ${currentUser.phone||''}</small>`;
    showSuccessView();
  }catch(err){console.error(err);showToast('❌ '+err.message,'#C62828');if(btn){btn.innerHTML='🛵 Confirmer';btn.disabled=false;}}
}
window.submitDeliveryOrder=submitDeliveryOrder;

// ── CATALOGUE 2 étapes ──
let _catStep=1;const _catState={lat:null,lng:null,label:'',mode:null};let _catPayMode='livraison';
function openCatLocSheet(svcId){
  if(!currentUser){openAuthModal('login');showToast('⚠️ Connectez-vous','#F5820A');return;}
  _catStep=1;_catState.lat=null;_catState.lng=null;_catState.label='';_catState.mode=null;_catPayMode='livraison';
  _setGpsUi('omni-gps-cat','Localisation GPS','Non détectée','Appuyez pour détecter',false);
  const s=document.getElementById('omni-zone-cat');if(s)s.value='';const d=document.getElementById('omni-cat-desc');if(d)d.value='';
  _catLocSelectPay('livraison');_catShowStep(1);
  const sh=document.getElementById('sheet-cat-loc');if(sh){sh.classList.add('open');sh.scrollTop=0;}
}
window.openCatLocSheet=openCatLocSheet;
function closeCatLocSheet(){const sh=document.getElementById('sheet-cat-loc');if(sh)sh.classList.remove('open');}
window.closeCatLocSheet=closeCatLocSheet;
function _catShowStep(n){
  _catStep=n;[1,2].forEach(i=>{const e=document.getElementById('cat-loc-step-'+i);if(e)e.style.display=i===n?'block':'none';});
  [1,2].forEach(i=>{const st=document.getElementById('cat-st-'+i);if(!st)return;st.className='omni-step'+(i<n?' done':i===n?' active':'');const num=st.querySelector('.omni-step-num');if(num)num.textContent=i<n?'✓':String(i);});
  const ln=document.getElementById('cat-stl-1');if(ln)ln.className='omni-step-line'+(n>1?' done':'');
  const t=document.getElementById('cat-hdr-title');const s=document.getElementById('cat-hdr-sub');
  if(t)t.textContent=['','Votre position de livraison','Paiement'][n];if(s)s.textContent=['','Étape 1 sur 2','Étape 2 sur 2'][n];
  const bk=document.getElementById('cat-back-btn');if(bk)bk.style.display=n>1?'inline-flex':'none';
  const sh=document.getElementById('sheet-cat-loc');if(sh)sh.scrollTop=0;
}
function catLocGoBack(){if(_catStep>1)_catShowStep(_catStep-1);}
window.catLocGoBack=catLocGoBack;
function catLocNext(){
  if(_catState.lat===null){showToast('⚠️ Indiquez votre position','#F5820A');return;}
  const items=Object.values(cart);const sub=cartTotal();
  const re=document.getElementById('cat-pay-recap');if(re)re.innerHTML=items.map(a=>`<div class="omni-total-row"><span class="omni-total-label">${a.name} ×${a.qty}</span><span class="omni-total-val">${_fcfa(a.price*a.qty)}</span></div>`).join('');
  const d=document.getElementById('omni-cat-desc')?.value.trim()||'';
  const le=document.getElementById('cat-pay-location');if(le)le.textContent=`📍 ${_catState.label}${d?' — '+d:''}`;
  const te=document.getElementById('cat-pay-total');if(te)te.textContent=_fcfa(sub);
  _catShowStep(2);
}
window.catLocNext=catLocNext;
function _catLocSelectPay(mode){_catPayMode=mode;['tmoney','flooz','livraison'].forEach(m=>{const e=document.getElementById('cat-pay-'+m);if(e)e.classList.toggle('selected',m===mode);});}
window._catLocSelectPay=_catLocSelectPay;
async function submitCatOrder(){
  if(!currentUser){openAuthModal('login');return;}
  const btn=document.getElementById('cat-submit-btn');if(btn){btn.innerHTML='<span class="spinner"></span>';btn.disabled=true;}
  const desc=document.getElementById('omni-cat-desc')?.value.trim()||'';
  const svc=SVCS[currentService];const items=Object.values(cart).map(a=>({id:a.id,name:a.name,price:a.price,qty:a.qty}));const total=cartTotal();
  const kitInfo=(currentService==='kits'&&currentKit)?{kitId:currentKit.id,kitNom:currentKit.nom}:{};
  try{
    const docRef=await addDoc(collection(db,'commandes'),{service:currentService,serviceName:svc?svc.name:'Commande',statut:'En attente',uid:currentUser.uid,clientNom:currentUser.nom||'',clientPrenom:currentUser.prenom||'',clientGenre:currentUser.genre||'',phone:currentUser.phone||'',clientVille:currentUser.ville||'',positionType:_catState.mode==='gps'?'GPS':'zone',lat:_catState.lat,lng:_catState.lng,adresse:_catState.label,descriptionLivraison:desc,articles:items,total,modePaiement:_catPayMode,paygateStatus:'non_configure',paygateTransactionId:'',paygateReference:'',...kitInfo,createdAt:serverTimestamp()});
    closeCatLocSheet();cart={};updateCartBar();
    const sm=document.getElementById('succ-msg');if(sm)sm.innerHTML=`Commande <strong style="color:var(--blue)">${svc?svc.name:'commande'}</strong> confirmée !<br/>Référence : <strong>#${docRef.id.slice(0,8).toUpperCase()}</strong><br/>Livraison à : ${_catState.label}<br/><small style="color:var(--light)">Contact : ${currentUser.phone||''}</small>`;
    showSuccessView();
  }catch(err){console.error(err);showToast('❌ '+err.message,'#C62828');if(btn){btn.innerHTML='✅ Confirmer';btn.disabled=false;}}
}
...
// ── RESTAURANT GPS auto ──
let _rstCoords=null;let _rstFare=0;
function openRstLocSheet(rId,rNom,rEmoji,rLat,rLng,rLoc){
  _rstCoords=null;_rstFare=0;window._currentRstPos={lat:rLat,lng:rLng,nom:rNom,localite:rLoc||'Lomé'};
  const s=(id,v)=>{const e=document.getElementById(id);if(e)e.textContent=v;};
  s('rst-loc-nom',rNom);s('rst-loc-info-nom',rNom);s('rst-loc-info-loc','📍 '+(rLoc||'Lomé, Togo'));s('rst-loc-info-coords',`${rLat.toFixed(4)}° N, ${rLng.toFixed(4)}° E`);
  const ico=document.getElementById('rst-loc-ico');if(ico)ico.textContent=rEmoji;
  _setGpsUi('omni-gps-rst','Localisation GPS','Détection en cours...','Calcul automatique',false);
  const desc=document.getElementById('omni-rst-desc');if(desc)desc.value='';
  ['omni-fare-rst','omni-total-rst','omni-rst-submit-btn'].forEach(id=>{const e=document.getElementById(id);if(e)e.classList.add('omni-fare-hidden');});
  const sh=document.getElementById('sheet-rst-localisation');if(sh)sh.classList.add('open');
  omniDetectGPS('rst');
}
window.openRstLocSheet=openRstLocSheet;
function closeRstLocSheet(){const sh=document.getElementById('sheet-rst-localisation');if(sh)sh.classList.remove('open');}
window.closeRstLocSheet=closeRstLocSheet;
function _calcRstFare(lat,lng){
  const rst=window._currentRstPos;if(!rst)return;
  _rstCoords={lat,lng};const km=_hav(lat,lng,rst.lat,rst.lng);const{fare,detail}=_fareFmt(km);_rstFare=fare;
  const sub=cartTotal();const s=(id,v)=>{const e=document.getElementById(id);if(e)e.textContent=v;};
  s('omni-fare-rst-km',km.toFixed(1)+' km');s('omni-fare-rst-price',_fcfa(fare));s('omni-fare-rst-detail',detail);
  s('omni-rst-subtotal',_fcfa(sub));s('omni-rst-delivery-fee',_fcfa(fare));s('omni-rst-total',_fcfa(sub+fare));
  ['omni-fare-rst','omni-total-rst','omni-rst-submit-btn'].forEach(id=>{const e=document.getElementById(id);if(e)e.classList.remove('omni-fare-hidden');});
}
function confirmRstWithFare(){
  if(!_rstCoords){showToast('⚠️ GPS requis','#F5820A');return;}
  gpsCoords={lat:_rstCoords.lat,lng:_rstCoords.lng};locMode='gps';
  window._rstFareToApply=_rstFare;window._rstDescToApply=document.getElementById('omni-rst-desc')?.value.trim()||'';
  closeRstLocSheet();
  const items=Object.values(cart);const sub=cartTotal();
  let rh=items.map(a=>`<div class="recap-item"><div><span class="recap-name">${a.name}</span><span class="recap-qty">x${a.qty}</span></div><div class="recap-price">${_fcfa(a.price*a.qty)}</div></div>`).join('');
  rh+=`<div class="recap-item"><div><span class="recap-name">🛵 Frais de livraison</span></div><div class="recap-price">${_fcfa(_rstFare)}</div></div>`;
  const re=document.getElementById('recap-items');if(re)re.innerHTML=rh;
  const te=document.getElementById('recap-total-val');if(te)te.textContent=_fcfa(sub+_rstFare);
  const bk=document.getElementById('delivery-back-btn');if(bk)bk.onclick=()=>showView('catalogue');
  showView('payment');
}
...
function openCartCheckout(){
  const CAT=['food','clothes','omni_drink','marketplace','kits'];
  if(currentService==='restaurant'&&currentRestaurant){_openRstFromCurrent();}
  else if(CAT.includes(currentService)){openCatLocSheet(currentService);}
  else{showView('delivery');}
}
window.openCartCheckout=openCartCheckout;
async function _openRstFromCurrent(){
  if(!currentRestaurant){showView('delivery');return;}
  let lat=6.1375,lng=1.2123,loc='Lomé';
  try{const snap=await getDoc(doc(db,'restaurants',currentRestaurant.id));if(snap.exists()){const d=snap.data();if(d.lat)lat=parseFloat(d.lat);if(d.lng)lng=parseFloat(d.lng);if(d.localite)loc=d.localite;}}catch(e){}
  openRstLocSheet(currentRestaurant.id,currentRestaurant.nom,currentRestaurant.emoji||'🍽️',lat,lng,loc);
}
window._openRstFromCurrent=_openRstFromCurrent;
