...
// ════════════════════════════════════════
// Règle absolue :
//   • Le retour physique ne quitte JAMAIS l'app directement.
//   • Il ferme d'abord toute modale ouverte, puis remonte la pile de
//     navigation jusqu'à l'accueil.
//   • La sortie n'est possible QUE depuis l'accueil, après 2 appuis
//     consécutifs (confirmation via toast).

(function () {

  // ── Pile de navigation interne ──
  // Chaque entrée : { tab, view }
  // tab  : id de l'onglet actif ('home', 'services', 'orders', 'profile', 'notifs')
  // view : vue interne services active (null si tab ≠ 'services')
  const _navStack = [];

  // ── État courant (snapshot de l'interface) ──
  ...
  // ── Vrai si l'état correspond à l'accueil (aucune navigation en cours) ──
  function _isHome(state) {
    return state.tab === 'home';
  }

  // ── Pousser un état dans la pile + réserver un slot dans l'historique ──
  function _pushNav(state) {
    _navStack.push(state);
    history.pushState({ omni: true, depth: _navStack.length }, '');
  }

  // ── Wrappers sur les fonctions de navigation pour alimenter la pile ──
  const _origGoTab       = window.goTab;
  const _origShowView    = window.showView;
  const _origOpenService = window.openService;

  window.goTab = function (id) {
    const before = _currentState();
    _origGoTab(id);
    const after = _currentState();
    if (before.tab !== after.tab || before.view !== after.view) _pushNav(before);
  };

  window.showView = function (v) {
    const before = _currentState();
    _origShowView(v);
    if (before.view !== v) _pushNav(before);
  };
  ...
  function _restoreState(state) {
    ...
    if (state.tab === 'services') {
      // Si on est déjà dans l'onglet services, changer seulement la vue
      // sans repasser par goTab (qui forcerait showView('list'))
      if (cur.tab === 'services') {
        _origShowView(state.view || 'list');
      } else {
        // Activer l'onglet services visuellement sans déclencher goTab
        document.querySelectorAll('.page').forEach(p => p.classList.remove('on'));
      ...
      // Pour tous les autres onglets (home, orders, profile) goTab est safe
      _origGoTab(state.tab);
    }

    window.scrollTo({ top: 0, behavior: 'smooth' });
  }

  // ── Détecter et fermer toute modale/overlay actuellement ouverte ──
  // Retourne true si une modale a été fermée (le retour est "consommé").
  function _closeOpenModal() {
    ...
    // Modales de paiement mobile (T-Money, Flooz, Wave…) — classe .show
    ...
    // Modales de type gardiennage / assistance (affichées via style.display)
    const securityModal = document.getElementById('security-type-modal');
    if (securityModal && securityModal.style.display === 'flex') {
      securityModal.style.display = 'none';
      return true;
    }
    // Modales génériques encore ouvertes (data-modal ou .modal-overlay visible)
  ...
  window.addEventListener('popstate', function () {
    ...
    // 1. Fermer toute modale ouverte en priorité
    if (_closeOpenModal()) {
      history.pushState({ omni: true, depth: _navStack.length }, '');
      _exitPending = false;
    ...
    // 2. Remonter la pile de navigation (peu importe l'onglet actif)
    if (_navStack.length > 0) {
      const prevState = _navStack.pop();
    ...
    // 3. Pile vide → on est forcément sur l'accueil
    //    On s'assure visuellement d'être bien sur home
    const cur = _currentState();
    if (!_isHome(cur)) {
      // Sécurité : forcer le retour à l'accueil avant toute sortie possible
      _origGoTab('home');
    ...
    // 4. Sur l'accueil : gestion des 2 appuis consécutifs
    if (!_exitPending) {
      _exitPending = true;
      _showExitToast();
      history.pushState({ omni: true, depth: 0 }, '');
    ...
    // 5. Deuxième appui confirmé → quitter l'application
    _exitPending = false;
    if (_exitTimer) { clearTimeout(_exitTimer); _exitTimer = null; }
    history.go(-1);
  });
