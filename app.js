...
const _authReady = new Promise((resolve) => {
  const timeout = setTimeout(() => {
    console.warn('[FirebaseAuth] Timeout — vérifiez que', window.location.hostname, 'est dans Firebase → Authentication → Authorized domains');
    resolve(null);
  }, 8000);
  const unsub = onAuthStateChanged(auth, async (user) => {
    if (user) {
      ...
      try {
        ...
        clearTimeout(timeout); unsub(); resolve(cred.user);
      } catch(e) {
        clearTimeout(timeout); unsub();
        console.error('[FirebaseAuth]', e.code, e.message);
        resolve(null);
      }
    }
  });
...
async function restoreSession() {
  ...
  try {
    ...
    if (snap && snap.exists()) {
      // Récupérer l'UID Firebase Auth actuel (peut différer sur un autre appareil)
      const savedFuid = localStorage.getItem('omni_fuid');
      const sessionUid = savedFuid || _firebaseUid || savedUid;
      currentUser = { uid: sessionUid, profileUid: savedUid, ...snap.data() };
    } else {
      localStorage.removeItem('omni_uid');
      localStorage.removeItem('omni_fuid');
      currentUser = null;
    }
  } catch(e) {
...
async function doLogin() {
  ...
  try {
    ...
    const profileUid = userDoc.id; // UID du profil Firestore (fixe)

    _authHandledManually = true;

    // Sur cet appareil, les commandes seront créées avec l'UID Firebase Auth actuel
    // (_firebaseUid). On stocke les deux UIDs pour compatibilité.
    const sessionUid = _firebaseUid || profileUid;

    // Mettre à jour le profil avec le nouvel UID Firebase si différent
    // + lastLogin — en arrière-plan, silencieux si bloqué
    const updateData = { lastLogin: serverTimestamp() };
    if (sessionUid !== profileUid) {
      updateData.firebaseUid = sessionUid;
    }
    setDoc(doc(db, 'users', profileUid), updateData, { merge: true }).catch(() => {});

    // currentUser.uid = UID Firebase Auth actuel (pour que les commandes passent les règles)
    currentUser = { uid: sessionUid, profileUid, ...userData };
    localStorage.setItem('omni_uid', profileUid);      // profil
    localStorage.setItem('omni_fuid', sessionUid);     // firebase uid actuel
...
async function paygateInitPayment({ channel, amount, phone, reference, description }) {
  ...
  const response = await fetch('https://paygate-proxy.ethancarter170407.workers.dev', {
    method:  'POST',
...
async function submitMobilePayment(channel) {
  ...
  try {
    ...
    // Fermer modal saisie, ouvrir modal USSD
    closePayModal('modal-' + channel);
    showUssdWaiting(channel, pgResult.transactionId, pgResult.identifier, reference);
...
function showUssdWaiting(channel, txId, identifier, reference) {
  ...
  // Lancer le polling PayGate
  pollPaygateStatus(txId, identifier, reference, channel);
}

async function pollPaygateStatus(txId, identifier, reference, channel) {
  const start = Date.now();
  const MAX   = 120000;
  ...
  while (Date.now() - start < MAX) {
    ...
    try {
      const data = await paygateCheckStatus(txId, identifier);
      ...
      if (s === 0) {
        ...
        closeUssdAndShowResult('success', reference, txId);
        return;
      }
      if (s === 4 || s === 6) {
        closeUssdAndShowResult(s === 4 ? 'expired' : 'cancelled', reference, txId);
        return;
      }
      // s === 2 = en cours → continuer
...
async function submitImmoForm() {
  ...
  try {
    const docRef = await addDoc(collection(db,'commandes'), {
      ...
      statut:            'En attente',
      // Satisfait totalIsConsistent() côté Firestore rules
      prestation:        currentImmoOption.article.name,
      // Satisfait mobilePaymentOk() côté Firestore rules
      modePaiement:      'livraison',
      paygateStatus:     'non_configure',
