...
let currentImmoCategory = null; // catégorie immobilier sélectionnée
let cart              = {};
let locMode           = 'gps';
let gpsCoords         = null;
...
function goTab(id) {
  ...
  // CORRECTION BUG : Réinitialiser les variables globales lors du changement d'onglet
  if (id === 'services') {
    if (!currentUser) {
      openAuthModal('login');
    ...
    // Réinitialiser toutes les variables de contexte
    currentService = null;
...
function showSuccessView() {
  ...
  const timer = setInterval(() => {
    ...
    if (count <= 0) {
      clearInterval(timer);
      // CORRECTION BUG : Réinitialiser les variables globales avant de changer de page
      currentService = null;
      currentRestaurant = null;
      currentKit = null;
      currentImmoOption = null;
...
function showTogoExpertiseMenu() {
  ...
  // CORRECTION : Retour à list depuis le niveau 1
...
function openConsultancesSubService(subId) {
  ...
  // Niveau 2 : retour au niveau 1 (menu principal consultances)
...
function openConsultancesForm(subId, sectionId) {
  ...
  // Niveau 3 formulaire : retour au niveau 2 (liste des sections du pôle)
...
async function loadRestaurantsList() {
  ...
  // CORRECTION BUG : Configurer le bouton retour de la vue restaurants pour revenir à 'list'
...
async function openRestaurant(restaurantId, restaurantNom, restaurantEmoji) {
  ...
  // CORRECTION BUG : Le bouton retour doit réinitialiser et revenir à la vue LIST principale
  const backBtn = document.getElementById('catalogue-back-btn');
  if (backBtn) backBtn.onclick = () => {
    currentRestaurant = null; // Réinitialiser
    currentService = null;
    cart = {};
    showView('list'); // Retour direct à la liste des services
  };
...
async function loadKitsList(serviceFilter = null, immoMode = false) {
  ...
  // CORRECTION BUG : Configurer le bouton retour de la vue kits pour revenir à 'list'
  const backBtn = document.getElementById('kits-back-btn');
  if (backBtn) backBtn.onclick = () => {
    currentService = null;
    currentKit = null;
    currentImmoCategory = null;
...
async function openKit(kitId, kitNom, kitEmoji) {
  ...
  // CORRECTION BUG : Configurer le bouton retour pour revenir à list
  const backBtn = document.getElementById('kit-detail-back-btn');
  if (backBtn) backBtn.onclick = () => {
    currentKit = null;
    currentService = null;
    showView('list');
...
async function commanderKit() {
  ...
  // CORRECTION BUG : Bouton retour de delivery vers list au lieu de kit-detail
...
function immoFormGoBack() {
  // Utiliser la fonction de retour définie par chaque contexte
  if (typeof _immoFormBackFn === 'function') {
    _immoFormBackFn();
  } else {
    // Par défaut, revenir à immo-options
    showView('immo-options');
  }
}
...
function openImmoCategory(kitId, kitNom, kitEmoji, kitDesc, kitCat) {
  ...
  // L'immobilier vient de la vue kits, donc retour à kits
