...
async function openRestaurant(restaurantId, restaurantNom, restaurantEmoji, restaurantLat, restaurantLng) {
  ...
  // Normaliser lat/lng en nombres (ils arrivent comme strings depuis le onclick HTML)
  const latNum = parseFloat(restaurantLat);
  const lngNum = parseFloat(restaurantLng);
  currentRestaurant = {
    id: restaurantId, nom: restaurantNom, emoji: restaurantEmoji,
    lat: !isNaN(latNum) && latNum ? latNum : null,
    lng: !isNaN(lngNum) && lngNum ? lngNum : null
  };
  ...
  // Reconstruire view-delivery avec les coords du restaurant et câbler la cart bar
  if (window.buildCatDelivery) window.buildCatDelivery('restaurant');
  window._cartBarFn = () => {
...
// RETOUR PHYSIQUE — cliquer le bouton ← visible à l'écran
(function () {
  history.pushState(null, '');
  window.addEventListener('popstate', function () {
    // Chercher un bouton ← visible dans toute la page
    var btn = document.querySelector(
      '.back-btn:not([style*="display:none"]), ' +
      '#immo-form-back-btn:not([style*="display:none"]), ' +
      '#immo-options-back-btn:not([style*="display:none"]), ' +
      '#catalogue-back-btn:not([style*="display:none"]), ' +
      '#delivery-back-btn:not([style*="display:none"])'
    );

    // Modale ouverte ?
    var modal =
      (document.getElementById('notif-panel')       || {style:{}}).style.display === 'flex'    ||
      (document.getElementById('auth-modal')         || {style:{}}).style.display === 'flex'    ||
      (document.getElementById('cancel-order-modal') || {style:{}}).style.display === 'flex'    ||
      (document.getElementById('security-type-modal')|| {style:{}}).style.display === 'flex'    ||
      (document.getElementById('ussd-overlay')       || {classList:{contains:function(){return false;}}}).classList.contains('show') ||
      (document.getElementById('pg-result-overlay')  || {classList:{contains:function(){return false;}}}).classList.contains('show') ||
      document.querySelectorAll('.pay-modal-overlay.show').length > 0;

    if (modal) {
      // Fermer la modale
      ['notif-panel','auth-modal','cancel-order-modal','security-type-modal'].forEach(function(id){
        var el = document.getElementById(id);
        if (el && el.style.display === 'flex') el.style.display = 'none';
      });
      ['ussd-overlay','pg-result-overlay'].forEach(function(id){
        var el = document.getElementById(id);
        if (el) el.classList.remove('show');
      });
      document.querySelectorAll('.pay-modal-overlay.show').forEach(function(e){ e.classList.remove('show'); });
      var pm = document.getElementById('privacy-modal');
      if (pm) pm.classList.remove('open');
      history.pushState(null, ''); // rester dans l'app
      return;
    }

    if (btn) {
      // Un bouton ← est visible : le cliquer
      btn.click();
      history.pushState(null, ''); // rester dans l'app
      return;
    }

    // Aucun bouton ← visible = on est sur home ou un onglet principal
    var tab = (document.querySelector('.btab.on') || {id:'t-home'}).id.replace('t-','');
    if (tab !== 'home') {
      // Onglet secondaire → aller sur home
      goTab('home');
      history.pushState(null, ''); // rester dans l'app
      return;
    }

    // On est sur home → laisser quitter (pas de pushState)
  });
}());
