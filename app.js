...
const PAYGATE_CONFIG = {
  token:     'b8217d22-5b2f-4a6b-a231-fc7dc086326f', // Votre clé API paygateglobal.com
  baseUrl:   'https://paygateglobal.com/api/v1',
  returnUrl: 'https://omniservice.tg/',
};

// Actif dès que le token est renseigné (pas la valeur TODO)
...
async function paygateInitPayment({ channel, amount, phone, reference, description }) {
  // API paygateglobal.com : POST /pay
  // Params: token, phone, amount, description, callback_url
  // channel: 1 = T-Money, 2 = Flooz
  const channelCode = channel === 'tmoney' ? 1 : 2;
  const phoneClean  = phone.replace(/\D/g, '');

  const payload = {
    token:        PAYGATE_CONFIG.token,
    phone:        phoneClean,
    amount:       Math.round(amount),
    description:  description || reference,
    callback_url: PAYGATE_CONFIG.returnUrl,
    network:      channelCode,
  };

  const response = await fetch(`${PAYGATE_CONFIG.baseUrl}/pay`, {
    method:  'POST',
  ...
  if (!response.ok) {
    ...
    throw new Error(err.message || `PayGate HTTP ${response.status}`);
  }

  const data = await response.json();
  // paygateglobal.com retourne : { status: 0|1, transaction_id, message }
  // status 0 = succès initiation, 1 = erreur
  const success = data.status === 0 || data.status === '0';
  if (!success) throw new Error(data.message || 'Initiation PayGate échouée');

  return {
    success:       true,
    transactionId: data.transaction_id || data.id || '',
    paymentUrl:    null, // paygateglobal.com envoie directement sur le téléphone
 ...
 */
async function paygateCheckStatus(transactionId) {
  // paygateglobal.com : GET /status?token=...&transaction_id=...
  const url = `${PAYGATE_CONFIG.baseUrl}/status?token=${encodeURIComponent(PAYGATE_CONFIG.token)}&transaction_id=${encodeURIComponent(transactionId)}`;
 ...
 */
function paygateWaitForPayment(transactionId, maxWait = 120000) {
  return new Promise((resolve) => {
    const FINAL = ['SUCCESS','COMPLETED','PAID','FAILED','CANCELLED','EXPIRED'];
    const start = Date.now();
    const interval = setInterval(async () => {
      try {
        const data = await paygateCheckStatus(transactionId);
        const status = (data.status || '').toUpperCase();
        if (FINAL.includes(status) || Date.now() - start >= maxWait) {
          clearInterval(interval);
          resolve({
            success: ['SUCCESS','COMPLETED','PAID'].includes(status),
            status,
            message: data.message || '',
          });
        }
    ...
    if (selectedPayment === 'tmoney' || selectedPayment === 'flooz') {
      if (PAYGATE_ACTIVE) {
        try {
          ...
          const pgFinal = await paygateWaitForPayment(paygateTransactionId);
