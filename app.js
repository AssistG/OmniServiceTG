...
// Expose module-scoped vars on window for inline scripts
// (ES modules are not global scope - inline scripts can't access these directly)
let currentKit = null;
Object.defineProperties(window, {
  currentService:    { get:()=>currentService,    set:v=>{currentService=v;},    configurable:true, enumerable:true },
  currentRestaurant: { get:()=>currentRestaurant, set:v=>{currentRestaurant=v;}, configurable:true, enumerable:true },
  currentKit:        { get:()=>currentKit,        set:v=>{currentKit=v;},        configurable:true, enumerable:true },
  cart:              { get:()=>cart,              set:v=>{cart=v;},              configurable:true, enumerable:true },
  gpsCoords:         { get:()=>gpsCoords,         set:v=>{gpsCoords=v;},         configurable:true, enumerable:true },
  selectedPayment:   { get:()=>selectedPayment,   set:v=>{selectedPayment=v;},   configurable:true, enumerable:true },
  locMode:           { get:()=>locMode,           set:v=>{locMode=v;},           configurable:true, enumerable:true },
  currentUser:       { get:()=>currentUser,       set:v=>{currentUser=v;},       configurable:true, enumerable:true },
});
...
// ════════════════════════════════════════
// OMNI LOCALISATION — LIVRAISON, CATALOGUE, RESTAURANT
// ════════════════════════════════════════
...
function _setGpsUi(pfx,title,coords,status,active){
  ...
  s(pfx+'-title',title);s(pfx+'-coords',coords);s(pfx+'-status',status);
...
function omniDetectGPS(target){
  ...
  const ok=(lat,lng)=>{
    if(isRst){_rstCoords={lat,lng};_calcRstFare(lat,lng);_setGpsUi('omni-gps-rst','Position détectée ✅',`${lat.toFixed(5)}° N, ${lng.toFixed(5)}° E`,'Votre position de livraison',true);}
...
function omniSelectZone(point){
  ...
  const sel=document.getElementById(isCat?'omni-zone-cat':'omni-zone-'+point);
  if(!sel)return;
  const val=sel.value;
  const state=isCat?_catState:_livState[point];
  ...
  const card=document.getElementById(isCat?'omni-gps-cat-card':'omni-gps-'+point+'-card');
...
function openDeliverySheet(){
  if(!currentUser){if(window.openAuthModal)openAuthModal('login');showToast('⚠️ Connectez-vous','#F5820A');return;}
  _livStep=1;_livState.A={lat:null,lng:null,label:'',mode:null};_livState.B={lat:null,lng:null,label:'',mode:null};_livState.payMode='livraison';_livState.fare=0;
...
function _livShowStep(n){
  ...
  const titles=['','Point de départ','Point d\'arrivée','Paiement'];
  ...
  if(t)t.textContent=titles[n];if(s)s.textContent=subs[n];
...
function livNextStep(from){
  if(from===1){
    ...
    const rA=document.getElementById('liv-recap-A');
    const dA=document.getElementById('omni-desc-A')?.value.trim()||'';
    ...
    const dA=document.getElementById('omni-desc-A')?.value.trim()||'';
    const dB=document.getElementById('omni-desc-B')?.value.trim()||'';
    const recap=document.getElementById('liv-recap-trajet');
    if(recap)recap.innerHTML=`<div style="font-weight:700;color:#1A1A2E;margin-bottom:6px">📋 Récap du trajet</div><div>🟢 <strong>Départ :</strong> ${_livState.A.label}${dA?`<div style="font-size:11px;color:#9999BB;margin-left:16px">${dA}</div>`:''}</div><div style="margin-top:4px">🔴 <strong>Arrivée :</strong> ${_livState.B.label}${dB?`<div style="font-size:11px;color:#9999BB;margin-left:16px">${dB}</div>`:''}</div>`;
    const s=(id,v)=>{const e=document.getElementById(id);if(e)e.textContent=v;};
    s('omni-fare-liv-km',dist.toFixed(1)+' km');s('omni-fare-liv-price',_fcfa(fare));
    s('omni-fare-liv-detail',detail);s('omni-liv-fare-line',_fcfa(fare));s('omni-liv-total',_fcfa(fare));
    _livShowStep(3);
  }
}
...
async function submitDeliveryOrder(){
  ...
  const dA=document.getElementById('omni-desc-A')?.value.trim()||'';
  const dB=document.getElementById('omni-desc-B')?.value.trim()||'';
  const km=_hav(_livState.A.lat,_livState.A.lng,_livState.B.lat,_livState.B.lng);
  const fare=_fare(km);
  try{
    ...
    closeLivraisonSheet();
    const sm=document.getElementById('succ-msg');
    if(sm)sm.innerHTML=`Livraison <strong style="color:var(--blue)">${_livState.A.label} → ${_livState.B.label}</strong> confirmée !<br/>Référence : <strong>#${docRef.id.slice(0,8).toUpperCase()}</strong><br/>Distance : ${km.toFixed(1)} km — Tarif : ${_fcfa(fare)}<br/><small style="color:var(--light)">Contact : ${currentUser.phone||''}</small>`;
    if(window.showSuccessView)showSuccessView();
  }catch(err){console.error(err);showToast('❌ Erreur : '+err.message,'#C62828');if(btn){btn.innerHTML='🛵 Confirmer';btn.disabled=false;}}
}
window.submitDeliveryOrder=submitDeliveryOrder;

// ── LIVRAISON CATALOGUE (Food/Marketplace/Kits) — 2 étapes ──
let _catStep=1;
const _catState={lat:null,lng:null,label:'',mode:null};
let _catPayMode='livraison';
...
function openCatLocSheet(svcId){
  ...
  window._catLocSvc=svcId||currentService;
  _setGpsUi('omni-gps-cat','Localisation GPS','Non détectée','Appuyez pour détecter',false);
  const s=document.getElementById('omni-zone-cat');if(s)s.value='';
  const d=document.getElementById('omni-cat-desc');if(d)d.value='';
...
function _catShowStep(n){
  _catStep=n;
  [1,2].forEach(i=>{const e=document.getElementById('cat-loc-step-'+i);if(e)e.style.display=i===n?'block':'none';});
  ...
  const titles=['','Votre position de livraison','Mode de paiement'];const subs=['','Étape 1 sur 2','Étape 2 sur 2'];
...
function catLocNext(){
  ...
  let rhtml=items.map(a=>`<div class="omni-total-row"><span class="omni-total-label">${a.name} ×${a.qty}</span><span class="omni-total-val">${_fcfa(a.price*a.qty)}</span></div>`).join('');
  const re=document.getElementById('cat-pay-recap');if(re)re.innerHTML=rhtml;
...
function _catLocSelectPay(mode){
  _catPayMode=mode;
  ['tmoney','flooz','livraison'].forEach(m=>{const e=document.getElementById('cat-pay-'+m);if(e)e.classList.toggle('selected',m===mode);});
}
...
async function submitCatOrder(){
  ...
  try{
    ...
    const sm=document.getElementById('succ-msg');
    if(sm)sm.innerHTML=`Commande <strong style="color:var(--blue)">${svc?svc.name:'commande'}</strong> confirmée !<br/>Référence : <strong>#${docRef.id.slice(0,8).toUpperCase()}</strong><br/>Livraison à : ${_catState.label}<br/><small style="color:var(--light)">Contact : ${currentUser.phone||''}</small>`;
    if(window.showSuccessView)showSuccessView();
  }catch(err){console.error(err);showToast('❌ Erreur : '+err.message,'#C62828');if(btn){btn.innerHTML='✅ Confirmer';btn.disabled=false;}}
}
window.submitCatOrder=submitCatOrder;

// ── RESTAURANT — GPS automatique ──
let _rstCoords=null;let _rstFare=0;

function openRstLocSheet(rId,rNom,rEmoji,rLat,rLng,rLoc){
  _rstCoords=null;_rstFare=0;
  window._currentRstPos={lat:rLat,lng:rLng,nom:rNom,localite:rLoc||'Lomé'};
  const s=(id,v)=>{const e=document.getElementById(id);if(e)e.textContent=v;};
  s('rst-loc-nom',rNom);s('rst-loc-info-nom',rNom);s('rst-loc-info-loc','📍 '+(rLoc||'Lomé, Togo'));s('rst-loc-info-coords',`${rLat.toFixed(4)}° N, ${rLng.toFixed(4)}° E — position fixe`);
...
function _calcRstFare(lat,lng){
  ...
  _rstCoords={lat,lng};
  const km=_hav(lat,lng,rst.lat,rst.lng);const{fare,detail}=_fareFmt(km);_rstFare=fare;
  const sub=cartTotal();
  const s=(id,v)=>{const e=document.getElementById(id);if(e)e.textContent=v;};
...
function confirmRstWithFare(){
  if(!_rstCoords){showToast('⚠️ GPS requis pour calculer les frais','#F5820A');return;}
  gpsCoords={lat:_rstCoords.lat,lng:_rstCoords.lng};locMode='gps';
  window._rstFareToApply=_rstFare;
  ...
  const rb=document.getElementById('del-rst-gps-block');if(rb)rb.style.display='none';
  const sb=document.getElementById('del-std-gps-block');if(sb)sb.style.display='none';
  showView('payment');
}
