...
window._mGPS=function(){
  ...
  detectGPS(ov('mg-btn'),
    (la,lo)=>{
      const km=haversine(la,lo,OMNI_DEPOT.lat,OMNI_DEPOT.lng),{fare,detail}=fmtFare(km);
    ...
    (e,la,lo)=>{
      ...
      window._mGps={lat:la,lng:lo,km,fare};
      const card=ov('mg-card');if(card){card.classList.remove('off');card.classList.add('on');}
      ov('mg-t').textContent='Position simulée (démo)';ov('mg-c').textContent=`${la.toFixed(5)}° N, ${lo.toFixed(5)}° E`;ov('mg-s').textContent='⚠️ GPS refusé — position approx. Lomé';
      showMF(km,fare,detail);
    }
  );
};
...
window._mZone=function(id,sel){
  ...
  const[la,lo,name]=v.split('|');
  const km=haversine(parseFloat(la),parseFloat(lo),OMNI_DEPOT.lat,OMNI_DEPOT.lng),{fare,detail}=fmtFare(km);
