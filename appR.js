...
function openService(id) {
  ...
  currentService = id;
  window.currentService = id; // exposé pour localisation-module.js
  cart = {};
...
function openImmoService(svcId) {
  ...
  window.currentService = svcId; // exposé pour localisation-module.js
  cart = {};
  window.cart = cart;
...
// ════════════════════════════════════════
// GESTION DU BOUTON RETOUR PHYSIQUE (Android / PWA)
// ════════════════════════════════════════
// Règle absolue :
//   • Le retour physique ne quitte JAMAIS l'app directement depuis une
//     page autre que l'accueil.
//   • Il ferme d'abord toute modale/overlay ouvert, puis remonte la pile
//     étape par étape jusqu'à l'accueil.
//   • La sortie n'est possible QUE depuis l'accueil, après 2 appuis
//     consécutifs (confirmation via toast).
//
// Cas particuliers gérés :
//   • Panel notifications (overlay display:flex, hors système goTab)
//   • Onglets profile / orders / about → pile enregistrée correctement
//   • Consultances/Expertises : showTogoExpertiseMenu,
//     openConsultancesSubService, openConsultancesForm réutilisent la
//     même vue DOM (immo-options / immo-form) → on utilise un champ
//     `subState` pour distinguer les niveaux dans la pile.

(function () {

  // ═══════════════════════════════
  // PILE DE NAVIGATION
  // Chaque entrée : { tab, view, subState }
  //   tab      : onglet actif ('home','services','orders','profile','about')
  //   view     : vue interne services active (null si tab ≠ 'services')
  //   subState : identifiant libre pour les sous-niveaux à même vue
  //              (ex: 'consultances-menu', 'consultances-sub-mathivick',
  //                   'consultances-form-mathivick-formation',
  //                   'notif-panel', 'security-modal', ...)
  //   restoreFn: fonction de rappel pour restaurer exactement cet état
  // ═══════════════════════════════
  const _navStack = [];

  // ── État courant (tab + vue DOM active) ──
  function _currentState() {
    const activeTab = (() => {
      const el = document.querySelector('.btab.on');
      return el ? el.id.replace('t-', '') : 'home';
    })();
    const activeView = (() => {
      if (activeTab !== 'services') return null;
      const VIEWS = ['list','restaurants','kits','kit-detail','immo-options',
                     'immo-form','catalogue','form','delivery','payment','success'];
    ...
    return { tab: activeTab, view: activeView };
  }

  // ── Pousser un état complet (avec fonction de restauration) dans la pile ──
  function _pushNav(entry) {
    // entry = { tab, view, subState, restoreFn }
    _navStack.push(entry);
    history.pushState({ omni: true, depth: _navStack.length }, '');
  }

  // ── Créer une entrée simple à partir de l'état courant ──
  function _entryFromCurrent(overrides) {
    const s = _currentState();
    return Object.assign({ tab: s.tab, view: s.view, subState: null, restoreFn: null }, overrides || {});
  }

  // ═══════════════════════════════
  // WRAPPERS SUR LES FONCTIONS DE NAVIGATION
  // ═══════════════════════════════

  const _origGoTab                    = window.goTab;
  const _origShowView                 = window.showView;
  const _origOpenService              = window.openService;
  const _origShowTogoExpertiseMenu    = window.showTogoExpertiseMenu;
  const _origOpenConsultancesSubSvc   = window.openConsultancesSubService;
  const _origOpenConsultancesForm     = window.openConsultancesForm;
  const _origOpenNotifPanel           = window.openNotifPanel;
  const _origCloseNotifPanel          = window.closeNotifPanel;

  // ── goTab ──
  window.goTab = function (id) {
    const before = _entryFromCurrent({
      restoreFn: () => _origGoTab(_currentState().tab) // sera écrasé si besoin
    });
    const tabBefore = before.tab;
    _origGoTab(id);
    const tabAfter = _currentState().tab;
    if (tabBefore !== tabAfter) {
      // Restaurer = revenir à l'onglet précédent (simple)
      before.restoreFn = () => _origGoTab(tabBefore);
      _pushNav(before);
    }
  };
  ...
  window.showView = function (v) {
    const before = _entryFromCurrent();
    _origShowView(v);
    const afterView = _currentState().view;
    // Empiler uniquement si la vue DOM a changé
    if (before.view !== afterView) {
      before.restoreFn = () => _activateServicesTab(() => _origShowView(before.view || 'list'));
      _pushNav(before);
    }
  };
  ...
  window.openService = function (id) {
    const before = _entryFromCurrent();
    _origOpenService(id);
    const after = _currentState();
    if (before.tab !== after.tab || before.view !== after.view) {
      before.restoreFn = () => {
        if (before.tab === 'services') {
          _activateServicesTab(() => _origShowView(before.view || 'list'));
        } else {
          _origGoTab(before.tab);
        }
      };
      _pushNav(before);
    }
  ...
  // ── showTogoExpertiseMenu (niveau 1 : menu Consultances/Expertises) ──
  window.showTogoExpertiseMenu = function () {
    const before = _entryFromCurrent();
    _origShowTogoExpertiseMenu();
    // La vue immo-options est affichée — on différencie avec subState
    before.restoreFn = () => {
      if (before.tab === 'services') {
        _activateServicesTab(() => _origShowView(before.view || 'list'));
      } else {
  ...
  // ── openConsultancesSubService (niveau 2 : sous-service Mathivick / Omega) ──
  window.openConsultancesSubService = function (subId) {
    const before = _entryFromCurrent({ subState: 'consultances-menu' });
    _origOpenConsultancesSubSvc(subId);
    // La vue reste immo-options mais le contenu change → on force l'empilement
    before.restoreFn = () => _activateServicesTab(() => {
      if (typeof _origShowTogoExpertiseMenu === 'function') _origShowTogoExpertiseMenu();
    });
    _pushNav(before);
  };

  // ── openConsultancesForm (niveau 3 : formulaire d'une section) ──
  window.openConsultancesForm = function (subId, sectionId) {
    const before = _entryFromCurrent({ subState: 'consultances-sub-' + subId });
    _origOpenConsultancesForm(subId, sectionId);
    before.restoreFn = () => _activateServicesTab(() => {
      if (typeof _origOpenConsultancesSubSvc === 'function') _origOpenConsultancesSubSvc(subId);
    });
    _pushNav(before);
  };

  // ── openNotifPanel (overlay notifications) ──
  window.openNotifPanel = function () {
    const before = _entryFromCurrent({ subState: 'notif-panel' });
    _origOpenNotifPanel();
    before.restoreFn = () => {
  ...
  // closeNotifPanel reste inchangé — la fermeture via bouton ✕ ne doit pas
  // dépiler (on dépile seulement via le bouton retour physique).
  // Par contre, on doit retirer l'entrée de la pile quand l'utilisateur
  // ferme le panel manuellement (pour ne pas dépiler deux fois).
  window.closeNotifPanel = function () {
    _origCloseNotifPanel();
    // Retirer de la pile l'entrée notif-panel si elle est au sommet
    if (_navStack.length > 0 && _navStack[_navStack.length - 1].subState === 'notif-panel') {
      _navStack.pop();
      // On ne change pas l'historique du navigateur : le slot reste,
      // mais sera ignoré (pile déjà dépilée) au prochain popstate.
    }
  };
  ...
  // Activer visuellement l'onglet services sans déclencher goTab
  // (qui forcerait showView('list'))
  function _activateServicesTab(then) {
    document.querySelectorAll('.page').forEach(p => p.classList.remove('on'));
  ...
  function _closeOpenModal() {
    ...
    // Modales de paiement mobile (.pay-modal-overlay.show)
    const payOverlays = document.querySelectorAll('.pay-modal-overlay.show');
    if (payOverlays.length > 0) {
      payOverlays.forEach(el => el.classList.remove('show'));
      return true;
    }
    // Overlay USSD
    const ussdOverlay = document.getElementById('ussd-overlay');
    if (ussdOverlay && ussdOverlay.classList.contains('show')) {
      ussdOverlay.classList.remove('show');
      return true;
    }
    // Overlay résultat paiement
    const pgResult = document.getElementById('pg-result-overlay');
    if (pgResult && pgResult.classList.contains('show')) {
      pgResult.classList.remove('show');
      return true;
    }
    // Modale gardiennage / sécurité
    const securityModal = document.getElementById('security-type-modal');
    if (securityModal && securityModal.style.display === 'flex') {
      securityModal.style.display = 'none';
      return true;
    }
    // Modales génériques
    const genericModal = document.querySelector('[data-modal].open, .modal-overlay.open, .modal-overlay.show');
    if (genericModal) {
      genericModal.classList.remove('open', 'show');
      return true;
    }
    return false;
  }

  // ── Toast de confirmation de sortie ──
  let _exitPending = false;
  let _exitTimer   = null;
  let _bypassNext  = false; // court-circuite le prochain popstate (vrai départ)
  ...
  function _showExitToast() {
    ...
    if (!t) {
      ...
      t.style.cssText = [
        ...
        'background:#1E6FBE','color:#fff','padding:12px 24px',
        'border-radius:24px','font-size:14px','font-weight:600',
        'z-index:99999','box-shadow:0 4px 20px rgba(0,0,0,.35)',
  ...
  // ═══════════════════════════════
  // GESTIONNAIRE PRINCIPAL popstate
  // ═══════════════════════════════
  window.addEventListener('popstate', function () {

    // Court-circuit : on est en train de quitter réellement → laisser passer
    if (_bypassNext) {
      _bypassNext = false;
      return;
    }

    // 1. Fermer toute modale/overlay en priorité absolue
    if (_closeOpenModal()) {
      history.pushState({ omni: true, depth: _navStack.length }, '');
      _exitPending = false;
    ...
    // 2. Dépiler et exécuter la fonction de restauration
    if (_navStack.length > 0) {
      const entry = _navStack.pop();
      if (typeof entry.restoreFn === 'function') {
        entry.restoreFn();
      }
      history.pushState({ omni: true, depth: _navStack.length }, '');
      _exitPending = false;
    ...
    // 3. Pile vide mais pas sur l'accueil → forcer home sans permettre la sortie
    const cur = _currentState();
    if (cur.tab !== 'home') {
      _origGoTab('home');
      history.pushState({ omni: true, depth: 0 }, '');
      _exitPending = false;
      if (_exitTimer) { clearTimeout(_exitTimer); _exitTimer = null; }
      return;
    }

    // 4. Sur l'accueil — 1er appui : toast de confirmation
    if (!_exitPending) {
      _exitPending = true;
      _showExitToast();
      history.pushState({ omni: true, depth: 0 }, '');
      _exitTimer = setTimeout(() => { _exitPending = false; }, 2500);
      return;
    }

    // 5. Sur l'accueil — 2e appui confirmé : quitter réellement
    _exitPending = false;
    if (_exitTimer) { clearTimeout(_exitTimer); _exitTimer = null; }
    _bypassNext = true;   // le prochain popstate déclenché par go(-1) sera ignoré
    history.go(-1);
  });

  // ── Initialisation ──
  function _initHistory() {
    history.replaceState({ omni: true, depth: 0 }, '');
    history.pushState({ omni: true, depth: 0 }, '');
  }
