...
function loadGoogleMaps() {
  return new Promise((resolve, reject) => {
    ...
    s.id  = 'gmap-script';
    s.src = `https://maps.googleapis.com/maps/api/js?key=${GOOGLE_MAPS_API_KEY}&libraries=places&language=fr`;
    s.async = true;
    s.defer = true;
    s.onload  = () => { _placesLib = window.google.maps.places; _placesLibReady = true; resolve(); };
...
function _placesPredict(inputId, dropdownId) {
  ...
  // Calcul manuel des bounds sans Circle (évite la dépendance à la lib geometry)
  // 1 degré de latitude ≈ 111 km ; 1 degré de longitude ≈ 111 km × cos(lat)
