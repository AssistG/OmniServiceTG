...
window._catPay = function(svcId){
  ...
  // Réinitialiser le bouton retour vers view-delivery (et non vers le modal livraison & course)
  window._paymentBackOverride=null;
  const bkCat=document.querySelector('#view-payment .back-btn');
  if(bkCat)bkCat.onclick=()=>{if(window.showView)window.showView('delivery');};
};
...
window.openService=function(id){
  ...
  // Pour les services non-GPS, réinitialiser le handler au comportement par défaut
  window._cartBarClick=null;
  _os1(id);
};
...
window._livNext=function(){
  ...
  // Recâbler retour — stocker dans une variable globale temporaire pour pouvoir le réinitialiser
  window._paymentBackOverride=()=>openLivSheet();
