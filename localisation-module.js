...
window._kitGPS = function() {
  detectGPS(ov('kl-btn'),
    (la, lo) => {
      ...
      const km = haversine(la, lo, OMNI_DEPOT.lat, OMNI_DEPOT.lng);
      const {fare, detail} = fmtFare(km);
...
// Intercepter commanderKit dès qu'il est disponible
function _patchCommanderKit() {
  if(typeof window.commanderKit !== 'function') return;
  const _orig = window.commanderKit;
  window.commanderKit = function() {
    const cu = window._currentUser;
    if(!cu) { if(window.openAuthModal) window.openAuthModal('login'); return; }
    const kit = window.currentKit;
    if(!kit) return;
    // Remplir le panier exactement comme l'original
    window.cart = {};
    (kit.articles||[]).forEach((a,i) => {
      const id = `kit_${kit.id}_${i}`;
      window.cart[id] = {id, name:a.name, price:a.prix||0, qty:a.qty||1, emoji:a.emoji||'📦'};
    });
    if(kit.prix_total) {
      window.cart = {};
      window.cart[`kit_${kit.id}`] = {
        id:`kit_${kit.id}`, name:kit.nom, price:kit.prix_total, qty:1, emoji:kit.emoji||'🎁'
      };
    }
    if(window.updateCartBar) window.updateCartBar();
    // Ouvrir la modal GPS au lieu de view-delivery
    openKitLocModal();
  };
}
// Appliquer dès maintenant si déjà défini, sinon attendre
if(typeof window.commanderKit === 'function') {
  _patchCommanderKit();
} else {
  Object.defineProperty(window, 'commanderKit', {
    configurable: true,
    set(fn) {
      Object.defineProperty(window, 'commanderKit', {configurable:true, writable:true, value:fn});
      _patchCommanderKit();
    }
  });
}
