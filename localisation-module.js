...
function calcFare(km){ return km<=0?0:km<=1?500:500+Math.ceil(km-1)*100; }
function fmtFare(km){
  const fare=calcFare(km);
  return { fare, detail: km>1?`${km.toFixed(1)} km · 500+${Math.ceil(km-1)}×100 FCFA`:`${km.toFixed(1)} km · forfait 500 FCFA` };
}
