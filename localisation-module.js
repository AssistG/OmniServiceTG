...
async function roadDistance(la1,lo1,la2,lo2){
  ...
  // Instance publique : router.project-osrm.org
  try {
    ...
    if(data.code === 'Ok' && data.routes && data.routes[0]){
      ...
      const durationText = mins >= 60
        ...
        : `${mins} min`;
      console.log(`[roadDistance] OSRM ✅ ${km.toFixed(2)} km — ${durationText}`);
      return { km, durationText };
    }
    throw new Error('OSRM code: ' + data.code);
  } catch(err) {
    console.warn('[roadDistance] OSRM échoué, fallback haversine ×1.4 —', err.message);
