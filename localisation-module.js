...
async function roadDistance(la1,lo1,la2,lo2){
  // Utilise OSRM (OpenStreetMap Routing Machine) — gratuit, sans clé API, sans CORS
  // IMPORTANT : l'URL OSRM attend longitude PUIS latitude (pas lat,lng)
  // distance retournée en MÈTRES → diviser par 1000 pour km
  // On essaie d'abord router.project-osrm.org, puis demo.project-osrm.org en fallback
  const OSRM_SERVERS = [
    'https://router.project-osrm.org',
    'https://demo.project-osrm.org'
  ];
  for(const server of OSRM_SERVERS){
    try {
      // Format : /route/v1/driving/lng1,lat1;lng2,lat2
      const url = `${server}/route/v1/driving/${lo1},${la1};${lo2},${la2}?overview=false&annotations=false`;
      const resp = await fetch(url, { signal: AbortSignal.timeout(8000) });
      if(!resp.ok) throw new Error('HTTP ' + resp.status);
      const data = await resp.json();
      if(data.code === 'Ok' && data.routes?.[0]){
        const metres = data.routes[0].distance; // toujours en mètres
        ...
        const durationText = mins >= 60
          ...
          : `${mins} min`;
        console.log(`[roadDistance] OSRM ✅ (${server}) ${km.toFixed(2)} km — ${durationText}`);
        return { km, durationText };
      }
      throw new Error('OSRM code: ' + data.code);
    } catch(err) {
      console.warn(`[roadDistance] ${server} échoué —`, err.message);
    }
  }
  // Fallback haversine ×1.4 si les deux serveurs échouent
  console.warn('[roadDistance] Tous les serveurs OSRM échoués — fallback haversine ×1.4');
...
const RESTAURANT_MAX_FARE = 2000; // Seuil hors zone — restaurants uniquement (FCFA)
...
window._catPay = function(svcId){
  ...
  // Blocage hors zone — restaurants uniquement
  if(svcId === 'restaurant' && CAT.fare >= RESTAURANT_MAX_FARE){
    if(window.showToast) window.showToast('🚫 Commande hors zone — votre position est trop éloignée pour la livraison restaurant','#C62828');
    return;
  }
...
console.log('[OmniService] localisation-module.js v3 ✅ — Distance routière OSRM activée');
