/* ══════════════════════════════════════════════════════════════════
   OmniService TG — localisation-module.js  v6  (Geoapify Edition)
   Nouveautés v6 :
   ▸ Migration Google Places → Geoapify Autocomplete API
   ▸ Zéro dépendance SDK externe — appels REST purs (fetch)
   ▸ Restriction géographique : country:tg + biais Lomé conservé
   ▸ Icônes distinctives par type de lieu (station, resto, quartier…)
   ▸ Extraction lat/lng directe depuis la réponse Autocomplete
   ▸ Fare Card affiche le nom complet du lieu
...
// CONFIG
// ════════════════════════════════════════
const GEOAPIFY_API_KEY    = '1e7a591d064540deb4560def0a9b5888';
...
// ── Rayon maximal de la grande zone de Lomé (services non-restaurant) ──
// Au-delà de ce rayon à vol d'oiseau depuis le centre, commande refusée.
const LOME_MAX_KM = 20; // ~20 km depuis le centre = toute la grande agglomération

/**
 * Retourne true si le point (lat, lng) est hors de la grande zone de Lomé.
 * Utilise Haversine direct (sans ×1.4) pour un contrôle géographique pur.
 */
function isHorsZoneLome(lat, lng) {
  const R = 6371;
  const dLa = (lat - LOME_CENTER.lat) * Math.PI / 180;
  const dLo = (lng - LOME_CENTER.lng) * Math.PI / 180;
  const a   = Math.sin(dLa/2)**2 + Math.cos(LOME_CENTER.lat*Math.PI/180)*Math.cos(lat*Math.PI/180)*Math.sin(dLo/2)**2;
  const distKm = R * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1-a));
  return distKm > LOME_MAX_KM;
}

// ════════════════════════════════════════
// GEOAPIFY — PAS DE SDK, APPELS REST DIRECTS
// ════════════════════════════════════════
// Aucun script externe à charger. Geoapify fonctionne
// entièrement via fetch() natif du navigateur.
...
async function roadDistance(la1, lo1, la2, lo2) {
  // Geoapify ne propose pas de Distance Matrix gratuite.
  // On utilise Haversine × 1.4 (facteur route réaliste pour Lomé).
...
// ════════════════════════════════════════════════════════════════
// MOTEUR GEOAPIFY AUTOCOMPLETE
// ────────────────────────────────────────────────────────────────
// Remplace entièrement le moteur Google Places de la v5.
// Appels REST purs vers api.geoapify.com/v1/geocode/autocomplete
// Les coordonnées lat/lng sont incluses directement dans la réponse
// (pas besoin d'un second appel Place Details).
// ════════════════════════════════════════════════════════════════
...
// Cache des coordonnées par placeId Geoapify (évite les doublons)
const _geoCache = {};

/**
 * Lance la prédiction Geoapify pour un widget donné
 */
function _placesPredict(inputId, dropdownId) {
  ...
  const params = new URLSearchParams({
    ...
    lang    : 'fr',
    limit   : '7',
    filter  : 'countrycode:tg',
    bias    : `proximity:${LOME_CENTER.lng},${LOME_CENTER.lat}`,
    apiKey  : GEOAPIFY_API_KEY,
    format  : 'json',
  });

  fetch(`https://api.geoapify.com/v1/geocode/autocomplete?${params}`)
    ...
    .then(data => {
      ...
      dd.innerHTML = results.map((r, i) => {
        // Construire un identifiant stable à partir des coordonnées
        const pid     = `geo_${r.lat}_${r.lon}`;
        // Mémoriser les coords immédiatement (évite un 2ème appel réseau)
        _geoCache[pid] = { lat: r.lat, lng: r.lon };

        // Déterminer le nom principal et la ligne secondaire
        const main    = r.name || r.address_line1 || r.formatted || '';
        const secondary = r.address_line2 || r.city || r.county || '';
        const types   = [r.result_type, r.category].filter(Boolean);
        const icon    = placeIcon(types);

        const safeMain = main.replace(/'/g, '&#39;');
        const safeSec  = secondary.replace(/'/g, '&#39;');

        return `
        <div class="lsf-item" data-i="${i}"
             onmousedown="window._lsfPlaceSelect('${inputId}','${dropdownId}','${pid}','${safeMain}','${safeSec}')">
          <span class="lsf-item-ico">${icon}</span>
          <div class="lsf-item-body">
            <div class="lsf-item-name">${main}</div>
            <div class="lsf-item-commune">${secondary}</div>
          </div>
        </div>`;
      }).join('') + `<div class="lsf-powered">Propulsé par Geoapify</div>`;
...
/**
 * Sélection d'une suggestion → coordonnées déjà en cache → callbacks contextuels
 */
window._lsfPlaceSelect = function(inputId, dropdownId, placeId, mainText, secondaryText) {
  ...
  const cached = _geoCache[placeId];
  if (cached) {
    _dispatchPlaceCoords(inputId, cached.lat, cached.lng, mainText, secondaryText, placeId);
    return;
  }

  // Fallback : géocodage direct si le cache est absent (ne devrait pas arriver)
  const params = new URLSearchParams({
  ...
  fetch(`https://api.geoapify.com/v1/geocode/search?${params}`)
    ...
    .then(data => {
      ...
      if (!r?.lat) { if (window.showToast) window.showToast('⚠️ Impossible de localiser ce lieu — réessayez', '#F5820A'); return; }
      _dispatchPlaceCoords(inputId, r.lat, r.lon, mainText, secondaryText, placeId);
    })
    .catch(() => { if (window.showToast) window.showToast('⚠️ Erreur réseau — réessayez', '#F5820A'); });
};
...
/**
 * Dispatcher coordonnées → bon module selon l'inputId (remplace le callback getDetails de Google)
 */
function _dispatchPlaceCoords(inputId, lat, lng, name, address, placeId) {
...
function catDepot(svcId) {
  if (svcId === 'restaurant' && window.currentRestaurant) {
    ...
    if (r.lat && r.lng) {
      console.log('[catDepot] Restaurant →', r.nom, r.lat, r.lng);
      return { lat: parseFloat(r.lat), lng: parseFloat(r.lng) };
    }
    console.warn('[catDepot] Restaurant sans coordonnées admin — fallback OMNI_DEPOT', r);
  }
  if (svcId !== 'restaurant') {
    const items = Object.values(window.cart || {});
    const withLoc = items.find(a => a.lat && a.lng);
    if (withLoc) {
      console.log('[catDepot] Article avec loc →', withLoc.name, withLoc.lat, withLoc.lng);
      return { lat: parseFloat(withLoc.lat), lng: parseFloat(withLoc.lng) };
    }
  }
  console.log('[catDepot] Fallback → OMNI_DEPOT');
  return OMNI_DEPOT;
}
...
function _catSelectPlace(lat, lng, name, address, placeId) {
  ...
  // ── Contrôle hors-zone Lomé pour les services non-restaurant ──
  if (svcId !== 'restaurant' && isHorsZoneLome(lat, lng)) {
    CAT.lat = lat; CAT.lng = lng; CAT.zone = name; CAT.address = address;
    CAT.placeId = placeId; CAT.mode = 'place'; CAT.km = 0; CAT.fare = 0;
    _showHorsZoneLome('cg-farecard-wrap', name, address);
    _blockCatSubmit();
    return;
  }
...
window._catGPS = function(svcId) {
  ...
  detectGPS(ov('cg-btn'),
    async (la, lo) => {
      ...
      // ── Contrôle hors-zone Lomé pour les services non-restaurant ──
      if (svcId !== 'restaurant' && isHorsZoneLome(la, lo)) {
        CAT.km = 0; CAT.fare = 0;
        setCatCard(true, 'Position détectée ✅', `${la.toFixed(5)}° N, ${lo.toFixed(5)}° E`, 'Votre position actuelle');
        _showHorsZoneLome('cg-farecard-wrap', 'Votre position GPS', '');
        _blockCatSubmit();
        return;
      }
...
// ── Affiche la carte hors-zone Grande Lomé (services non-restaurant) ──
function _showHorsZoneLome(wrapId, name, address) {
  const wrap = ov(wrapId);
  if (!wrap) return;
  wrap.innerHTML = `
    <div class="lsf-fare-card hors-zone" style="margin-top:10px">
      <div style="font-size:26px">🚫</div>
      <div class="lsf-fare-zone">
        <div class="lsf-fare-qname">${name}</div>
        ${address ? `<div class="lsf-fare-commune">${address}</div>` : ''}
        <div class="lsf-fare-price">Hors zone</div>
        <div class="lsf-fare-detail">En dehors de la grande zone de Lomé</div>
      </div>
    </div>`;
  ov('loc-horszone-block')?.remove();
  const fc = ov('cg-fare');
  if (fc) { fc.style.display = 'flex'; fc.style.background = 'linear-gradient(135deg,#C62828,#B71C1C)'; }
  const fkmEl = ov('cg-fkm'); if (fkmEl) fkmEl.textContent = '— km';
  const fpEl  = ov('cg-fp');  if (fpEl)  fpEl.innerHTML    = '🚫 Hors zone';
  const fdEl  = ov('cg-fd');  if (fdEl)  fdEl.textContent  = 'Zone non desservie — hors Lomé';
  const hz = document.createElement('div');
  hz.id = 'loc-horszone-block'; hz.className = 'loc-horszone';
  hz.innerHTML = `
    <div class="loc-horszone-ico">🚫</div>
    <div class="loc-horszone-body">
      <div class="loc-horszone-title">Position hors zone</div>
      <div class="loc-horszone-msg">
        Votre position est en dehors de la grande agglomération de Lomé.<br/>
        Nos services de livraison couvrent uniquement la zone de Lomé et ses environs immédiats.
      </div>
      <div class="loc-horszone-detail">📍 Zone desservie : grande agglomération de Lomé (rayon ~${LOME_MAX_KM} km)</div>
      <div class="loc-horszone-tip">💡 <strong>Que faire ?</strong><br/>• Vérifiez votre position ou choisissez un lieu dans Lomé<br/>• Contactez-nous pour une livraison hors zone spéciale</div>
    </div>`;
  if (fc?.parentNode) fc.parentNode.insertBefore(hz, fc.nextSibling);
  if (window.showToast) window.showToast('🚫 Position hors de la zone de Lomé', '#C62828');
}

function _blockCatSubmit() {
  const subBtn = document.querySelector('#view-delivery .btn-primary');
  if (subBtn) { subBtn.disabled = true; subBtn.style.opacity = '0.45'; }
}

function _unblockCatSubmit() {
  const subBtn = document.querySelector('#view-delivery .btn-primary');
...
window._catPay = function(svcId) {
  ...
  if (svcId !== 'restaurant' && isHorsZoneLome(CAT.lat, CAT.lng)) {
    if (window.showToast) window.showToast('🚫 Position hors de la zone de Lomé', '#C62828');
    return;
  }
...
// Intercept openRestaurant — transmet lat/lng pour que catDepot() fonctionne
const _or = window.openRestaurant;
if (typeof _or === 'function') {
  window.openRestaurant = async function(rid, rnom, remo, rlat, rlng) {
    await _or(rid, rnom, remo, rlat, rlng);
    // Garantir que window.currentRestaurant a bien les coordonnées admin
    if (window.currentRestaurant) {
      window.currentRestaurant.lat = rlat || window.currentRestaurant.lat || null;
      window.currentRestaurant.lng = rlng || window.currentRestaurant.lng || null;
    }
...
window._livNext = function() {
  ...
  // Construire le récap dans view-payment
  ...
  // Boîte de trajet dans le récap
  const labelA = s.A.address ? `${s.A.label} <span style="color:#9999BB;font-size:10px">(${s.A.address})</span>` : s.A.label;
  const labelB = s.B.address ? `${s.B.label} <span style="color:#9999BB;font-size:10px">(${s.B.address})</span>` : s.B.label;
  let lb = ov('recap-loc-info');
  ...
  lb.innerHTML = `<strong style="font-size:12px;display:block;margin-bottom:5px">📍 Trajet</strong>
    <span style="color:#2E7D32">●</span> <strong>Départ :</strong> ${labelA}${s.A.desc ? `<br/><span style="color:#4A4A6A;margin-left:14px">${s.A.desc}</span>` : ''}
    <br/><span style="color:#C62828">●</span> <strong>Arrivée :</strong> ${labelB}${s.B.desc ? `<br/><span style="color:#4A4A6A;margin-left:14px">${s.B.desc}</span>` : ''}`;

  // Recâbler le bouton confirmer
  const cb = ov('confirm-btn');
  if (cb) { cb.onclick = window._livSubmit; cb.innerHTML = '🛵 Confirmer la livraison'; }
  // Recâbler le bouton retour
...
window._livSubmit = async function() {
  ...
  try {
    ...
    const ref = await addDoc(collection(db, 'commandes'), {
      ...
      pointA: {
        positionType: s.A.mode, lat: s.A.lat, lng: s.A.lng,
        placeName: s.A.label, placeAddress: s.A.address || '', placeId: s.A.placeId || '',
        desc: s.A.desc
      },
      pointB: {
        positionType: s.B.mode, lat: s.B.lat, lng: s.B.lng,
        placeName: s.B.label, placeAddress: s.B.address || '', placeId: s.B.placeId || '',
        desc: s.B.desc
      },
    ...
    const labelA = s.A.label; const labelB = s.B.label;
    if (sm) sm.innerHTML = `Livraison <strong style="color:var(--blue)">${labelA} → ${labelB}</strong> confirmée !<br/>Référence : <strong>#${ref.id.slice(0,8).toUpperCase()}</strong><br/>Distance : ${s.km.toFixed(1)} km — Tarif : ${fmtMoney(s.fare)}<br/>${pm === 'livraison' ? '💵 Paiement à la livraison.' : '📱 Paiement ' + (pLbl[pm] || pm) + ' à préparer.'}<br/><small style="color:var(--light)">Contact : ${cu.phone || ''}</small>`;
...
// Intercept openService delivery — RESTAURÉ depuis v3
...
console.log('[OmniService] localisation-module.js v6 ✅ — Geoapify Autocomplete · Zéro SDK · Lomé TG');
