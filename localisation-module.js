...
function calcFare(km){ return km<=0?0:km<2?500:500+Math.ceil(km-1)*100; }
function fmtFare(km){
  const fare=calcFare(km);
  return { fare, detail: km>=2?`${km.toFixed(1)} km · 500+${Math.ceil(km-1)}×100 FCFA`:`${km.toFixed(1)} km · forfait` };
}
...
function detectGPS(btnEl, onOk, onFail){
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    ()=>{ const la=6.1375+(Math.random()-.5)*.04,lo=1.2123+(Math.random()-.5)*.04; if(btnEl){btnEl.disabled=false;btnEl.innerHTML='🔄 Actualiser';} if(onFail) onFail('refused',la,lo); },
...
window._catGPS = function(svcId){
  ...
  detectGPS(ov('cg-btn'),
    (la,lo)=>{
      ...
      const d=catDepot(svcId),km=haversine(la,lo,d.lat,d.lng),{fare,detail}=fmtFare(km);
    ...
    (e,la,lo)=>{
      if(la===null){if(window.showToast)window.showToast('⚠️ GPS non disponible — choisissez un quartier','#F5820A');CAT.mode=null;return;}
      CAT.lat=la;CAT.lng=lo;
      const d=catDepot(svcId),km=haversine(la,lo,d.lat,d.lng),{fare,detail}=fmtFare(km);
      CAT.km=km;CAT.fare=fare;
      setCatCard(true,'Position simulée (démo)',`${la.toFixed(5)}° N, ${lo.toFixed(5)}° E`,'⚠️ GPS refusé — position approx. Lomé');
      setCatFare(km,fare,detail);
    }
  );
};
...
window._catZone = function(id,sel){
  ...
  const d=catDepot(svcId),km=haversine(CAT.lat,CAT.lng,d.lat,d.lng),{fare,detail}=fmtFare(km);
  CAT.km=km;CAT.fare=fare;
  setCatFare(km,fare,detail);
};
...
function setCatFare(km,fare,detail){
  document.getElementById('cg-fkm').textContent=km.toFixed(1)+' km';
  document.getElementById('cg-fp').textContent=fmtMoney(fare);
  document.getElementById('cg-fd').textContent=detail;
  const fc=ov('cg-fare');if(fc)fc.style.display='flex';
}
...
window.openService=function(id){
  if(CATALOGUE_GPS_SERVICES.includes(id)){
    ...
    setTimeout(()=>{
      buildCatDelivery(id);
      // Utiliser window._cartBarFn lu par le onclick HTML de la cart-bar
      window._cartBarFn=()=>{
  ...
  window._cartBarFn=null; // service non-GPS : comportement par défaut
  _os1(id);
};
...
window._livGPS=function(pt){
  ...
  detectGPS(ov('gb-'+pt),
    ...
    (e,la,lo)=>{
      if(la===null){if(window.showToast)window.showToast('⚠️ GPS non disponible — choisissez un quartier','#F5820A');LIV[pt].mode=null;return;}
      LIV[pt].lat=la;LIV[pt].lng=lo;LIV[pt].label='Position approx.';
      setPtCard(pt,true,'Position simulée (démo)',`${la.toFixed(5)}° N, ${lo.toFixed(5)}° E`,'⚠️ GPS refusé');
      calcLiv();
    }
  );
};
...
function calcLiv(){
  ...
  const km=haversine(LIV.A.lat,LIV.A.lng,LIV.B.lat,LIV.B.lng);
  ...
  ov('lv-fd').textContent=`${LIV.A.label} → ${LIV.B.label} · ${detail}`;
    ...
    <div class="loc-body">
      ...
      <div class="loc-gc" id="mg-card">
        ...
        <div class="loc-gi">
          ...
          <div class="loc-gs" id="mg-s">Distance depuis Adidogomé (base OmniService)</div>
        </div>
      ...
      <div class="loc-fare" id="mg-fare" style="display:none;margin-top:14px">
        <div style="font-size:26px">📏</div>
        <div style="flex:1">
          <div style="font-size:10px;color:rgba(255,255,255,.65);text-transform:uppercase;letter-spacing:.4px;margin-bottom:3px">Depuis Adidogomé (base OmniService)</div>
          <div style="display:flex;align-items:baseline;gap:8px;flex-wrap:wrap">
...
window._mGPS=function(){
  ...
  detectGPS(ov('mg-btn'),
    (la,lo)=>{
      const km=haversine(la,lo,ADIDOGOME.lat,ADIDOGOME.lng),{fare,detail}=fmtFare(km);
      window._mGps={lat:la,lng:lo,km,fare};
      const card=ov('mg-card');if(card){card.classList.remove('off');card.classList.add('on');}
      ov('mg-t').textContent='Position détectée ✅';ov('mg-c').textContent=`${la.toFixed(5)}° N, ${lo.toFixed(5)}° E`;ov('mg-s').textContent="Votre position d'intervention";
      showMF(km,fare,detail);
    },
    (e,la,lo)=>{
      if(la===null){if(window.showToast)window.showToast('⚠️ GPS non disponible — choisissez un quartier','#F5820A');return;}
      const km=haversine(la,lo,ADIDOGOME.lat,ADIDOGOME.lng),{fare,detail}=fmtFare(km);
      window._mGps={lat:la,lng:lo,km,fare};
      const card=ov('mg-card');if(card){card.classList.remove('off');card.classList.add('on');}
      ov('mg-t').textContent='Position simulée (démo)';ov('mg-c').textContent=`${la.toFixed(5)}° N, ${lo.toFixed(5)}° E`;ov('mg-s').textContent='⚠️ GPS refusé';
      showMF(km,fare,detail);
    }
  );
};
...
window._mZone=function(id,sel){
  ...
  if(!v){
    ...
    ov('mg-t').textContent='Localisation GPS';ov('mg-c').textContent='Non détectée';ov('mg-s').textContent='Distance depuis Adidogomé';
    return;
  }
  ...
  const[la,lo,name]=v.split('|');
  const km=haversine(parseFloat(la),parseFloat(lo),ADIDOGOME.lat,ADIDOGOME.lng),{fare,detail}=fmtFare(km);
  window._mGps={lat:parseFloat(la),lng:parseFloat(lo),km,fare,label:name};
  sel.classList.add('on');
  showMF(km,fare,detail);
};

function showMF(km,fare,detail){
  ov('mg-fkm').textContent=km.toFixed(1)+' km depuis Adidogomé';
...
window._mSubmit=async function(){
  ...
  try{
    ...
    const ref=await addDoc(collection(db,'commandes'),{
      ...
      refLat:ADIDOGOME.lat,refLng:ADIDOGOME.lng,refLabel:'Adidogomé',
      uid:cu.uid,clientNom:cu.nom||'',clientPrenom:cu.prenom||'',clientGenre:cu.genre||'',phone:cu.phone||'',clientVille:cu.ville||'',
      createdAt:serverTimestamp(),
    });
    const sm=ov('succ-msg');
...
// Remplace la vue "Informations de livraison" (view-delivery)
// par la modal GPS du module (sans select de quartier)
// pour le service kits/packs.
// ══════════════════════════════════════════════

function openKitLocModal(){
  const cu=window._currentUser;
  if(!cu){if(window.openAuthModal)window.openAuthModal('login');if(window.showToast)window.showToast('⚠️ Connectez-vous','#F5820A');return;}
  if(!window.currentKit){if(window.showToast)window.showToast('⚠️ Kit non sélectionné','#F5820A');return;}

  window._kitGps=null;

  const el=makeOv('ov-kit-loc');
  const kit=window.currentKit;
  const svc=window.SVCS&&window.SVCS['kits'];
  const soon=(!svc||(!svc.active&&svc.soon))?`<div style="background:#FFF3E0;border-radius:12px;padding:10px 14px;margin-bottom:14px;border:1px solid #FFE082;font-size:12px;color:#E65100">⏳ <strong>Bientôt disponible</strong> — ${svc&&svc.soon?svc.soon:''}. Pré-enregistrement possible.</div>`:'';
  ...
  el.innerHTML=`<div class="loc-sh">
    ...
    <div class="loc-body">
      ...
      <button class="loc-btn" id="kl-sub" onclick="window._kitSubmit()">🛒 Confirmer la commande</button>
    </div>
  </div>`;
  document.body.appendChild(el);
  el.addEventListener('click',e=>{if(e.target===el)el.remove();});
...
window._kitGPS=function(){
  detectGPS(ov('kl-btn'),
    (la,lo)=>{
      ...
      const d=OMNI_DEPOT,km=haversine(la,lo,d.lat,d.lng),{fare,detail}=fmtFare(km);
      window._kitGps.km=km;window._kitGps.fare=fare;
      const card=ov('kl-card');if(card){card.classList.remove('off');card.classList.add('on');}
      ov('kl-t').textContent='Position détectée ✅';
      ov('kl-c').textContent=`${la.toFixed(5)}° N, ${lo.toFixed(5)}° E`;
      ov('kl-s').textContent='Votre position actuelle';
    ...
    (e,la,lo)=>{
      if(la===null){if(window.showToast)window.showToast('⚠️ GPS non disponible — saisissez votre adresse','#F5820A');return;}
      window._kitGps={lat:la,lng:lo};
      const d=OMNI_DEPOT,km=haversine(la,lo,d.lat,d.lng),{fare,detail}=fmtFare(km);
      window._kitGps.km=km;window._kitGps.fare=fare;
      const card=ov('kl-card');if(card){card.classList.remove('off');card.classList.add('on');}
      ov('kl-t').textContent='Position simulée (démo)';
      ov('kl-c').textContent=`${la.toFixed(5)}° N, ${lo.toFixed(5)}° E`;
      ov('kl-s').textContent='⚠️ GPS refusé — position approx. Lomé';
      ov('kl-fkm').textContent=km.toFixed(1)+' km';
      ov('kl-fp').textContent=fmtMoney(fare);
      ov('kl-fd').textContent=detail;
      const fc=ov('kl-fare');if(fc)fc.style.display='flex';
...
window._kitSubmit=async function(){
  const cu=window._currentUser;
  if(!cu){if(window.openAuthModal)window.openAuthModal('login');return;}
  const kit=window.currentKit;
  ...
  try{
    ...
    // Construire les articles du kit pour la commande
    ...
    const positionData=g
      ?{positionType:'GPS',clientLat:g.lat,clientLng:g.lng,distanceKm:parseFloat(g.km.toFixed(2)),fraisLivraison:g.fare}
      :{positionType:'description',positionDesc:desc};

    const ref=await addDoc(collection(db,'commandes'),{
      service:'kits',
      serviceName:`Kits/PACKS — ${kit.nom}`,
      statut:'En attente',
      kitId:kit.id,kitNom:kit.nom,
      articles:items,
      total,
      adresse:desc,
      notes,
    ...
    const sm=ov('succ-msg');
    if(sm)sm.innerHTML=`Commande <strong style="color:var(--blue)">${kit.nom}</strong> confirmée !<br/>Référence : <strong>#${ref.id.slice(0,8).toUpperCase()}</strong><br/>${g?`🛵 Frais de livraison : ${fmtMoney(g.fare)}<br/>`:''}💵 Paiement à la livraison — notre agent vous contacte bientôt.<br/><small style="color:var(--light)">Contact : ${cu.phone||''}</small>`;
...
// Intercepter commanderKit pour afficher la modal GPS au lieu de view-delivery
const _origCommanderKit=window.commanderKit;
window.commanderKit=function(){
  if(!window._currentUser){if(window.openAuthModal)window.openAuthModal('login');return;}
  if(!window.currentKit){if(window.showToast)window.showToast('⚠️ Kit non sélectionné','#F5820A');return;}
  // Préparer le panier comme le fait l'original, puis ouvrir la modal GPS
  const kit=window.currentKit;
  window.cart={};
  const articles=kit.articles||[];
  articles.forEach((a,i)=>{
    const id=`kit_${kit.id}_${i}`;
    window.cart[id]={id,name:a.name,price:a.prix||0,qty:a.qty||1,emoji:a.emoji||'📦'};
  });
  if(kit.prix_total){
    window.cart={};
    window.cart[`kit_${kit.id}`]={id:`kit_${kit.id}`,name:kit.nom,price:kit.prix_total,qty:1,emoji:kit.emoji||'🎁'};
  }
  if(window.updateCartBar)window.updateCartBar();
  openKitLocModal();
};
...
console.log('[OmniService] localisation-module.js v2 ✅');
