...
function setCatFare(km,fare,detail){
  ...
  if(horsZone){
    ...
    if(fdEl) fdEl.textContent = `${km.toFixed(1)} km — Livraison non disponible dans cette zone`;
    if(subBtn){ subBtn.disabled=true; subBtn.style.opacity='0.45'; }
    if(window.showToast) window.showToast('🚫 Hors zone — Livraison non disponible à cette distance','#C62828');
  } else {
