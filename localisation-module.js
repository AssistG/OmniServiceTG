...
async function roadDistance(la1,lo1,la2,lo2){
  // Utiliser le SDK Maps JS (DistanceMatrixService) pour éviter le blocage CORS
  // du fetch direct vers l'API Distance Matrix JSON.
        ...
        svc.getDistanceMatrix({
          ...
          if(status === 'OK'){
            ...
            if(el?.status === 'OK' && el.distance?.value){
              ...
              return resolve({ km, durationText: dur });
            }
          }
          // Fallback haversine si le SDK échoue
          console.warn('[roadDistance] SDK fallback haversine ×1.4 — status:', status);
          resolve({ km: haversine(la1,lo1,la2,lo2)*1.4, durationText:'' });
        });
      } else {
        // SDK Maps JS pas encore chargé — fallback haversine
        console.warn('[roadDistance] Google Maps SDK absent — fallback haversine ×1.4');
        resolve({ km: haversine(la1,lo1,la2,lo2)*1.4, durationText:'' });
      }
    } catch(err) {
      console.warn('[roadDistance] Erreur —', err.message, '— fallback haversine ×1.4');
      resolve({ km: haversine(la1,lo1,la2,lo2)*1.4, durationText:'' });
    }
  });
}
...
const RESTAURANT_MAX_FARE = 2000; // Seuil hors zone — tous services catalogue (FCFA)

function setCatFare(km,fare,detail){
  const horsZone = fare >= RESTAURANT_MAX_FARE;
...
window._catPay = function(svcId){
  ...
  // Blocage hors zone — tous services catalogue
  if(CAT.fare >= RESTAURANT_MAX_FARE){
