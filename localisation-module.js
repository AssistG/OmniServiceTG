...
function setCatFare(km,fare,detail){
  ...
  if(horsZone){
    ...
    // Insérer le bloc juste après le panneau de frais
    // Utilise requestAnimationFrame pour garantir que le DOM est stable
    // après le calcul asynchrone (GPS / roadDistance)
    requestAnimationFrame(() => {
      ov('loc-horszone-block')?.remove(); // éviter les doublons
      const fc2 = ov('cg-fare');
      const container = (fc2 && fc2.parentNode)
        ? fc2.parentNode
        : document.querySelector('#view-delivery .order-card');
      if (container) {
        if (fc2 && fc2.parentNode === container) {
          container.insertBefore(hz, fc2.nextSibling);
        } else {
