...
// ════════════════════════════════════════
function goTab(id) {
  // Annuler le compte à rebours de la vue succès si l'utilisateur navigue manuellement
  if (typeof cancelSuccessRedirect === 'function') cancelSuccessRedirect();
...
// ── Timer global du compte à rebours succès (annulable) ──
let _succRedirectTimer = null;

function cancelSuccessRedirect() {
  if (_succRedirectTimer) {
    clearInterval(_succRedirectTimer);
    _succRedirectTimer = null;
  }
}
window.cancelSuccessRedirect = cancelSuccessRedirect;
...
function showSuccessView() {
  ...
  // Annuler tout timer précédent, puis démarrer le nouveau
  cancelSuccessRedirect();
  ...
  _succRedirectTimer = setInterval(() => {
    count--;
    if (cdEl) cdEl.textContent = count;
    if (count <= 0) {
      cancelSuccessRedirect();
...
function openService(id) {
  // Annuler le compte à rebours de la vue succès si l'utilisateur ouvre un service
  cancelSuccessRedirect();
...
async function submitConsultancesForm() {
  ...
  if (!opt) {
    showToast('⚠️ Veuillez sélectionner une prestation avant d\'envoyer', '#F5820A');
    return;
  }
  ...
  try {
    ...
    const succBtn = document.getElementById('succ-view-orders-btn');
    if (succBtn) succBtn.style.display = 'block';
    // Réinitialiser le bouton et vider les champs
    if (btn) { btn.innerHTML = '📨 Envoyer ma demande'; btn.disabled = false; }
    ['immo-ff-besoin','immo-ff-localisation','immo-ff-date','immo-ff-notes'].forEach(id => {
      const el = document.getElementById(id);
