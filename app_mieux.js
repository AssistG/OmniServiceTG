...
// ════════════════════════════════════════
// PAYGATE — désactivé (à reconfigurer)
// Remplacer PAYGATE_TOKEN par votre vraie clé API paygateglobal.com
// et brancher votre proxy (Netlify Function, Cloudflare Worker, etc.)
// ════════════════════════════════════════
const PAYGATE_ACTIVE = false; // Mettre true après configuration du proxy

// Stub — sera remplacé lors de la reconnexion du proxy
async function paygateInitPayment({ channel, amount, phone, reference, description }) {
  throw new Error('Paiement mobile non configuré. Veuillez choisir "Paiement à la livraison".');
}

async function paygateCheckStatus(transactionId) {
  throw new Error('Paiement mobile non configuré.');
}

function paygateWaitForPayment(transactionId, maxWait = 120000) {
  return Promise.resolve({ success: false, status: 'NON_CONFIGURE', message: 'Paiement mobile non configuré.' });
}
